...
int TreeCallHook(TreeshrHookType htype, TREE_INFO *info, int nid)
{
  ...
  static int (*rtn)(TreeshrHookType,char *, int,int) = 0;
  int status = 1;
  if (rtn == (int (*)(TreeshrHookType,char *,int,int))-1) 
    return 0;
  else if (rtn == 0)
    status = LibFindImageSymbol(&image,&rtnname,&rtn);
  if (status & 1)
    return (*rtn)(htype, info->treenam, info->shot,nid);
  else
    rtn = (int (*)(TreeshrHookType,char *,int,int))-1;
  return status;
}
#endif
