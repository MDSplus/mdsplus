...
static int Refresh();
static Widget mw;
typedef struct _info { unsigned char on;
...
int       main(int argc, String *argv)
{
  ...
  top = XtVaAppInitialize(&app_context, "Actions", options, XtNumber(options), &argc, argv, NULL, NULL);
...
static void ToggleEssential(Widget w, XtPointer tag, XmListCallbackStruct *cb)
{
  ...
  if (i < num_actions)
  {
    ...
    if (flags & NciM_ESSENTIAL)
    {
      ...
      static NCI_ITM itmlst[] = {{sizeof(flags),NciCLEAR_FLAGS,(unsigned char *)&flags,0},{0,NciEND_OF_LIST,0,0}};
    ...
    {
      static int flags = NciM_ESSENTIAL;
      static NCI_ITM itmlst[] = {{sizeof(flags),NciSET_FLAGS,(unsigned char *)&flags,0},{0,NciEND_OF_LIST,0,0}};
      TreeSetNci(actions[i].nid,itmlst);
      if (strlen(item_string) > 45)
        item_string[45] = '*';
    }
...
static int Refresh()
{
  ...
  if (num_actions)
  {
    ...
    for (item_idx=1,i=0;i<num_actions;i++)
    {
      ...
      {
        item = XmStringCreateSimple("********************** The following nodes are not included in pulse *********************");
      ...
      {
        item = XmStringCreateSimple("****************************** The following nodes are turned off *********************");
        XmListAddItem(w, item, 0);
        XmStringFree(item);
        item_idx++;
      ...
      {
        static String line_s = "************************** Phase: 12345678901234567890123456789012*********************";
        ...
        {
          case 0x10000001: StrCopyDx(&error,&bad_phase); break;
          case 0x10000002: StrCopyDx(&error,&bad_index); break;
          case 0x10000003: StrCopyDx(&error,&bad_server); break;
          case 0x10000004: StrCopyDx(&error,&not_sequential); break;
          case 0x10000005: StrCopyDx(&error,&not_an_action); break;
          default: if (TdiExecute(&phase_lookup,&phase_d,&phase MDS_END_ARG)&1)
                     StrCopyDx(&prefix,&phase_str);
                   else
                     StrCopyDx(&error,&bad_phase); break;
        }
