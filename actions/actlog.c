...
static void PutLog(char *time, char *mode, char *status, char *server, char *path){
  fprintf(stdout, "%s %12d %-10.10s %-44.44s %-20.20s %s", time, current_shot, mode, status, server, path);
}

static void PutError(char *time, char* mode, char *status, char *server, char *path){
  fprintf(stderr, "%s %12d %-10.10s %-44.44s %-20.20s %s", time, current_shot, mode, status, server, path);
}
