...
MessageSender *TCPMessageManager::getSender(NetworkAddress *addr)
{
	return new TCPMessageSender((IPAddress *)addr, this);
}
...
bool TCPMessageManager::sendMessage(NetworkAddress *addr, char *buf, int bufLen)
{
...
unsigned int TCPMessageManager::fromNative(unsigned int n)
{
	return htonl(n);
}

unsigned int TCPMessageManager::toNative(unsigned int n)
{
	return ntohl(n);
}
