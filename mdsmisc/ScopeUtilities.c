...
static void compressData(float *y, char *x, int xSize, int xType, int nSamples, int reqPoints,
			 float xMin, float xMax, int *retPoints, float *retResolution)
{
...
//From here, consider xMin and xMax
  for (startIdx = 0; startIdx < nSamples && toDouble(&x[startIdx * xSize], xType) < xMin;
       startIdx++) ;
  if (startIdx == nSamples)
    startIdx--;
  for (endIdx = startIdx; endIdx < nSamples && toDouble(&x[endIdx * xSize], xType) < xMax;
       endIdx++) ;
  if (endIdx == nSamples)
    endIdx--;
...
//Adjust to a number of samples close to 2 * reqPoints
  ...
  if (deltaSamples == 1)
    ...
    *retResolution =
	reqPoints / (toDouble(&x[endIdx * xSize], xType) - toDouble(&x[startIdx * xSize], xType));

    printf("Resolution: %f, points: %d, interval: %f\n", *retResolution, reqPoints,
	   (toDouble(&x[endIdx * xSize], xType) - toDouble(&x[startIdx * xSize], xType)));
  }
...
static char *recGetHelp(struct descriptor *dsc)
{
  ...
  case CLASS_A:
    return NULL;
  case CLASS_XD:
    if (!((struct descriptor_xd *)dsc)->pointer)
      return NULL;
  ...
  case CLASS_R:
	...
	if ((status & 1) && xd.pointer && xd.pointer->class == CLASS_S
	    && xd.pointer->dtype == DTYPE_T) {
...
static char *recGetUnits(struct descriptor *dsc, int isX)
{
  ...
  case CLASS_R:
	...
	&& (*(short *)rDsc->pointer == OpcAdd || *(short *)rDsc->pointer == OpcSubtract)) {
    ...
    if (rDsc->dtype == DTYPE_FUNCTION && rDsc->pointer
	&& (*(short *)rDsc->pointer == OpcMultiply || *(short *)rDsc->pointer == OpcDivide)) {
...
struct descriptor_xd *GetXYSignal(char *inY, char *inX, float *inXMin, float *inXMax,
				  int *reqNSamples)
{
...
//Number of asmples set to minimum between X and Y
  ...
  compressData(y, xArrD->pointer, xArrD->length, xArrD->dtype, nSamples, *reqNSamples, *inXMin,
	       *inXMax, &retSamples, &retResolution);
