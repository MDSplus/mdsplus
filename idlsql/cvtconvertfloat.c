/*  CMS REPLACEMENT HISTORY, Element CVTCONVERTFLOAT.C */
/*  *21   30-APR-1999 11:46:15 TWF "Remove IsRoprand" */
/*  *20   30-APR-1999 11:43:26 TWF "Add IsRoprand function" */
/*  *19    3-SEP-1996 13:26:28 TWF "FIx compile errors" */
/*  *18   28-AUG-1996 12:03:00 TWF "Fix for osf" */
/*  *17   28-AUG-1996 11:33:10 TWF "Quicker Floating conversion" */
/*  *16   28-AUG-1996 11:26:52 TWF "Quicker Floating conversion" */
/*  *15   28-AUG-1996 09:20:55 TWF "Replace with new version" */
/*  *14   27-AUG-1996 08:04:46 TWF "Fix compile errors" */
/*  *13   27-AUG-1996 07:51:43 TWF "Put IsRoprand in another module" */
/*  *12   22-AUG-1996 15:23:32 TWF "Fix g2s" */
/*  *11   22-AUG-1996 14:55:21 TWF "Fix ieee conversion when exp <= 2" */
/*  *10   22-AUG-1996 14:37:26 TWF "Fix conversion to ieee when exponent < 2" */
/*  *9    22-AUG-1996 12:51:54 TWF "Fix IsRoprand" */
/*  *8    22-AUG-1996 12:40:34 TWF "Fix IsRoprand" */
/*  *7    22-AUG-1996 12:31:39 TWF "Check for -0.0 in ieee" */
/*  *6    21-AUG-1996 09:43:32 TWF "Check for roprand on converting to same" */
/*  *5    15-AUG-1996 09:53:21 TWF "New version from sourcesafe" */
/*  *4    12-AUG-1996 10:48:30 TWF "Fix port" */
/*  *3    12-AUG-1996 10:43:20 TWF "Fix IsRoprand" */
/*  *2    12-AUG-1996 09:38:12 TWF "Add ieee types" */
/*  *1    12-AUG-1996 09:23:27 TWF "Convert floating point to any other floating point type" */
/*  CMS REPLACEMENT HISTORY, Element CVTCONVERTFLOAT.C */
#include <string.h>
#include <descrip.h>
#ifdef _MSC_VER
...
#define CVT$K_VAX_F DSC$K_DTYPE_F                   /* VAX F     Floating point data    */
#define CVT$K_VAX_D DSC$K_DTYPE_D                   /* VAX D     Floating point data    */
#define CVT$K_VAX_G DSC$K_DTYPE_G                   /* VAX G     Floating point data    */
#define CVT$K_VAX_H DSC$K_DTYPE_H                   /* VAX H     Floating point data    */
#define CVT$K_IEEE_S DSC$K_DTYPE_FS                  /* IEEE S    Floating point data    */
#define CVT$K_IEEE_T DSC$K_DTYPE_FT                  /* IEEE T    Floating point data    */
...
static void round(UNPACKED_REAL	intermediate_value,
  ...
  else 
  {
    ...
    */
    else 
    {
      ...
      **-----------------------------------------------------------------------
      */
      intermediate_value[1] = ((intermediate_value[1] << 16) | 
    ...
    */	  
    else 
    {
      ...
      */	  
      intermediate_value[1] = ((intermediate_value[1] << 16) | 
                               (intermediate_value[1] >> 16));
      intermediate_value[2] = ((intermediate_value[2] << 16) | 
    ...
    */	  
    else 
    {
      ...
      */	  
      ...
      intermediate_value[2] = ((intermediate_value[2] << 16) | 
			       (intermediate_value[2] >> 16));
      intermediate_value[3] = ((intermediate_value[3] << 16) | 
			       (intermediate_value[3] >> 16));
      intermediate_value[4] = ((intermediate_value[4] << 16) | 
			       (intermediate_value[4] >> 16));
  ...
  else 
  {
	...
	*/
	...
	if (options & CVT$M_BIG_ENDIAN) 
	{
	  ...
	  intermediate_value[0] |= ((intermediate_value[1] << 8) & 0x00FF0000L);
	  intermediate_value[0] |= ((intermediate_value[1] >> 8) & 0x0000FF00L);
	  intermediate_value[1]  = ((intermediate_value[2] << 24) | 
				    (intermediate_value[2] >> 24));
  ...
  else 
  {
	...
	*/
	...
	if (options & CVT$M_BIG_ENDIAN) 
	{
	  ...
	  intermediate_value[2]  = ((intermediate_value[3] << 24) | 
				    (intermediate_value[3] >> 24));
	  intermediate_value[2] |= ((intermediate_value[3] << 8) & 0x00FF0000L);
	  intermediate_value[2] |= ((intermediate_value[3] >> 8) & 0x0000FF00L);

	  intermediate_value[3]  = ((intermediate_value[4] << 24) | 
				    (intermediate_value[4] >> 24));
    ...
    */	  
    else 
    {
      ...
      */
      ...
      if (i) 
      {
	if (intermediate_value[U_R_EXP] > U_R_BIAS) j += 1;
	i = 12 - i;
      } 
      else 
      {
...

{
  ...
  */	  
  if ( ! (options & CVT$M_TRUNCATE) ) 
  {
	...
	** --------------------------------------------------------------------
	*/
	default : if ( ! (more_bits = intermediate_value[i] & (bit_mask - 1)) )
		    switch ( i ) 
		    {
		      case  1 : more_bits  = intermediate_value[2];
		      case  2 : more_bits |= intermediate_value[3];
		      case  3 : more_bits |= intermediate_value[4];
		      default : break;
		    } 
    ...
    */	  
    if ( roundup ) 
    {
      ...
      */	  
      bit_mask <<= 1;
      intermediate_value[i] = (intermediate_value[i] & ~(bit_mask - 1)) + 
                               bit_mask;
