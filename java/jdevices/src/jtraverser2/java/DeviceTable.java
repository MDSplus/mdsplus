...
*/          int firstCol = (displayRowNumber)?1:0;
    ...
    public void initializeData(Descriptor<?> data, boolean is_on)
    {
	initializing = true;
	String decompiled = "";
	try {
...
//        for(int i = 1; i < numRows/2; i++)
    ...
    public void displayData(Descriptor<?> data, boolean is_on)
    {
	state = is_on;
	String decompiled = data.decompile();
	StringTokenizer st = new StringTokenizer(decompiled, ",[]");
	items = new String[numCols * numRows];
	int idx = 0;
	while( idx < numCols * numRows && st.hasMoreTokens())
...
//        for(int i = 0; i < numCols; i++)
	...
	try {
		if(useExpressions)
		    return subtree.api.tdiCompile(subtree, "COMPILE(\'"+expandBackslash(dataString)+"\')").getData();
		else
