...
*/
    ...
    protected void initializeData(Descriptor<?> data, boolean is_on)
    {
        ...
        //Min X
        try {
            currNid = new Nid(nidData.getNidNumber() + 1, subtree);
            currData = currNid.getData();
            minX = minXOld = currData.toFloat();
        }catch(Exception exc) {minX = minXOld = 0;}
        //Max X
        try {
            currNid = new Nid(nidData.getNidNumber() + 2, subtree);
            currData = currNid.getData();
            maxX = maxXOld = currData.toFloat();
        }catch(Exception exc) {maxX = maxXOld = 1;}

        //Min Y
        try {
            currNid = new Nid(nidData.getNidNumber() + 3, subtree);
            currData = currNid.getData();
            minY = minYOld = currData.toFloat();
        }catch(Exception exc) {minY = minYOld = 0;}
        //Max Y
        try {
            currNid = new Nid(nidData.getNidNumber() + 4, subtree);
            currData = currNid.getData();
            maxY = maxYOld = currData.toFloat();
        ...
        try {
            ...
            currNid = nidData;
            currData = currNid.getData();
	    yData = ((Signal)currData).getValue();
	    xData = ((Signal)currData).getDimension();
            currY = yData.toFloatArray();
            currX = xData.toFloatArray();
        }catch(Exception exc)
        {
    ...
    {
        Descriptor<?> dim = new Float32Array(waveX);
        Descriptor<?> values = new Float32Array(waveY);
        return new Signal(values, null, dim);
    }
    ...
    public void apply() throws Exception
    {
        ...
        super.apply();
        updateLimits();
        int nid = nidData.getNidNumber();
        if(minXVisible)
        {
            try {
                subtree.putRecord(nid + 1, new Float32(minX));
            }catch(Exception exc)
            {
        ...
        {
            try {
            	subtree.putRecord(nid + 2, new Float32(maxX));
            }catch(Exception exc)
            {
        ...
        {
            try {
            	subtree.putRecord(nid + 3, new Float32(minY));
            }catch(Exception exc)
            {
        ...
        {
            try {
            	subtree.putRecord(nid + 4, new Float32(maxY));
            }catch(Exception exc)
            {
    ...
    public void fireUpdate(String updateId, Descriptor<?> newExpr)
    {
        if(updateIdentifier != null && updateExpression != null && updateIdentifier.equals(updateId))
        {
            ...
            try {
                ...
                //Compute new Max
                Descriptor<?> newData = subtree.getMds().getDescriptor(newExprStr);
                maxY = newData.toFloat();
