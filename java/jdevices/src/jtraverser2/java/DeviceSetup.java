    ...
    extends JDialog
{
    ...
    protected Vector<Component> device_components = new Vector<Component>();
    protected Vector<Component> device_controls = new Vector<Component>();
    public TREE subtree = null;
    protected Vector<?> dataChangeListeners = new Vector<Object>();
    protected String[] methods;
    protected DeviceButtons buttons = null;
    JMenuItem pop_items[];
    JPopupMenu pop_methods = null;
    Hashtable<String, Vector<Component>> updateHash = new Hashtable<String, Vector<Component>>();
    ...
    {
	this(JOptionPane.getRootFrame(), readOnly);
	setTitle(deviceTitle);
...
//        getContentPane().setLayout(new BorderLayout());
    ...
    public DeviceSetup(Frame frame, boolean readOnly)
    {
	super(frame);
	this.readOnly = readOnly;
	setTitle(deviceTitle);
    ...
    public void configure(TREE subtree, int baseNid)
    {
    ...
    public void configure(TREE subtree, int baseNid, Nid node)
    {
	deviceNode = node;
	...
	try
	{
		path = subtree.getNciFullPath(baseNid);
	}
	catch (Exception exc)
	{
	    System.out.println(exc);
	...
	for (int i = 0; i < num_components; i++)
	{
	    ...
	    {
	        Vector<Component> components = (Vector<Component>) updateHash.get(currUpdateId);
	        if (components == null)
	        {
	...
	if(deviceNode != null)
	{
	    //deviceNode.setAllOnUnchecked(); TODO: what does it do?
	...
	{
	    oldNid = subtree.getDefault();
	    subtree.setDefault(currBaseNid);
	}
	catch (Exception exc)
	{}
	...
	{}
	//if(deviceNode != null) deviceNode.setAllOnUnchecked(); TODO: what does it do
	justApplied = true;
    }
    public void reset()
    {
    ...
    **/}
	...
	for (int idx = 0; idx < device_components.size(); idx++)
	{
	    ...
	    if (currId != null)
	    {
	    	Descriptor<?> currData = ( (DeviceComponent) (device_components.
	...
	if (device_components.size() > 0)
		try {
			subtree.api.tdiExecute(subtree, varExpr.toString());
	...
	for (idx = 0; idx < num_expr; idx++)
	{
	    ...
	    try
	    {
	    	subtree.api.tdiExecute(subtree, varExpr + expressions[idx]);
	    }
	    catch (MdsException e)
	    {
    ...
    public void fireUpdate(String id, Descriptor<?> val)
    {
    ...
    public static void showDialog(Nid nid, String model) throws ClassNotFoundException
    {
	...
	try
	{
	    ...
	    ds.configure(nid.getTree(), nid.getNidNumber(), null);
