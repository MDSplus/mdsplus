...
class Action
{
	...
	public Action(final MDSplus.Action action, final int nid, final String name, final boolean on, final boolean essential, final String server_address)
	{
		this.action = action;
		this.nid = nid;
		this.name = name;
	...
	public synchronized void setManual(final boolean manual) {this.manual = manual; }

	public synchronized int getStatus() {return status; }
	synchronized void setStatus(final int status) {this.status = status; }

	public synchronized String getServerAddress(){return server_address;}
	public synchronized void setServerAddress(final String server_address){this.server_address = server_address;}
