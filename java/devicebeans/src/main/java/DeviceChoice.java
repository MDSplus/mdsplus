...
public class DeviceChoice extends DeviceComponent
{
	...
	{
	    if (subtree.isString(data))
	    {
                try {
                    data_string = subtree.getString(data);
                }catch(Exception exc){data_string = "";}
	    ...
	    {
	        try {
	            data_float = subtree.getFloat(data);
	    ...
	    {
	        try {
	            data_double = subtree.getDouble(data);
