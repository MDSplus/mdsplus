...
public class EnumEditor extends Editor{
	...
	public EnumEditor(final Descriptor<?> data, final boolean editable, final CTX ctx, final String tooltip, final MODE mode, final String... items){
	...
	public final Descriptor<?> getData() throws MdsException {
		if(this.mode == MODE.INDEX){
			...
			if(index >= 0) return new Int32(index);
		}
		this.expr = ((String)this.combo.getSelectedItem()).trim();
		if(this.expr.isEmpty()) return null;
		if(this.quotes_added) return new StringDsc(this.expr);
	...
	public final void reset(final boolean hard) {
		if(this.mode == MODE.INDEX && this.data instanceof Int32){
			...
			}else this.combo.setSelectedIndex(index);
			...
			if(!this.quotes_added) this.expr = this.data.decompile();
			this.updateItems();
