...
public class SegmentEditor extends Editor{
	...
	public static final boolean checkData(final Node node) {
		if(node != null) if(node.getNumSegments() > 0) return true;
		return false;
	}
	...
	public SegmentEditor(final NODE<?> node, final boolean editable, final Window window){
		...
		final JLabel label = new JLabel(new StringBuilder(21).append("Segments: ").append(num_segments & 0xFFFFFFFFl).toString(), SwingConstants.CENTER);
