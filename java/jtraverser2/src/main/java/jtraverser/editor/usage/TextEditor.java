...
public class TextEditor extends Editor implements ActionListener{
	...
	public final void setData(final Descriptor<?> data) {
		...
		this.checkData();
		if(this.curr_mode_idx != this.mode_idx) this.setMode(this.mode_idx);
		else if(this.curr_mode_idx > 0) if(this.curr_mode_idx < this.mode_idx_usr) this.data_edit.setData(data);
