...
public class GraphPanel extends JPanel{
	...
	public static final JDialog newPlot(final Descriptor<?> sig, final Frame frame, final String tree, final long shot, final String expr) {
	...
	public static final void plotNode(final NODE<?> node, final Frame frame, final String title) {
		try{
			...
			final DataStruct ds;
			if(use_new_misc) ds = misc.miscGetXYSignal(node.getTree(), expr, null, Float.NEGATIVE_INFINITY, Float.POSITIVE_INFINITY, 3200);
			else ds = misc.miscGetXYSignalLongTimes(node.getTree(), "__s=" + expr, "DIM_OF(__s)", Long.MIN_VALUE, Long.MAX_VALUE, 3200);
	...
	protected final void paintComponent(final Graphics g) {
		...
		if(this.scores == null || this.scores.size() == 0) g.drawString("no data", this.getWidth() / 2, this.getHeight() / 2);
		else if(this.scores.size() == 1) g.drawString(this.scores.get(0).toString(), this.getWidth() / 2, this.getHeight() / 2);
		else this.drawAxes((Graphics2D)g);
	}

	private final void drawAxes(final Graphics2D g2) {
		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		g2.setColor(Color.WHITE);
		g2.fillRect(this.getOrigin(GraphPanel.xaxis), GraphPanel.padding, this.getSize(GraphPanel.xaxis), -this.getSize(GraphPanel.yaxis));
	...
	private final void drawAxis(final Graphics2D g2, final Params params) {
		...
		for(int i = 0; i <= GraphPanel.numberYDivisions; i++){
			...
			if(params.isX) g2.drawLine(a, bt - 1, a, b1);
			else g2.drawLine(bt + 1, a, b1, a);
	...
	private final int getOrigin(final boolean isX) {
		if(isX) return GraphPanel.padding + GraphPanel.labelPadding;
		return GraphPanel.padding - this.getSize(isX);
	}

	private final Params getParams(final boolean isX) {
		if(isX) return new Params(this.scores.get(0).X, this.scores.get(this.scores.size() - 1).X, isX);
		float min = Float.POSITIVE_INFINITY, max = Float.NEGATIVE_INFINITY;
		for(final XY pair : this.scores){
			if(min > pair.Y) min = pair.Y;
			if(max < pair.Y) max = pair.Y;
		}
		if(min < max) return new Params(min, max, isX);
