...
public class Node{
	public static class TreeNodeLabel extends JLabel{
		...
		public TreeNodeLabel(final Node node, final String name, final Icon icon, final boolean isSelected){
			...
			else if(flags.isNoWriteModel() & flags.isNoWriteModel()) this.setForeground(this.CNoWrite);
	...
	public final void paste() {
		if(!this.treeview.equals(Node.copied.treeview)) MdsException.stderr("Copying between different trees is not yet supported.", null);
			...
			if(Node.copied != null && Node.copied != this) if(Node.cut){
