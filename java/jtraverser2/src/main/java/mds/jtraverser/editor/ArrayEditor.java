...
public class ArrayEditor extends Editor implements ChangeListener
{
	...
	public class RowNumberTable extends JTable implements ChangeListener, PropertyChangeListener
	{
		...
		private class RowNumberRenderer extends DefaultTableCellRenderer
		{
			...
			@Override
			public Component getTableCellRendererComponent(final JTable table_in, final Object value,
					final boolean isSelected, final boolean hasFocus, final int row, final int column)
			{
				...
				// if(isSelected)
				// RowNumberTable.this.setFont(RowNumberTable.this.getFont().deriveFont(Font.BOLD));
		...
		(this.updater = new Thread("ArrayEditor.updater")
		{
			...
			public final void run()
			{
				...
				for (int i = 0; i < n; i++)
				{
					...
					model.addRow(new Number[]
					{ narray.get(i * inc + offset) });
				}
				column.setHeaderValue("");
				header.repaint();
			}
