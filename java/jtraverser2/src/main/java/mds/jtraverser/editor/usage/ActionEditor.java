...
public final class ActionEditor extends TextEditor{
	...
	protected final boolean addExtraEditor() {
		...
		this.editor.add(this.data_edit = new mds.jtraverser.editor.ActionEditor(this.editable, this.ctx, this.window), BorderLayout.CENTER);
		return true;
	}

	@Override
	protected final boolean checkUsrData() {
		if(!mds.jtraverser.editor.ActionEditor.checkData(this.data)) return false;
