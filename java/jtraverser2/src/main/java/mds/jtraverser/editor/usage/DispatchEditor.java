...
public class DispatchEditor extends TextEditor
{
	...
	public final class DispatchEdt extends Editor
	{
		...
		public DispatchEdt(final Descriptor<?> data)
		{
			...
			this.jp.add(this.edit[0] = Editor.addLabel("Server",
					new ExprEditor(this.editable, this.ctx, DispatchEditor.server_tip, true, false)));
			this.jp.add(this.edit[1] = Editor.addLabel("Phase",
					new ExprEditor(this.editable, this.ctx, DispatchEditor.phase_tip, true, false)));
			this.jp.add(this.edit[2] = Editor.addLabel("When",
					new ExprEditor(this.editable, this.ctx, DispatchEditor.when_tip, false, false)));
			this.jp.add(this.edit[3] = Editor.addLabel("Completion",
					new ExprEditor(this.editable, this.ctx, DispatchEditor.completion_tip, true, false)));
		...
		public final Dispatch getData() throws MdsException
		{
			...
			return new Dispatch(type, this.edit[0].getData(), this.edit[1].getData(), this.edit[2].getData(),
					this.edit[3].getData());
		}
		...
		{
			this.data = data;
			if (data instanceof Dispatch && ((Dispatch) data).getType() != Dispatch.SCHED_SEQ)
				JOptionPane.showMessageDialog(JOptionPane.getRootFrame(),
						"The current dispatch type is currently not implemented. Use Sequential instead.",
						"Dispatch Type", JOptionPane.PLAIN_MESSAGE);
			this.setDescR();
		}
	}
	...
	{
		super(data, editable, ctx, window, "Dispatch", false, "Asynchron (not implemented)", "Sequential",
