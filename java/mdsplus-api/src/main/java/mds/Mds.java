...
public abstract class Mds{
	...
	private final Descriptor<?> getDataArray(final CTX ctx, final String expr, final Descriptor<?>... args) throws MdsException {
		...
		if(desc instanceof StringDsc){
			if(desc.length() < 8) return Missing.NEW;
			ByteBuffer bb = desc.getBuffer();
			int line = bb.getInt();
			int len = bb.getInt();
			if(bb.remaining() < len) return Missing.NEW;
			throw new MdsException(new StringBuilder(desc.length()).append(line).append(':').append(desc.toString().substring(8, 8 + len)).toString(), 0);
		}
		return desc;
	}
