...
public abstract class Descriptor<T>{
	...
	public static final NUMBER<? extends Number> valueOf(final Number value, final boolean signed) {
		if(value instanceof Integer) return signed ? new Int32(value.intValue()) : new Uint32(value.intValue());
		if(value instanceof Long) return signed ? new Int64(value.longValue()) : new Uint64(value.longValue());
		...
		if(value instanceof Short) return signed ? new Int16(value.shortValue()) : new Uint16(value.shortValue());
		if(value instanceof Byte) return signed ? new Int8(value.byteValue()) : new Uint8(value.byteValue());
		if(value instanceof BigInteger) return signed ? new Int128((BigInteger)value) : new Uint128((BigInteger)value);
	...
	public Descriptor<?> getDimension(final int idx) {
		...
		if(dsc instanceof Signal) return dsc.getDimension(idx);
		return null;
	}
	...
	public Descriptor<?> getHelp() {
		...
		if(dsc instanceof Param) return dsc.getHelp();
		return null;
	}
	...
	public Descriptor<?> getRaw() {
		final Descriptor<?> dsc = this.getData(DTYPE.SIGNAL);
		if(dsc instanceof Signal) return dsc.getRaw();
		return null;
	}
	...
	public Descriptor<?> getUnits() {
		...
		if(dsc instanceof With_Units) return dsc.getUnits();
		return null;
	}
