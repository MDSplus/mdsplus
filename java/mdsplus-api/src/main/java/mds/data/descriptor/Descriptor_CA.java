...
public class Descriptor_CA extends ARRAY<ByteBuffer>
{
	...
	{
		return this.pointer() == 0 ? null : this.unpack().getData(omits);
	}
	...
	public final Descriptor_A<?> unpack()
	{
		...
		try
		{
			if (this.isLocal() && this.use_mds_local())
				return (Descriptor_A<?>) Descriptor.mds_local.getDescriptor("DATA($)", this);
