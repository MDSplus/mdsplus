...
public final class Dim extends Descriptor_R<Number>
{
	...
	public final Descriptor<?> getData_(final DTYPE... omits)
	{
		...
		{
			if (!Descriptor.isMissing(((Range) (this.getAxis())).getDelta()))
				delta = ((Range) (this.getAxis())).getDelta().toFloat();
			if (!Descriptor.isMissing(((Range) (this.getAxis())).getBegin()))
				begin = ((Range) (this.getAxis())).getBegin().toFloat();
			if (!Descriptor.isMissing(((Range) (this.getAxis())).getEnding()))
				ending = ((Range) (this.getAxis())).getEnding().toFloat();
		}
		if (this.getWindow() instanceof Window)
		{
			if (!Descriptor.isMissing(((Window) (this.getWindow())).getValueAtIdx0()))
				origin = ((Window) (this.getWindow())).getValueAtIdx0().toFloat();
			if (!Descriptor.isMissing(((Window) (this.getWindow())).getStartingIdx()))
				begin = Math.max(begin, origin + delta * ((Window) (this.getWindow())).getStartingIdx().toFloat());
			if (!Descriptor.isMissing(((Window) (this.getWindow())).getEndingIdx()))
				ending = Math.min(ending, origin + delta * ((Window) (this.getWindow())).getEndingIdx().toFloat());
		}
		if (begin == Float.NEGATIVE_INFINITY || ending == Float.POSITIVE_INFINITY)
			return new Uint8(1);
		...
		return new Float32Array(DTYPE.FLOAT, buf, new int[]
		{ elements });
	}
