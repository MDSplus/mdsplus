...
public final class Range extends Descriptor_R<Number>
{
	...
	public final StringBuilder decompile(final int prec, final StringBuilder pout, final int mode)
	{
		...
		if (!Descriptor.isMissing(this.getDelta()))
			this.getDelta().decompile(prec, pout.append(" : "), mode);
		return pout;
	}
	...
	public final Descriptor<?> getData_(final DTYPE... omits)
	{
		...
		if (Descriptor.isMissing(begin) || Descriptor.isMissing(end))
			return null;
		if (begin instanceof FLOAT || end instanceof FLOAT || delta instanceof FLOAT)
			return new Float64Array(
					Range.range(begin.toDouble(), end.toDouble(), Descriptor.isMissing(delta) ? 1d : delta.toDouble()));
		if (begin instanceof Int64 || end instanceof Int64 || delta instanceof Int64)
			return new Int64Array(
					Range.range(begin.toLong(), end.toLong(), Descriptor.isMissing(delta) ? 1l : delta.toLong()));
	...
	{
		return new int[]
		{ (int) ((this.getEnding().toFloat() - this.getBegin().toFloat()) / this.getDelta().toFloat()) };
	}
}
