...
public final class Descriptor_CA_Test{
	...
	public final void testARRAY() throws MdsException {
		...
		Assert.assertEquals("Set_Range(1000,0 /*** etc. ***/)", Descriptor.deserialize(ByteBuffer.allocate(4024).put(new byte[]{4, 0, 8, 4, 24, 0, 0, 0, 0, 0, 112, 1, -96, 15, 0, 0, 24, 0, 0, 0, -24, 3}).array()).toString());
		Assert.assertArrayEquals(new byte[]{8, 0, 5, 4, 16, 0, 0, 0, 0, 0, 48, 1, 8, 0, 0, 0, 0, 16, -91, -44, -24, 0, 0, 0}, test.serializeArray());
		Assert.assertEquals("ABS([1000000000000QU])", Descriptor_CA_Test.mds.getString("_a=*;_s=MdsShr->MdsSerializeDscIn(ref($),xd(_a));_s=TdiShr->TdiDecompile(xd(_a),xd(_a),val(1));_a", Function.ABS(new Uint64Array(1000000000000l)).serializeDsc()));
		...
		Assert.assertArrayEquals(new int[]{2, 2, 2}, Descriptor_CA_Test.mds.getDescriptor(null, "$", signal).getShape());
		Assert.assertArrayEquals(new int[]{2, 2}, Descriptor_CA_Test.mds.getDescriptor("Build_Signal([[1,2],[3,4]],*,[1,2])").getShape());
	}
}
