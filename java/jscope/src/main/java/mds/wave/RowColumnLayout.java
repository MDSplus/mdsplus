 ...
 * defined by number of column and number of components in column. Column width
 * and component height can be modified by pointer interaction. An Example
 * explaine how to use it:
 *
 * <pre>
 * <code>
...
public class RowColumnLayout implements LayoutManager
{
	...
	/**
	 * Costruct a RowColumnLayout with a column number defined by _column argument
	 * and number of object per column defined by row[] vector. The width size of
	 * ith column is defined by pw[i] value and height size of jth component is
	 * defuned by pw[j]. The pw[i] and ph[j] value are normalize value in rangeform
	 * 0 to 1.
	 *
	 * @param row row[i] define number of component in column i
	 * @param ph  Vector of normalize height of component. The sum of ph[x] of the
	 *            objects in a column must be 1.
	 * @param pw  Vector of normalize width of the culomn. The sum of pw[x] must be
	 *            1
	 */
...
//	    resetPercentHW(parent);
	 ...
	 * Line/s are drow on the component only if double buffering is supported on
	 * component. RowColumnLayout manager check double buffering component
	 * capability by isDoubleBuffered() method of component.
	 *
...
//		            ((RowColumnComponent)c).DrawYResize(0, 0, 0);
	 ...
	 *           resize all component in column are resized to equal height.
	 */
	 ...
	 *           resize component height are resize on y value.
	 * @param x  x position
	 * @param y  y position
	 */
	public void ResizeRowColumn(Component _b, int x, int y)
	{
		...
		if (resize_mode == VERTICAL)
		{
			...
			for (k = start_comp, new_y = 0; k != end_comp; k += inc_comp)
			{
				...
				else if (curr_y + currHeight - new_y < MIN_SIZE_H)
		...
		if (resize_mode == HORIZONTAL)
		{
			...
			max_x = (int) (curr_x + currMaxWidth * (percent_width[col_idx] + percent_width[nextColumn(col_idx)]))
					+ hgap;
			// percent_width[col_idx + 1])) + hgap;
			if (x < min_x)
				x = min_x;
			if (x > max_x - MIN_SIZE_W)
				x = max_x - MIN_SIZE_W;
