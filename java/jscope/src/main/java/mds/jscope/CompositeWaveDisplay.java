	...
	public class AppendThread extends Thread
	{
		...
		public void run()
		{
			...
			while (true)
			{
				try
				{
					final Object obj[] = updSignalDataQeue.dequeue();
					...
					for (final Signal s : signals2DVector)
		...
		/*
		 * private UpdSignalData mergeMessages(UpdSignalData[] usds) { UpdSignalData
		 * out[] = new UpdSignalData[2 + signals1DVector.size() +
		 * signals2DVector.size()]; for(int i = 0; i < usds.length; i++) {
		 *
		 * } return null; }
		 *
		 *
		 ...
		 * if(upd1.type == Signal.TYPE_1D) { if(upd1.operation ==
		 * CompositeWaveDisplay.CMND_CLEAR) return upd1 ; if(upd2.operation ==
		 * CompositeWaveDisplay.CMND_CLEAR) return upd2 ;
		 *
		 * int numSignal1 = upd1.x.length / upd1.numPointsPerSignal; int numSignal2 =
		 * upd2.x.length / upd2.numPointsPerSignal;
		 *
		...
		{
			final Signal s = signals1DHash.get(usd.name);
			if (s != null)
		...
		private void processsSignals1DPacket(UpdSignalData usd)
		{
			...
			{
				final Signal s = signals1DVector.elementAt(i);
...
//          System.out.println("N point per sig "+ usd.numPointsPerSignal +" tot point "+ totPoints +" time len "+ usd.times.length);
			...
			{
				final Signal s = signals2DVector.elementAt(i);
		...
		{
			final Signal s = signals2DHash.get(usd.name);
			if (s != null)
			{
	...
	{
		final Enumeration<AbstractButton> e = pointer_mode.getElements();
	...
	public void init()
	{
		...
		final WavePopup wave_popup = new MultiWavePopup(new SetupWaveformParams(f, "Waveform Params"),
	...
	/**
	 * Add new signal, defined by x and y float vectors, to the panel in (row,
	 * column) position. The panel is created if not already present.
	 *
	 ...
	 * @param inter  Interpolation flag, if true a line is draw between adiacent
	 *               point
	 * @param marker Marker point
	 */
	public void addSignal(float[] x, float[] y, int row, int column, String color, String label, boolean inter,
			int marker)
	{
		addSignal(new Signal(x, y), row, column, color, label, inter, marker);
	}

	public void addSignal(double[] x, float[] y, int row, int column, String color, String label, boolean inter,
			int marker)
	{
		addSignal(new Signal(x, y), row, column, color, label, inter, marker);
	}
	...
	private String getExpression(String paramString, boolean infoFlag) throws IOException
	{
		...
		{
			final Object localObject = paramString.substring(paramString.lastIndexOf(',') + 1,
					paramString.lastIndexOf(')'));
			final String str10 = paramString.substring(paramString.indexOf('(') + 1, paramString.lastIndexOf(','));
	...
	public String addSignal(String paramString1, int paramInt1, int paramInt2, String paramString2, String paramString3,
			boolean paramBoolean, int paramInt3, String paramString4)
	{
		String str1 = null;
					...
					if (w.IsImage() || (w.GetSignals() != null && w.GetSignals().size() != 0))
					{
						JOptionPane.showMessageDialog(this,
								"The selected waveform panel contains signals or frame.\n"
										...
										+ da.getDataProvider().getClass().getName() + " data provider.",
								"alert", JOptionPane.ERROR_MESSAGE);
						return;
					}
	...
	public void removeAllSignals()
	{
		...
		if (signals1DHash.size() > 0)
			signals1DHash.clear();
		if (signals2DHash.size() > 0)
			signals2DHash.clear();
		if (signals1DVector.size() > 0)
			signals1DVector.clear();
		if (signals2DVector.size() > 0)
			signals2DVector.clear();
	}
	...
	public void removeAllSignals(int row, int col)
	{
		if (wave_container != null)
		{
			final MultiWaveform wave = (MultiWaveform) wave_container.getGridComponent(row, col);
			wave.Erase();
		}
	}

	/**
	 * Add new signal, defined as x and y float vectors, to panel in (row, column)
	 * position. The panel is created if not already present.
	 *
	 * @param x      x array values
	 * @param y      y array values
	...
	/**
	 * Add new signal, defined as Signal class, to panel in (row, column) position.
	 * The panel is created if not already present.
	 *
	...
	public void addSignal(Signal s, int row, int col)
	{
		...
		if (isApplet)
			showStatus("Add " + s.getName() + " col " + col + " row " + row);
		switch (s.getType())
		{
		case Signal.TYPE_1D:
