...
// various status conditions
	switch( statcode ) {
		...
		// bad things  :(
		case	FAILURE:		printf( "failure--failure--failure\n"		);	break;	//	 0 an even number
		case	ERROR:			printf( "general status error\n"            );	break;	//  -1 * 2
		case	FILE_ERROR:		printf( "file doesn't exist\n"              );	break;	//  -2 * 2
		case	DUPLICATE:		printf( "duplicate module name\n"           );	break;	//  -3 * 2
		case	LOCK_ERROR:		printf( "unable to lock file\n"             );	break;	//  -4 * 2
		case	MAP_ERROR:		printf( "db file failed to map to memory\n" );	break;	//  -5 * 2
		case	COMMIT_ERROR:	printf( "data did not get added to db\n"    );	break;	//  -6 * 2
		case	UNLOCK_ERROR:	printf( "unable to unlock file\n"           );	break;	//  -7 * 2
		case	EXPAND_ERROR:	printf( "failure to expand db file\n"       );	break;	//  -8 * 2
		case	COPY_ERROR:		printf( "file copy error\n"                 );	break;	//  -9 * 2
		case	ASSIGN_ERROR:	printf( "assign failure\n"                  );	break;	// -10 * 2
		case	DEASSIGN_ERROR:	printf( "deassign failure\n"                );	break;	// -11 * 2
		case	DELCRATE_ERROR:	printf( "delete crate failure\n"            );	break;	// -12 * 2
		case	NO_MEMORY:		printf( "malloc() failure\n"                );	break;	// -13 * 2
		case	NO_DEVICE:		printf( "no scsi device found\n"            );	break;	// -14 * 2
		case	QUERY_ERROR:	printf( "error determining highway type\n"  );	break;	// -15 * 2
		case	NO_PERMISSION:	printf( "need read/write permission\n"      );	break;	// -16 * 2
