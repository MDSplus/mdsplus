...
static int closeIndirectLevel()
{
  ...
  if (io->fp) {			/* It's a file:  close it ...                   */
    fclose(io->fp);
    io->fp = 0;
  } else {			/*..else, it's a macro ...                      */
    macro = io->ioMacro;
    macro->isOpen = 0;
    io->ioMacro = 0;
  }
    ...
    )
{
  ...
  if (ctrl->depth >= MAX_DEPTH) {
    return (MdsMsg(MDSDCL_STS_INDIRECT_ERROR, "Exceeded MAX_DEPTH for indirect files"));
  }

  if (!ascFilename(&p, &dsc_filename, 0))
    return (MdsMsg(MDSDCL_STS_INDIRECT_ERROR, "Illegal filename: %s", p ? p : ""));
  ...
  if (!fp) {
	...
	str_replace(&dsc_filename, &ctrl->def_file, k, k + 1, &dsc_filename);
	fp = fopen(dsc_filename.dscA_pointer, "r");
      }
    }
    if (!fp) {
      p2 = dsc_filename.dscA_pointer;
      return (MdsMsg(MDSDCL_STS_INDIRECT_ERROR, "No such filename: %s", p2 ? p2 : ""));
    }
  }
		/*=====================================================
    ...
    )
{
  ...
  if (fp) {			/* If it's not a macro                  */
    p = fgets_with_edit(line, sizeof(line), fp, prompt);
    if (!p) {
      sts = closeIndirectLevel();
      return (sts);			/*--------------------> return	*/
    }
  } else {			/*..else, get next line from macro ...  */
    macro = io->ioMacro;
    if (!macro) {
#ifdef vms
      lib$signal(SS$_DEBUG);
#endif
      return (MdsMsg(MDSDCL_STS_INDIRECT_ERROR, "Error getting next macro line", 0));
    }
    if (io->lineno == macro->numLines) {	/* We're at end of macro        */
      sts = closeIndirectLevel();
      return (sts);			/*--------------------> return	*/
    }
    ...
    )
{
  ...
  for (; *p; p++) {
    if (IS_COMMENT_CHARACTER(*p)) {
      for (; isspace(*(p - 1)); p--) ;	/* remove white space ...       */
      *p = '\0';
      break;			/* out of loop                  */
    }
	...
	if (*p != quote_char)
	  continue;
	/* found a quote_char ...               */
	if (*(p + 1) == quote_char) {	/*..2 successive quote_chars ...        */
	  p++;
	  continue;
	}
