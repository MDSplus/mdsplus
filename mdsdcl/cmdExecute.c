...
static void freeParameter(dclParameterPtr * p_in)
{
...
static void freeCommandParamsAndQuals(dclCommandPtr cmdDef)
{
  if (cmdDef) {
    int i;
	  ...
	  if (list.count == 1) {
		...
		if (nameNode && nameNode->name && (strcasecmp(nameNode->name, "name") == 0)) {
		  if (nameNode->children &&
			...
			findVerbInfo(list.nodes[0]->children, cmdDef);
		      }
		      if (list.nodes)
			free(list.nodes);
		      goto REDO;
...
int cmdExecute(dclCommandPtr cmd)
{
  ...
  for (doc_l = dclDocs; (status == CLI_STS_IVVERB) && (doc_l != NULL); doc_l = doc_l->next) {
    ...
    findEntity(doc_l->doc->children, "verb", cmdDef->verb, &matchingVerbs, &exactFound);
