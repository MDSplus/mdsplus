...
extern smg$set_out_of_band_asts();
	 ...
	 * displayCmdline:
	 ****************************************************************/
static void displayCmdline(char *cmdline	/* <r> the command line                 */
    )
{
  int i;
...
STATIC_THREADSAFE int mdsdclMutex_initialized = 0;
	 ...
	 ****************************************************************/
int mdsdcl_do_command(void *command	/* <r:opt> command -- cstring or dsc    */
    )
{
  int tblidx;
  int sts, stsParse;
  ...
  if (!ctrl->tbladr[0])
    mdsdcl_initialize(ctrl);
  tblidx = ctrl->tables;

  if (!doMacro[0]) {		/* first time ...                       */
  ...
  for (; tblidx > 0; tblidx--) {
    ...
    if (stsParse & 1) {		/* Try to dispatch the routine ...      */
	...
	if (~sts & 1) {
	  if (dsc_cmd.dscA_pointer)
	    MdsMsg(0, "--> failed on line '%s'", dsc_cmd.dscA_pointer);
  ...
  if (sts & 1) {		/* Try to dispatch the macro ...        */
    sts = cli_dispatch(ctrl);
