    ...
    )
{
  ...
  if (*p2 == '\'')
    p2++;
		 ...
		 *======================================================*/
  if (k == 2 && toupper(symbol[0]) == 'P' && symbol[1] >= '1' && symbol[1] <= '8') {
    sscanf(symbol + 1, "%d", &i);
    ...
    )
{
  ...
  for (; *p; p++) {
	...
	doubleQuotes = 0;
    } else if (*p == '\'') {	/*========= Single-quote char =========*/
      if (!doubleQuotes) {	/* normal unquoted string ...           */
	if (*(p + 1) == '\'')
	  strcpy(p, p + 1);	/* replace by single quote      */
	else {			/*...else, replace symbol       */
	  offsetStart = p - dsc_cmdline->dscA_pointer;
	  p = insertSymbol(dsc_cmdline, offsetStart, offsetStart + 1, dsc_prmList);
	}
      } else {			/* inside quoted string ...             */
	if (*(p + 1) == '\'') {	/* 2 single quotes denote symbol */
	  offsetStart = p - dsc_cmdline->dscA_pointer;
	  p = insertSymbol(dsc_cmdline, offsetStart, offsetStart + 2, dsc_prmList);
	}
      }
    }
  }
