...
public class DeviceFieldCustomizer extends DeviceCustomizer implements Customizer
{
    ...
    public void setObject(Object o)
    {
        ...
        public void actionPerformed(ActionEvent e)
        {
            ...
            listeners.firePropertyChange("labelString", oldLabel, labelString.getText());
            String colStr = numCols.getText();
            int oldCols = bean.getNumCols();
            //bean.setNumCols(Integer.parseInt(colStr));
            listeners.firePropertyChange("numCols", oldCols, Integer.parseInt(colStr));
            boolean oldTextOnly = bean.getTextOnly();
            //bean.setTextOnly(textOnly.getState());
            listeners.firePropertyChange("textOnly", oldTextOnly, textOnly.getState());
            boolean oldEditable = bean.getEditable();
            //bean.setEditable(editable.getState());
            listeners.firePropertyChange("editable", oldEditable, editable.getState());
            boolean oldShowState = bean.getShowState();
            //bean.setShowState(showState.getState());
            listeners.firePropertyChange("showState", oldShowState, showState.getState());
            boolean oldDisplayEvaluated = bean.getDisplayEvaluated();
            //bean.setDisplayEvaluated(displayEvaluated.getState());
            listeners.firePropertyChange("displayEvaluated", oldDisplayEvaluated, displayEvaluated.getState());
            int oldOffsetNid = bean.getOffsetNid();
            //bean.setOffsetNid(nids.getSelectedIndex() + 1);
            listeners.firePropertyChange("offsetNid", oldOffsetNid, nids.getSelectedIndex() + 1);
            String oldIdentifier = bean.getIdentifier();
            //bean.setIdentifier(identifier.getText());
            listeners.firePropertyChange("identifier", oldIdentifier, identifier.getText());
