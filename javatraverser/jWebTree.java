...
//        "<PARAM NAME = \"CODEBASE\" VALUE = \"/wwwai/\""+"\n"+
...
//        "CODEBASE = \"/\""+"\n"+
    ...
    throws IOException, ServletException
    {
        ...
        if(experiment == null || tree == null || !experiment.equals(curr_experiment)||curr_shot != shot)
        {
            if(tree != null)
                try {
                    tree.close(0);
       ...
       else
       {
            ...
            if(default_nid == null)
            {
                out.println("Internal error: default_nid == null for default path= "+curr_path);
                return;
            }
            try{
                NodeInfo node_info = tree.getInfo(default_nid, 0);
       ...
       out.println("<ul>");
       try {
            NidData sons[] = tree.getSons(default_nid, 0);
            NidData [] members = tree.getMembers(default_nid, 0);
            if(sons != null)
            {
                for(i = 0; i < sons.length; i++)
                {
                    NodeInfo node_info = tree.getInfo(sons[i], 0);
            ...
            if(members != null)
            {
                ...
                {
                    NodeInfo node_info = tree.getInfo(members[i], 0);
                    out.println("<li><a href="+BASEREF+"?path="+ default_path+":"+node_info.getName().trim()+
                        "&experiment="+curr_experiment+"&shot="+shot+"&display="+display_wave+"&jscope="+is_jscope+">"+
                        node_info.getName().trim()+"</a>");
                }
    ...
    {
        try {
            return tree.resolve(new PathData(path), 0);
    ...
    protected void displayData(NidData nid, PrintWriter out)
    {
        try 
        {
            ...
            if(data != null)
            {
                ...
                {
                    try {
                        System.out.println("Parte Evaluated");
                        Data evaluated_data = tree.evaluateData(nid, 0);
                        ...
                        System.out.println("Finisce ToString");
                        }
    ...
    protected Data convertPaths(Data data)
    {
        ...
        {
            try {
                NodeInfo info = tree.getInfo((NidData)data, 0);
