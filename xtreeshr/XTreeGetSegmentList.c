...
static void freeResources(struct descriptor_xd *startTimeXds, struct descriptor_xd *endTimeXds, int numSegments)
{
	int i;
...
EXPORT int _XTreeGetSegmentList(void *dbid, int nid, struct descriptor *startDsc, struct descriptor *endDsc, struct descriptor_xd *outSignalXd)
{
	int status;
	char isGreater;
	int numSegments;
	...
	{
		status = (dbid)?_TreeGetSegmentLimits(dbid, nid, currIdx, &startTimeXds[currIdx], &endTimeXds[currIdx]):TreeGetSegmentLimits(nid, currIdx, &startTimeXds[currIdx], &endTimeXds[currIdx]);
		if(!(status & 1)) 
		{
...
EXPORT int XTreeGetSegmentList(int nid, struct descriptor *startD, struct descriptor *endD, struct descriptor_xd *outSignal)
{
