...
public abstract class Editor extends JPanel{
	...
	protected class EvalActionListener implements ActionListener{
	    ...
	    public void actionPerformed(final ActionEvent e) {
	        String aeeval;
	        if(this.exec == null) aeeval = MdsException.getMdsMessage(MdsException.TdiINVCLADSC);
	        else try{
	            final Descriptor<?> aedata = Editor.this.getMds().getDescriptor(Editor.this.ctx, this.exec, Editor.this.getData());
	            if(Descriptor.isMissing(aedata)) aeeval = "*";
	        ...
	        JOptionPane.showMessageDialog(Editor.this, "<html><body><p style='width: 360px;'>" + EvalPopupMenu.this.escapeHTML(aeeval) + "</p></body></html>", "Evaluated Data (also copied to Clipboard)", JOptionPane.PLAIN_MESSAGE);
	    }
	}
	...
	public void mouseClicked(final MouseEvent e) {
	    if(POPUP_MODE.CLICK == this.mode) EvalPopupMenu.this.show(e.getComponent(), e.getX(), e.getY());
	}
	...
	public void mousePressed(final MouseEvent e) {
	    if(POPUP_MODE.CONTEXT == this.mode && e.isPopupTrigger()) EvalPopupMenu.this.show(e.getComponent(), e.getX(), e.getY());
	}
