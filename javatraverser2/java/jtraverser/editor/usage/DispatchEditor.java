...
public class DispatchEditor extends TextEditor{
    public final class DispatchEdt extends Editor{
        ...
        public DispatchEdt(final Descriptor<?> data){
            super(data, DispatchEditor.this.editable, 4);
            this.jp = new JPanel();
            this.jp.setLayout(new GridLayout(4, 1, 0, 0));
            this.jp.add(this.edit[0] = Editor.addLabel("Server", new ExprEditor(DispatchEditor.server_tip, true, this.editable, false)));
            this.jp.add(this.edit[1] = Editor.addLabel("Phase", new ExprEditor(DispatchEditor.phase_tip, true, this.editable, false)));
            this.jp.add(this.edit[2] = Editor.addLabel("When", new ExprEditor(DispatchEditor.when_tip, false, this.editable, false)));
            this.jp.add(this.edit[3] = Editor.addLabel("Completion", new ExprEditor(DispatchEditor.completion_tip, true, this.editable, false)));
    ...
    public DispatchEditor(final boolean editable, final Window window){
        this(null, editable, window);
    }

    public DispatchEditor(final Descriptor<?> data, final boolean editable, final Window window){
        super(data, editable, window, "Dispatch", false, "Asynchron (not implemented)", "Sequential", "Conditional (not implemented)");
    }
