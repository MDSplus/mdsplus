...
public abstract class FLOAT<T extends Number>extends NUMBER<T>{
    public static final String decompile(final Number value, final DTYPE dtype, final int mode) {
        ...
        if(value.doubleValue() == 0) return isF ? "0." : new StringBuilder(3).append('0').append(dtype.suffix).append('0').toString();
        ...
        return new StringBuilder(16).append(tmp.replaceAll("((?<=^-|^)0*|\\.?0*$)", "")).append(dtype.suffix).append(E).toString();
    }
