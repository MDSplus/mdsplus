...
SYNOPSIS
    ./bash.sh --os=name [--test[=skip]] [--test_format=list]
               [--testrelease]
...
if [ ! -r "${SRCDIR}/deploy/platform/${PLATFORM}/${PLATFORM}_bash.sh" ]
then
...
OS=${OS} \
  ...
  ${SRCDIR}/deploy/platform/${PLATFORM}/${PLATFORM}_bash.sh
if [ "$?" != "0" ]
then
    RED $COLOR
    cat <<EOF >&2
