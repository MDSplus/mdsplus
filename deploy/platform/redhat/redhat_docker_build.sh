...
config() {
    /source/configure \
        ...
        --host=$2-linux \
        --with-gsi=/usr:gcc$3 $4 $5 $6 $7 $8 $9;
}
makelist(){
    rpm2cpio $1 | \
        cpio --list --quiet | \
...
EOF
    ...
    then
        ...
        for test in $(spacedelim ${SANITIZE}); do
            ...
            config_test 64 x86_64 64 --enable-sanitize=${test}
            status=$?
            if [ "$status" = "111" ]; then
...
EOF
    ...
    then
        ...
        for test in $(spacedelim ${SANITIZE}); do
            ...
            config_test 32 i686 32 --enable-sanitize=${test}
            status=$?
            if [ "$status" == 111 ]; then
                echo "Sanitizer ${test} not supported. Skipping."
                break
