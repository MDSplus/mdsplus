...
function getScopeTitle(tree, shot, title, success_cb,failure_cb,svg, scopeIdx, numCols, numRows, 
  ...
  var getStr;
  if (tree)
    getStr = 'http://'+window.location.host+'/mdsplusWsgi/1darray/'+tree+'/'+shot+'?expr='+encodeUrl(title);
  else
    getStr = 'http://'+window.location.host+'/mdsplusWsgi/1darray?expr='+encodeUrl(title);
...
function getScopeLimits(tree, shot, limit, limitId, success_cb,failure_cb,svg, scopeIdx, numCols, numRows, 
  ...
  var getStr;
  if (tree)
    getStr = 'http://'+window.location.host+'/mdsplusWsgi/1darray/'+tree+'/'+shot+'?expr='+encodeUrl(limit);
  else
    getStr = 'http://'+window.location.host+'/mdsplusWsgi/1darray?expr='+encodeUrl(limit);
...
*/
 ...
 if (tree)
    req.open('POST','http://'+window.location.host+'/mdsplusWsgi/1dsignal/'+tree+'/'+shot+'?expr='+encodeUrl(expr),true);
  else
    req.open('POST','http://'+window.location.host+'/mdsplusWsgi/1dsignal?expr='+encodeUrl(expr),true);
  req.send();
}
...
{
    var req = new XMLHttpRequest();
    req.open('POST','http://'+window.location.host+'/mdsplusWsgi/scopeconfig?expr='+configName,true);
    req.onreadystatechange = function() {
        if (this.readyState == 4) {
            if (this.getResponseHeader('ERROR'))
