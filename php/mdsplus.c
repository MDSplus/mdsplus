...
function_entry mdsplus_functions[] = {
	...
	{NULL, NULL, NULL}	/* Must be the last line in mdsplus_functions[] */
};
/* }}} */
...
PHP_FUNCTION(mdsplus_connect)
{
	...
	{
        char *error = strcpy(malloc(arg_len+strlen("Error connecting to host: ")+1),"Error connecting to host: ");
        strcat(error,arg);
    ...
    if (num_args == 2)
	...
	else
		{
          ...
          if (zend_get_parameters_array_ex(num_args,args) != SUCCESS)
			  WRONG_PARAM_COUNT;
          status = SendArg(socket, 0, DTYPE_CSTRING, num_args - 1, strlen(expression), 0, dims, expression);
          for (i=2;(status & 1) && i<num_args;i++)
		  {
			  switch(Z_TYPE_P(*args[i]))
				  {
				  case IS_LONG: status = SendArg(socket,i-1,DTYPE_LONG,num_args - 1,sizeof(int),0,dims,(void *)&Z_LVAL_P(*args[i])); break;
				  case IS_DOUBLE:status = SendArg(socket,i-1,DTYPE_DOUBLE,num_args - 1,sizeof(double),0,dims,(void *)&Z_DVAL_P(*args[i])); break;
				  case IS_STRING:status = SendArg(socket,i-1,DTYPE_CSTRING,num_args - 1,Z_STRLEN_P(*args[i]),0,dims,Z_STRVAL_P(*args[i])); break;
				  default: 
		  ...
		  if (!(status & 1))
          ...
          else
			  {
				  ...
				  void *dptr;
				  void *mem = 0;
				  status = GetAnswerInfoTS(socket, &ans.dtype, &len, &ans.ndims, &ans.dims, &numbytes, &dptr, &mem);
				  ans.length = len;
				  if (numbytes)
					  {
	...
	if (status & 1)
		{
			...
			switch (ans.dtype)
				{
				case DTYPE_UCHAR: RETURN_LONG((int)*(unsigned char *)ans.ptr); break;
				case DTYPE_USHORT:RETURN_LONG((int)*(unsigned short *)ans.ptr); break;
				case DTYPE_ULONG: RETURN_LONG((int)*(int *)ans.ptr); break;
				case DTYPE_CHAR:  RETURN_LONG((int)*(char *)ans.ptr); break;
				case DTYPE_SHORT: RETURN_LONG((int)*(short *)ans.ptr); break;
				case DTYPE_LONG: RETURN_LONG(*(int *)ans.ptr); break;
				case DTYPE_FLOAT: RETURN_DOUBLE((double)*(float *)ans.ptr); break;
				case DTYPE_DOUBLE: RETURN_DOUBLE(*(double *)ans.ptr); break;
				case DTYPE_CSTRING: RETURN_STRING(ans.ptr ? ans.ptr : strcpy(malloc(1),""),1); break;
				default: 
					{ char error[128];
			...
			if (ans.ptr)
				{
					...
					sprintf(error,"expression '%s' evaluation failed with the following error: %s",expression,ans.ptr);
				}
			else
				{
    ...
    if (num_args == 3)
	...
	else
		{
          ...
          for (i=3;(status & 1) && i<num_args;i++)
		  {
			  switch(Z_TYPE_P(*args[i]))
				  {
				  case IS_LONG: status = SendArg(socket,i,DTYPE_LONG,num_args,sizeof(int),0,dims,(void *)&Z_LVAL_P(*args[i])); break;
				  case IS_DOUBLE:status = SendArg(socket,i,DTYPE_DOUBLE,num_args,sizeof(double),0,dims,(void *)&Z_DVAL_P(*args[i])); break;
				  case IS_STRING:status = SendArg(socket,i,DTYPE_CSTRING,num_args,Z_STRLEN_P(*args[i]),0,dims,Z_STRVAL_P(*args[i])); break;
				  default: 
					  mdsplus_replace_error("invalid argument type, must be long, double or string only",0);
                      status = 0;
