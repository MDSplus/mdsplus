...
def msiUpdateSetup(FLAVOR,WORKSPACE,VERSION,release,bits,outfile,msiflavor):
    ...
    while len(line) > 0:
        ...
        if '"ProductName"' in line:
            ...
            line='        "Url" = "8:http://www.mdsplus.org/windows-%s/%s/%s.%d"' % (FLAVOR,setupdir,VERSION,release)
        print >>f_out,line
        line=f_in.readline()
    f_in.close()
    f_out.close()
...
def makeMsiCommand(args):
    ...
    if need_to_build:
        if rebuild:
            ...
            p=subprocess.Popen('devenv /build "Release|Java" mdsplus.sln',shell=True,cwd=WORKSPACE+"\\mdsplus")
            stat=p.wait()
            ...
            p=subprocess.Popen('devenv /build "Release|Win32" mdsplus.sln',shell=True,cwd=WORKSPACE+"\\mdsplus")
            stat=p.wait()
            ...
            p=subprocess.Popen('devenv /build "Release|x64" mdsplus.sln',shell=True,cwd=WORKSPACE+"\\mdsplus")
            stat=p.wait()
