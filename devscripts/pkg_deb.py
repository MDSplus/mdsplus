...
def makeDebsCommand(args):
    ...
    if need_to_build:
        cmd='rm -Rf DEBS/* SOURCES;' +\
             ...
             './configure --enable-mdsip_connections --enable-nodebug --prefix=%s/BUILDROOT/usr/local/mdsplus --exec_prefix=%s/BUILDROOT/usr/local/mdsplus --with-gsi=/usr:gcc%d --with-labview=$LABVIEW_DIR --with-jdk=$JDK_DIR --with-idl=$IDL_DIR;' % (WORKSPACE,WORKSPACE,BITS) +\
