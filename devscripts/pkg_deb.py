...
def makeDebsCommand(args):
    ...
    status="ok"
    if need_to_build:
        p=subprocess.Popen('ln -sf ../mdsplus%s-%s $(pwd);tar zcfh ../SOURCES/mdsplus%s-%s.tar.gz --exclude CVS ../mdsplus%s-%s;rm -f ../mdsplus%s-%s;' % (debflavor,VERSION,debflavor,VERSION,debflavor,VERSION,debflavor,VERSION) +\
