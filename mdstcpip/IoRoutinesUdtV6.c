...
static IoRoutines UDTV6_routines = {UDTV6_connect,UDTV6_send,UDTV6_recv,UDTV6_flush,UDTV6_listen,UDTV6_authorize,UDTV6_reuseCheck,UDTV6_disconnect};
...
static int UDTV6_listen(int argc, char **argv) {
  ...
  if (GetMulti()) {
    ...
    while (1) {
      int i;
      int readfds_num=1024;
      status=udt_epoll_wait2(server_epoll, readfds, &readfds_num, NULL ,NULL, 5000,NULL,NULL,NULL,NULL);
	...
	while(1) {
	  for (c=ClientList; c; c=c->next) {
	    ...
	    status=udt_epoll_wait2(c_epoll,readfds,&readnum,writefds,&writenum,0,NULL,NULL,NULL,NULL);
