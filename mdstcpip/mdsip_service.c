...
static int SpawnWorker(SOCKET sock) {
  ...
  sprintf(cmd,"%s\\%s\\mdsip.exe --port=%s --hostfile=\"%s\" --compression=%d --sockethandle=%d:%d",getenv("MDSPLUSDIR"),dirname,
...
static void InstallService() {
  ...
  if (hSCManager) {
    ...
    sprintf(cmd,"%%MDSPLUSDIR%%\\%s\\mdsip_service.exe --port=%s --hostfile=\"%s\" %s",dirname,GetPortname(),GetHostfile(),opts);
