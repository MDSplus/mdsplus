...
#ifdef _USE_VARARGS
int MdsValue(va_alist)
va_dcl
{
  SOCKET sock;
  char *expression;
#else
int MdsValue(SOCKET sock, char *expression, ...)
{
/**** NOTE: NULL terminated argument list expected ****/
  ...
  if (status & 1) {
    ...
    void *mem = 0;
    status =
	GetAnswerInfoTS(sock, &ans_arg->dtype, &len, &ans_arg->ndims, ans_arg->dims, &numbytes,
			&dptr, &mem);
    ans_arg->length = len;
    if (numbytes) {
...
va_dcl
{
  ...
  char *expression;
#else
int MdsPut(SOCKET sock, char *node, char *expression, ...)
{
/**** NOTE: NULL terminated argument list expected ****/
#endif
  va_list incrmtr;
  ...
  for (i = idx; i < a_count && (status & 1); i++) {
    status =
	SendArg(sock, (char)i, arg->dtype, nargs, ArgLen(arg), arg->ndims, arg->dims, arg->ptr);
    arg = va_arg(incrmtr, struct descrip *);
  }
  va_end(incrmtr);
  if (status & 1) {
...
int MdsOpen(SOCKET sock, char *tree, int shot)
{
  ...
  int status =
      MdsValue(sock, expression, MakeDescrip((struct descrip *)&treearg, DTYPE_CSTRING, 0, 0, tree),
...
int MdsSetDefault(SOCKET sock, char *node)
{
  ...
  int status =
      MdsValue(sock, expression, MakeDescrip(&nodearg, DTYPE_CSTRING, 0, 0, node), &ansarg, NULL);
  if ((status & 1) && (ansarg.dtype == DTYPE_LONG))
    status = *(int *)ansarg.ptr;
  if (ansarg.ptr)
    free(ansarg.ptr);
...
STATIC_ROUTINE int MdsLoginVMS(SOCKET sock, char *username, char *password)
{
  ...
  ansarg.ptr = 0;
  status =
      MdsValue(sock, LOGINREQUEST, MakeDescrip(&loginget_arg, DTYPE_CSTRING, 0, 0, LOGINGETP1),
	       &ansarg, NULL);
  if (status & 1) {
    if (ansarg.ptr && ansarg.dtype == DTYPE_CHAR) {
      ...
      free(ansarg.ptr);
      status =
	  MdsValue(sock, LOGINREQUEST, MakeDescrip(&loginget_arg, DTYPE_CSTRING, 0, 0, LOGINGETP2),
		   &ansarg, NULL);
      if (status & 1) {
	if (ansarg.ptr && ansarg.dtype == DTYPE_SHORT) {
	  ...
	  if (status & 1) {
	    int two = 2;
	    status =
		MdsValue(sock, LOGINREQUEST,
			 MakeDescrip(&loginpwd_arg, DTYPE_CSTRING, 0, 0, LOGINPWD),
...
*/
  ...
  BlockSig(SIGALRM);
  status =
      MdsOpen((SOCKET) ((char *)lpvArgv[0] - (char *)0), (char *)lpvArgv[1],
...
*/
  ...
  BlockSig(SIGALRM);
  status =
      MdsLogin((SOCKET) ((char *)lpvArgv[0] - (char *)0), (char *)lpvArgv[1], (char *)lpvArgv[2]);
  UnBlockSig(SIGALRM);
  return status;
}
...
*/
  ...
  BlockSig(SIGALRM);
  status =
      GetAnswerInfo((SOCKET) ((char *)lpvArgv[0] - (char *)0), (char *)lpvArgv[1],
		    (short *)lpvArgv[2], (char *)lpvArgv[3], (int *)lpvArgv[4], (int *)lpvArgv[5],
		    (void **)lpvArgv[6]);
  UnBlockSig(SIGALRM);
  return status;
}
...
*/
  ...
  BlockSig(SIGALRM);
  status =
      SendArg((SOCKET) ((char *)lpvArgv[0] - (char *)0), idx, dtype, nargs, length, ndims,
...
*/
  return MdsSetCompression((SOCKET) ((char *)lpvArgv[0] - (char *)0),
