...
int createUDTSocket(UDTSOCKET &usock, int port = 0, bool rendezvous = false) {
  ...
  // since we will start a lot of connections, we set the buffer size to smaller
  // value.
  int snd_buf = 16000;
  int rcv_buf = 16000;
...
#endif
{
  ...
  while (torecv > 0) {
    int rcvd = UDT::recv(new_sock,
                         (char *)buffer + g_TotalNum * sizeof(int32_t) - torecv,
                         torecv, 0);
    if (rcvd < 0) {
...
#endif
{
  ...
  while (tosend > 0) {
    int sent = UDT::send(client,
                         (char *)buffer + g_TotalNum * sizeof(int32_t) - tosend,
                         tosend, 0);
    if (sent < 0) {
...
#endif
{
  ...
  while (count > 0) {
    ...
    for (set<SYSSOCKET>::iterator i = tcpread.begin(); i != tcpread.end();
         ++i) {
      int32_t data;
  ...
  for (vector<UDTSOCKET>::iterator i = new_socks.begin(); i != new_socks.end();
       ++i) {
    UDT::close(*i);
  }

  for (vector<SYSSOCKET>::iterator i = tcp_socks.begin(); i != tcp_socks.end();
       ++i) {
#ifndef WIN32
    close(*i);
#else
...
#endif
{
  ...
  getnameinfo(addr, size, NULL, 0, sharedport, sizeof(sharedport),
              NI_NUMERICSERV);

  // Note that the first shared port has been created, so we start from
  // g_IndUDTNum + 1.
  ...
  for (vector<UDTSOCKET>::iterator i = cli_socks.begin(); i != cli_socks.end();
       ++i) {
...
#endif
{
  ...
  for (vector<UDTSOCKET>::iterator i = srv_socks.begin(); i != srv_socks.end();
       ++i) {
    connect(*i, peer_port++);
  }
...
#endif
{
  ...
  for (vector<pthread_t>::iterator i = cli_threads.begin();
       i != cli_threads.end(); ++i) {
  ...
  for (vector<HANDLE>::iterator i = cli_threads.begin(); i != cli_threads.end();
       ++i) {
