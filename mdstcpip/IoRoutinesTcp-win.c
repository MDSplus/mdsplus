...
static int tcp_reuseCheck(char *host, char *unique, int buflen) {
  struct sockaddr_in sin;
  int status = getHostAndPort(host,&sin);
  if (status == 1) {
    unsigned char *addr=(char *)&sin.sin_addr;
    *unique=(char *)malloc(64);
...
static int tcp_listen(int argc, char **argv) {
  ...
  if (GetMulti()) {
	...
	if (FD_ISSET(s, &readfds)) {
	  ...
	  status=AcceptConnection("tcp", "tcp", sock, &sock, sizeof(s),&id,&username);
	  if (status & 1) {
    ...
    int sock=0;
    char *username;
    int status=AcceptConnection("tcp", "tcp", sock, &sock, sizeof(sock),&id,&username);
    if (status & 1) {
      Client *new=memset(malloc(sizeof(Client)),0,sizeof(Client));
      new->id=id;
      new->sock=sock;
