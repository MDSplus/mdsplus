...
static void ParseCommand(int argc, char **argv, char **portname, short *port, char **hostfile,
			 char *mode, int *flags, int *compression_level);
...
static void ConvertFloat(int nbytes, int in_type, char in_length, char *in_ptr, int out_type,
...
extern int ConnectToInet(unsigned short port,
			 void (*AddClient_in) (SOCKET, struct sockaddr_in *, char *),
			 int (*DoMessage_in) (SOCKET s));
...
{
  HANDLE sharedMemHandle =
      CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, sizeof(int), ServiceName());
...
static int SpawnWorker(SOCKET sock)
{
  ...
  char cmd[1024];
  sprintf(cmd, "mdsip_service -p %s -c %d -W %d %d", Portname, MaxCompressionLevel,
...
int main(int argc, char **argv)
{
  ...
  if (flags & IS_SERVICE) {
    SERVICE_TABLE_ENTRY srvcTable[] = { {ServiceName(), (LPSERVICE_MAIN_FUNCTION) ServiceMain}
    , {NULL, NULL}
    };
    IsService = 1;
...
static void InstallService(int typesrv)
{
  ...
  if (hSCManager) {
    ...
    hService = CreateService(hSCManager, ServiceName(), ServiceName(), 0, SERVICE_WIN32_OWN_PROCESS,
			     SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, file, NULL, NULL, NULL, NULL,
			     NULL);
    if (hService == NULL)
      status = GetLastError();
    free(file);
...
#else
static void InstallService(int arg)
{
  printf("install option only valid with mdsip_service on NT\n");
  exit(0);
}
...
static int SpawnWorker(int arg)
{
  return getpid();
}
  ...
  if (f) {
	...
	if (access_id.length) {
	  if (access_id.pointer[0] != '!') {
	    if ((StrMatchWild(&match1, &access_id) & 1)
		|| (match2.length > 0 && (StrMatchWild(&match2, &access_id) & 1)))
...
#else
    if (hp)
      printf("%s (pid %d) Connection received from: %s [%s]\n%s\r\n", timestr, pid, hp->h_name,
	     inet_ntoa(sin->sin_addr), dn);
    else
...
static void ExecuteMessage(Client * c)
{
  ...
  if (StrCompare(c->descrip[0], &eventastreq) == 0) {
    ...
    if (c->event) {
      if (java)
	for (p = &c->event, e = c->event; e && (*c->descrip[1]->pointer != e->jeventid);
	     p = &e->next, e = e->next) ;
      else
    ...
    if (status & 1)
      status = TdiData(xd, &ans MDS_END_ARG);
    if (!(status & 1))
...
static int ParseOption(char *option, char **argv, int argc, char **portname, short *port,
		       char **hostfile, char *mode, int *flags, int *compression_level)
{
  int increment = 0;
  ...
  *hostfile = 0;
  for (i = 1; i < argc; i++) {
	...
	PrintHelp(0);
      ...
      case 'p':
	if (argc > (i + 1))
	  *port = ParsePort(argv[++i], portname);
	else
	  PrintHelp(0);
	break;
      case 'I':
	...
	if (argc > (i + 1)) {
	  i++;
	  *hostfile = strcpy((char *)malloc(strlen(argv[i]) + 1), argv[i]);
	} else
	  PrintHelp(0);
	break;
      case 'c':
	if (argc > (i + 1) && *argv[i + 1] >= '0' && *argv[i + 1] <= '9') {
	  i++;
	...
	break;
      case '-':
	i += ParseOption(&option[2], &argv[i + 1], argc - i - 1, portname, port, hostfile, mode,
  ...
  if (*compression_level < 0)
    ...
    *compression_level = 9;
  if (*hostfile == 0)
    *hostfile = strcpy((char *)malloc(strlen(DEFAULT_HOSTFILE) + 1), DEFAULT_HOSTFILE);
  if (*port == 0) {
