...
static char fmt_syntax1[] = "static struct cduVerb  %s = {\t/* Syntax def\t*/\n";
static char *fmt_syntax2 = fmt_verbs2;

static char fmt_params1[] = "static struct cduParam  %sparams[%d] = {\n";
static char fmt_params2[] = "       %c\"%s\",0x%04X,%s,%s,%s,0\n";

static char fmt_qualifiers1[] = "static struct cduQualifier %squalifiers[%d] = {\n";
...
static char fmt_protoUserType1[] = "static struct cduKeyword  %s[%d];\t/* Prototype only\t*/\n";
static char fmt_userType1[] = "static struct cduKeyword  %s[%d] = {\t/* \"Type\" def\t*/\n";
...
static void writeSyntax();
	 ...
	 * writeExtern:
	 **************************************************************/
static void writeExtern(char name[]	/* <r> name of extern value         */
    )
{
  ...
  return;
	 ...
	 * writeValue:
	 **************************************************************/
static void writeValue(char parentPrefix[]	/* <r> string to identify "parent"  */
    ...
    )
{
  ...
  if (debugCdu)
	 ...
	 * writeUserType:
	 ***************************************************************/
static void writeUserType(char name[]	/* <r> userType name                        */
    )
{
  int icnt;
  char prefix[32];
  ...
  for (icnt = 0; (key = findKeywordByIdx(name, icnt)); icnt++) {
    ...
    fprintf(fp, fmt_userType2, icnt ? ',' : ' ', key->keyA_name,
	    key->keyL_flags | ENT_M_KEYWORDS, textLabel, textSyntax, textValue);
  }
  ...
  return;
	 ...
	 * writeParams:
	 ****************************************************************/
static void writeParams(char parentPrefix[]	/* <r> string to identify "parent"  */
    ...
    )
{
  ...
  for (icnt = 0; (prm = findParamByIdx(v, icnt + 1)); icnt++) {
    ...
    fprintf(fp, fmt_params2, icnt ? ',' : ' ',
	    prm->prmA_name, prm->prmL_flags | ENT_M_PARAMETERS, textLabel, textPrompt, textValue);
  }
  fprintf(fp, "       ,0\t\t\t\t/* null entry at end\t*/\n");
  fprintf(fp, "       };\n");
  return;
	 ...
	 * writeQualifiers:
	 ****************************************************************/
static void writeQualifiers(char parentPrefix[]	/* <r> string to identify "parent"  */
			    , struct verblist *v	/* <r> the verb                         */
    )
{
  int icnt;
  ...
  return;
	 ...
	 * writeSyntax:
	 ****************************************************************/
static void writeSyntax(char name[]	/* <r> name for user-defined syntax */
    )
{
  char prefix[32];
  ...
  fprintf(fp, fmt_syntax2, ' ',
	  ...
	  v->vrbA_routine ? v->vrbA_routine : "0", textParameters, textQualifiers);
  fprintf(fp, "       };\n");
  return;
}
    ...
    )
{
  ...
  return (1);
	 ...
	 * cdu_write:
	 ****************************************************************/
void cdu_write(char _module[]	/* <r> name of module -- filename too       */
    )
{
  int i, k;
  int icnt;
