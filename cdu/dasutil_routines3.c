...
#define HISTORY_NOT_ACTIVE (-99)	/* special value for irecall    */
	 ...
	 ****************************************************************/
static int decode_escape_sequence(int fd	/* <r> input file descr         */
    )
{
  int i;
  char c;
...
#ifndef CURSES
static int setupterm(char *term, int fd, int *ierr)
{
  *ierr = 1;
  return (0);
}
    ...
    )
{
  ...
  if (!isatty(fd))
    return (fgets(usrline, maxlen, fp));
		 ...
		 *======================================================*/
  if (!cursorRight[0]) {	/* first time only ...                  */
...
/*        smg$create_key_table(&key_table_id);		/*  */
  ...
  sts = smg$read_composed_line(&keyboard_id, 0, &dsc_cmdline, dscPtr, &wlen);
  if (~sts & 1) {
  ...
  for (p = line;;) {
    ...
    if (isprint(c) || isspace(c)) {
      ...
      if (*p) {			/* if not at end of line                */
	if (insert_mode) {
	  k = strlen(p);
	  p[k + 1] = '\0';
	  for (; k; k--)
	    p[k] = p[k - 1];
	  k = sprintf(displayLine, "%s%s%s", save_cursor, p + 1, restore_cursor);
	...
	*(p + 1) = '\0';	/* else, set new EOL            */
	...
	continue;
      ...
      k = sprintf(displayLine, "%s%s%s %s", cursor_left, save_cursor, (i ? p : ""), restore_cursor);
      write(1, displayLine, k);
      continue;
    }
	...
	write(1, displayLine, k);
	for (i = 0; (line[i] = p[i]); i++) ;	/* shift chars within line[]    */
