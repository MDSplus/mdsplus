...
#define LINEWIDTH    70		/*..width of display line               */
	 ...
	 **************************************************************/
static int strncmp_nocase(char *s	/* <r> 1st string to compare            */
			  , char *t	/* <r> 2nd string  "    "               */
...
//        return(0);
  ...
  if (icount == 0) {
	...
	printf("Valid commands are:\n");
      for (i = 1, j = 0; i < ilast; i++) {	/* Print out commands           */
	if (!(cmdlist + i)->cmdA_string)
	  break;
	ispace = j ? (COLWIDTH - (j - FIRST_COLUMN) % COLWIDTH) : FIRST_COLUMN;
...
************************************************************************/
	 ...
	 ****************************************************************/
char *l2u(char *s		/* <m> string to modify                         */
    ...
    )
{
  ...
  return (to);
	 ...
	 ****************************************************************/
char *u2l(char *s		/* <m> string to modify                         */
	  , char *t		/* <r:opt> source string                        */
    )
{
  char *in;
    ...
    )
{
  ...
  if (isalpha(*p) || isAcceptable(*p)) {
    ...
    tknLen = p2 - p;
    if (tknLen > maxLen) {
      dasmsg(0, "nextToken: *WARN* tkn truncated from %d to %d chars", tknLen, maxLen);
      tknLen = maxLen;
    }
    ...
    for (; *p == quoteMark; p = p2) {
      ...
      if (p2) {			/* Found a matching quoteMark...        */
	n = p2 - p;		/* string length, could be zero */
	if (*(p2 + 1) == quoteMark)
	  n++;			/*..it's a double quoteMark     */
      } else {			/*..no balancing quoteMark              */
	...
	p2 = p + n;
      ...
      if (tknLen + n > maxLen) {
	if (tknLen < maxLen)
	  dasmsg(0, "nextToken: *WARN* truncating to %d chars", maxLen);
	n = maxLen - tknLen;
      }
      if (n) {
    ...
    )
{
  ...
  if (!p)
    return (0);				/*----------------> return: err	*/
			...
			/*------------------- day...--------------------*/
  if (isdigit(*p)) {		/* string starts with decimal digit ...         */
  ...
  } else {			/* string starts with non-digit ...             */
    day = 0;
    if (*p == '-')
      p++;
    if (!(isdigit(*p) || *p == ':'))
