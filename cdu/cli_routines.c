    ...
    )
{
  ...
  if (icnt > maxEntities) {	/*..need larger entLookup[] table       */
  ...
  for (i = 1; i <= icnt; i++, e++) {
    ...
    entLookup[i].cmdA_string = e->entA_label ? e->entA_label : e->entA_name;
  }
  if (i < maxEntities) {
    ...
    )
{
  ...
  for (i = 1; i <= icnt; i++, e++) {
    ...
    e->entL_status = (e->entL_flags & ENT_M_DEFAULTED) ? CLI_STS_DEFAULTED : sts;
  }
  return (icnt);
}
    ...
    )
{
  ...
  if (icnt > maxVerbs) {	/*...need larger cmdVerb[] table        */
    maxVerbs = icnt;
    if (cmdVerb)
  ...
  return 1;
	 ...
	 * clearCurrentSyntax:
	 ***************************************************************/
static void setCurrentSyntax(int flag	/* <r> T==>newVerb  F==>newSyntax only  */
    ...
    )
{
  ...
  if (flag || v->vrbA_parameters) {
    ...
    for (i = 0; i < k; i++) {	/* Copy val_dsc and status for each param ...   */
    ...
    )
{
  ...
  keycnt = 0;
  for (; p; keycnt++) {
    if (keycnt) {		/* skip separator character             */
      if (*p == '\01' || *p == '\02') {
    ...
    )
{
  ...
  } else {			/* Not found:  check for negated qualifier ...  */
    if (toupper(**pp) == 'N' && toupper(*(*pp + 1)) == 'O') {
      p = *pp + 2;
      opt = cmd_lookup(&p, cmdQual, 0, NOMSG, 0);
      if (opt > 0) {
	qual = currentQualifiers + (opt - 1);
	if (qual->qualL_flags & QUAL_M_NONNEGATABLE) {	/* default for qualifiers is NEGATABLE  */
    ...
    )
{
  ...
  *pp = p;			/* set current pos within cmdline       */
  return (sts);
	 ...
	 * make_entityString:
	 ***************************************************************/
static void make_entityString(void *entity	/* <r> dsc or cstring                   */
...
#ifdef vms
  if (val->valL_flags & VAL_M_DELTATIME) {	/* VMS deltatime only ...       */
