...
static struct cmd_struct monthList[13] = {
  ...
  , 1, "JANUARY", 2, "FEBRUARY", 3, "MARCH", 4, "APRIL", 5, "MAY", 6, "JUNE", 7, "JULY", 8,
	 ...
	 * cdate:
	 **************************************************************/
char *cdate(int usrtime		/* <r> time in seconds                  */
    )
{
  struct tm *t;
  time_t bintim;
  ...
  sprintf(datebuf, "%02d-%s-%02d", t->tm_mday, month[t->tm_mon], t->tm_year);
  return (datebuf);
}
	 ...
	 * cdatime:
	 ************************************************************/
char *cdatime(int usrtime	/* <r> time in seconds                  */
    )
{
  int year;
  struct tm *t;
  ...
  sprintf(datimbuf, "%02d-%s-%04d %02d:%02d:%02d",
	  t->tm_mday, month[t->tm_mon], year, t->tm_hour, t->tm_min, t->tm_sec);
  return (datimbuf);
}
...
static void clearAll()
{
  ...
  for (; inIdx; inIdx--) {
    ...
    p = "indirect file";
#endif
    printf("  --> closing %s,\tline %d\n", p ? p : " ???unknown???", inputStruct[inIdx].lineno);
    fclose(fp);
  }
  ...
  return;
	 ...
	 * successReturn:
	 *****************************************************************/
static int successReturn(char *linePtr	/* <r> addr at end of scanned token     */
    ...
    )
{
  ...
  return (1);
	 ...
	 * openIndirectFile:
	 *****************************************************************/
static void openIndirectFile(char *linePtr	/* <r> current pos within cmd line      */
    )
{
  FILE *fp;
  ...
  else {			/*..else tty input: clear rest of line  */
  ...
  return;			/*----------------------------> return	*/
	 ...
	 * startNextToken:
	 *****************************************************************/
static char *startNextToken(void *dsc_prompt	/* <r> string or descr: prompt string   */
    )
{
  int k;
  int len;
  ...
  for (; !linePtr; linePtr = nonblank(linePtr)) {
		 ...
		 *-------------------------------------------------------*/
    if (dsc_prompt && ip->isatty) {	/*..only if in interactive mode         */
...
double dval;
{
  static char fmt_OutOfRange[] = "--> GETNEXT: input is out-of-range.  Legal range is %.6g <= val";

			/*-----------------------------------------------
			 * Check for min and max range ...
			 *----------------------------------------------*/
  if (dmin == 0.0 && dmax == 0.0) ;	/* do nothing           */
  else {			/*..else check limits   */
  ...
  return (0);				/*--------------------> return	*/
	 ...
	 * badInput:
	 **************************************************************/
static void badInput(char *txt	/* <r> Message text to print out [0 for none]   */
    )
{
  if (!inputStruct[inIdx].isatty)
    ...
    )
{
  ...
  static struct cmd_struct cmd_yesno[5] = {
    ...
    , CMD_YES, "YES", CMD_NO, "NO", CMD_YES, "TRUE", CMD_NO, "FALSE"
  };
