...
#endif
#define P2W_UNIQUE_LOCK_ACQUIRE(lock, mutex) {std::unique_lock<const decltype(mutex)> lock(mutex);P2W_LOCK_CLEANUP_PUSH(lock)
#define P2W_UNIQUE_LOCK_ADOPT(lock, mutex)   {std::unique_lock<const decltype(mutex)> lock(mutex, std::adopt_lock);P2W_LOCK_CLEANUP_PUSH(lock)
#define P2W_UNIQUE_LOCK_RELEASE(lock, mutex) P2W_LOCK_CLEANUP_POP(lock);}
#define P2W_SHARED_LOCK_ACQUIRE(lock, mutex) {(mutex).lock_shared();std::unique_lock<const decltype(mutex)> lock(mutex, std::adopt_lock);P2W_LOCK_CLEANUP_PUSH(lock)
#define P2W_SHARED_LOCK_ADOPT(lock, mutex)   {std::unique_lock<const decltype(mutex)> lock(mutex, std::adopt_lock);P2W_LOCK_CLEANUP_PUSH(lock)
#define P2W_SHARED_LOCK_RELEASE(lock, mutex) P2W_LOCK_CLEANUP_POP(lock);}
...
public:
    ...
    P2WMutex(int const type)
    {
        ...
        if (unlikely(err != 0)) throw std::runtime_error(strerror(err));
