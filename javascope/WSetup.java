...
//wave selezionata
//*****
 class SetupData extends ScopePositionDialog { 
   ...
   Label            lab;
   SignalError      error_w = new SignalError(fw);
   ErrorMessage	    error_msg = new ErrorMessage(fw);
   Setup            setup;       
   EvaluateWaveform sd_block;
   WaveformConf	    wc_default, curr_wc;
   int              sel_signal = -1;
   ExpandExpressionDialog expand_expr = new ExpandExpressionDialog(fw, this);	    
     
   static final int  LINE = 0, POINT = 1, BOTH = 2, NONE = 3;


    public SetupData(Frame fw, String frame_title) {
...
//      if(wc.num_expr != 0) sel_signal = 0;	
   ...
   private int getDefaultFlags()
   {
	...
	if  (experiment_b.getState() ) then  value |= 1<<WaveformConf.B_exp ; 
   ...
   private void removeSignalSetup()
   {
      ...
      int num_shot = sd_block.num_shot;
      int num_signal = sd_block.num_signal;  		
	...
	ws_new = new WaveSetupData[sd_block.num_signal - sig_idx];	
	...
	for (i = 0,  j = 0 ; i < num_signal; i++)
	  if(i < start_idx || i >= end_idx)  
             ws_new[j++] = sd_block.signals[i];
	  else   
	     sig_list.remove(start_idx + 1);
	     
	sd_block.signals     = ws_new;
	sd_block.num_signal -= sig_idx; 
      }
      sel_signal = -1;
   ...
   { 
	...
	//opt_network.setState(wc.opt_network);
	sd_block.initSignals(wc, shot.getText());
   ...
   { 
      WaveSetupData[] ws_new = new WaveSetupData[sd_block.num_signal + 1];
     
      for(int i = 0; i < sd_block.num_signal; i++)
	ws_new[i] = sd_block.signals[i];
      ws_new[sd_block.num_signal++] = ws;
      
      sd_block.signals = ws_new;

      return sd_block.signals;
   }
   ...
   private void addSignals()
   {
	...
	return;
      ...
      ws = getSignalSetup();
      idx = sd_block.findSignalSetup(ws);
      if( idx == -1) {
      
	String x_e =  new String(x_expr.getText() == null ? "" : x_expr.getText()) ; 
	String y_e =  new String(y_expr.getText());

	if(sd_block.num_shot != 0)
	{
	    for (int i = 0; i < sd_block.num_shot; i++, ws = getSignalSetup())
    	    {
		ws.shot = sd_block.shots[i];
	 ...
	 sel_signal = sd_block.findSignalSetup(ws);
	 sig_list.select(sel_signal + 1);
	 putSignalSetup(ws);
      }	 
   }
   ...
   private void updateSignals()
   {
      ...
      if(sd_block.findSignalSetup(getSignalSetup()) != -1)
	    return;	
	...
	for (i = 0,  j = 0 ; i < num_signal; i++)
	  if(i >= start_idx && i < end_idx)  {
             sd_block.signals[i].x_expr = x_expr.getText();	        
             sd_block.signals[i].y_expr = y_expr.getText();
	     signalListReplace(i, sd_block.signals[i]);
	  }		
   ...
   private void signalListRefresh()
   {
	...
	for(int i = 0; i < sd_block.num_signal; i++)
	    signalListAdd(sd_block.signals[i]);
	sig_list.select(sel_signal + 1);
   } 
  ...
  public void eraseForm()
  {
   ...
   public WaveformConf saveConfiguration()
   {
      ...
      wc.num_shot = sd_block.num_shot;
      
      if(sd_block.num_shot != 0)
	wc.num_expr = sd_block.num_signal/sd_block.num_shot;
      else
	wc.num_expr = sd_block.num_signal;
	...
	wc.y_label      = new String(y_label.getText());
      ...
      wc.x_expr	      = new String[sd_block.num_signal];
      wc.y_expr	      = new String[sd_block.num_signal];
      wc.up_err	      = new String[sd_block.num_signal];
      wc.low_err      = new String[sd_block.num_signal];
            
      wc.markers      = new int[sd_block.num_signal];
      wc.colors       = new Color[sd_block.num_signal];
      wc.interpolates = new boolean[sd_block.num_signal];
      for(int j = 0, k = 0;  j < wc.num_expr; j++)
      { 
	 if(sd_block.signals[k].x_expr != null)
	    wc.x_expr[j] = new String(sd_block.signals[k].x_expr);
	 if(sd_block.signals[k].y_expr != null)
	    wc.y_expr[j] = new String(sd_block.signals[k].y_expr);
	 if(sd_block.signals[k].up_err != null)
	    wc.up_err[j] = new String(sd_block.signals[k].up_err);
	 if(sd_block.signals[k].low_err != null)
	    wc.low_err[j] = new String(sd_block.signals[k].low_err);

	 for(int i=0; i < sd_block.num_shot; i++)
	 {
	    wc.interpolates[k] = sd_block.signals[k].interpolate;
	    wc.markers[k]      = sd_block.signals[k].marker;
	    wc.colors[k]       = sd_block.signals[k].color;
	    k++;
	 }
      }
   ...
   public void actionPerformed(ActionEvent e)
   {
	...
	if(sel_signal == -1)
    	...
    	if(sd_block.num_signal != 0) {
	    sd_block.signalsRefresh(shot.getText(), setup.main_scope.shot_t.getText(), false);
	    signalListRefresh();
	}
	...
	if(error == 0) {
	    sd_block.UpdateWave(wc);
	    sd_block.error_msg.showMessage();
	}
	else
	...
	putWindowSetup(setup.GetWaveformConf()); 	
	 ...
	 if(sel_signal == -1)
	    return; 
	 error_w.setError(sd_block.signals[sel_signal]);	
	 error_w.pack();
   ...
   public void keyPressed(KeyEvent e)
   {
	 ...
	 if(ob == y_expr || ob == x_expr || ob == shot || ob == experiment) {
		...
		updateSignals();	    	
	    if(sd_block.num_signal != 0) {
		sd_block.signalsRefresh(curr_wc.shot_str = shot.getText(), setup.main_scope.shot_t.getText(), false);
	        signalListRefresh();
	    }	    
         } 
      }
   ...
   public void itemStateChanged(ItemEvent e)
   {
	...
	if(ob == sig_list)
	{
	   ...
	   if(sel_signal >= 0)
	       putSignalSetup(sd_block.signals[sel_signal]);
	   else {
		x_expr.setText("");
		y_expr.setText("");
	    }
	...
	if(ob == marker) {
	    sd_block.signals[sel_signal].marker = marker.getSelectedIndex();
	}	

	if(ob == show_type) {
	    sd_block.signals[sel_signal].interpolate = show_type.getSelectedIndex() == 0 ? true : false;
	}	

	if(ob == color) {
	    sd_block.signals[sel_signal].color = WaveSetupData.COLOR_SET[color.getSelectedIndex()];       
	}
