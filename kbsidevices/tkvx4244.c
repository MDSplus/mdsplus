...
ViInt16 c5,ViInt16 c6,ViInt16 c7,ViInt16 c8,ViInt16 c9,ViInt16 c10,
ViInt16 c11,ViInt16 c12,ViInt16 c13,ViInt16 c14,ViInt16 c15,ViInt16 c16,
ViInt16 ca,ViChar buf[]);
ViStatus tkvx4244_gplist_parse (ViInt16 g1,ViInt16 g2,ViInt16 g3,
ViInt16 g4,ViInt16 ga,ViChar buf[]);
/***************************************************************************
		...
		ViInt16 arrayLength, ViPInt16 numberConnected, ViInt16 slot)
{
	...
	*numberConnected = 0;
	...
	retError = tkvx4244_getSlotandLAList(theSlotPtr, theLogAdr, tkvx4244_RCV_STRLEN,
				&numFound);
	if (retError<VI_SUCCESS)
	{
		return retError;
	...
	for (ii=0; ii<numFound; ii++)
	{
		...
		if (*numberConnected == arrayLength) break;
	}
			...
			ViBoolean resetDevice, ViPSession instrumentHandle) 
{
	...
	retError = viSetAttribute (*instrumentHandle, VI_ATTR_USER_DATA,
				(ViAttr)VI_NULL);
	if (retError < VI_SUCCESS)
	{
			...
			ViChar errorMessage[])		
{
	...
	if ((errorCode >= tkvx4244_GENERIC_ERROR) && (errorCode <= 0xBFFC0FFFL) && (errorCode < VI_SUCCESS))
	 {
			...
			ViChar revision[])
{
	 ...
	 * get the descriptor
	 */
	retError = viGetAttribute (instrumentHandle, VI_ATTR_RSRC_SPEC_VERSION,
					&tmpRev);
...

{
	...
	retError = tkvx4244_confGroupOneChannels (instrumentHandle,
	   ...
	   tkvx4244_CH_THREE_ENABLE,tkvx4244_CH_FOUR_ENABLE,tkvx4244_SELECT_DISABLE);
    	   if(retError) return retError;
    ...
    if(channel<tkvx4244_ANALYZE_CH5){
		...
		retError = tkvx4244_confSamplingInterval (instrumentHandle,
		      tkvx4244_CLOCK_INTERNAL,intervalFrequency,samplingRate,
		      gone,gtwo,gthree,gfour,tkvx4244_ALL_GROUPS_DISABLE);
				if(retError) return retError;
...
ViInt16 ttl4,ViInt16 ttl5,ViInt16 ttl6,ViInt16 ttl7)
{
	...
	drvrHdl = (tkvx4244_PdrvrStruct)userData;
	/*Validate the Command*/
	if((external+immediate+threshold+vxicmd+ttl0+ttl1+ttl2+ttl3+ttl4+ttl5+ttl6+ttl7)==0)
			return tkvx4244_ERROR_TRIG_SRC_CHOICE;
...
ViInt16 numberOfSamples,ViInt32 startingAddress,ViInt16 channel,
ViChar dataArray[])
{
    ...
    /* Validate Command */
    retError = tkvx4244_invalidShortRange (numberOfSamples, VI_TRUE,
        1000,tkvx4244_ERROR_SAMP_CNT);
        if(retError) return retError;
    retError = tkvx4244_invalidShortRange (channel,tkvx4244_ANALYZE_CH1,
        tkvx4244_ANALYZE_CH16,tkvx4244_ERROR_FETCH_CHL);
        if(retError) return retError;
...
ViInt16 numberOfSamples,ViInt32 startingAddress,ViInt16 channel,ViInt16 response[])
{
    ViInt16 btcnt;
	ViChar		cmd[tkvx4244_CMD_STRLEN];/* storage for instrument command strings */
	ViStatus	retError;			/* Returned error status */
...
ViInt16 numberOfSamples,ViInt32 startingAddress,ViInt16 channel,ViInt16 response[])
{
	...
	if (retError<VI_SUCCESS) {free(rdd); return retError;}
...
ViInt32 startingAddress,ViInt16 channel,ViChar response[])
{
    ...
    btcnt = 2 * numberOfSamples;
    /* Validate Command */
    retError = tkvx4244_invalidShortRange (numberOfSamples,1,
        5000,tkvx4244_ERROR_SAMP_CNT);
        if(retError) return retError;
    retError = tkvx4244_invalidShortRange (channel,tkvx4244_ANALYZE_CH1,
        tkvx4244_ANALYZE_CH16,tkvx4244_ERROR_FETCH_CHL);
        if(retError) return retError;
...
ViInt16 queryCondition,ViChar list[])
{
	...
	drvrHdl = (tkvx4244_PdrvrStruct)userData;
    /* Validate command */
    retError = tkvx4244_invalidShortRange (queryCondition,tkvx4244_CHAN_QUERY_OPEN,tkvx4244_CHAN_QUERY_STATE,tkvx4244_ERROR_ACT_CHNL_QUERY);
        if(retError) return retError;
    /* Build the command string */
...
,ViInt16 c15,ViInt16 c16,ViInt16 ca,ViChar answer[])
{
	...
	retError = tkvx4244_invalidShortRange (ca, 0, 1, tkvx4244_ERROR_ALL_CHAN);
		   if(retError) return retError;
    if (ca + c1 + c2 + c3 + c4 + c5 + c6 + c7 + c8 + c9 + c10 + c11 + c12 + c13 + c14 + c15 + c16 == VI_FALSE)
        return  tkvx4244_NO_CHAN_SELECTED_ERROR;
    /* Parse the ch list */
    tkvx4244_chlst_parse (c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15,c16,ca,chan);
    /* Build the command string */
    sprintf (cmd, "VOLT:RANG:%s? (%s)\n", vlt_type[rang], chan);
    /* Send the command string */
	retError = viWrite (instrumentHandle, (ViBuf)cmd, strlen(cmd),(ViPUInt32)&retCnt);
...
ViInt16 group4)
{
	...
	tkvx4244_THRESH_POS_SLOPE, tkvx4244_THRESH_LESS_THAN,tkvx4244_ERROR_THRESH_TYPE);
		   if(retError) return retError;
...
ViInt16 memoryZero)
{
    ...
    /* Validate the command */
	retError = tkvx4244_invalidShortRange (source,tkvx4244_EXTERNAL_ARM,tkvx4244_IMMEDIATE_ARM,tkvx4244_ERROR_EXT_ARM_SLP);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (memoryZero,tkvx4244_MEMORY_ZERO_ON,tkvx4244_MEMORY_ZERO_OFF,tkvx4244_ERROR_EXT_ARM_ZERO);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (gp1,tkvx4244_GROUP_ONE_DISABLE,
	       tkvx4244_GROUP_ONE_ENABLE,tkvx4244_ERROR_GP_ONE);
		   if(retError) return retError;
...
ViInt16 ch1,ViInt16 ch2,ViInt16 ch3,ViInt16 ch4,ViInt16 voltageRange)
{
    ViChar chan[40];
	ViChar		cmd[tkvx4244_CMD_STRLEN];/* storage for instrument command strings */
	ViStatus	retError;			/* Returned error status */
	...
	tkvx4244_VOLT_RANGE_20VOLT,tkvx4244_ERROR_INPUT_VOLT);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch1,tkvx4244_CH_ONE_DISABLE,tkvx4244_CH_ONE_ENABLE,tkvx4244_ERROR_CHAN_ONE);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch2,tkvx4244_CH_TWO_DISABLE,tkvx4244_CH_TWO_ENABLE,tkvx4244_ERROR_CHAN_TWO);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch3,tkvx4244_CH_THREE_DISABLE,tkvx4244_CH_THREE_ENABLE,tkvx4244_ERROR_CHAN_THREE);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch4,tkvx4244_CH_FOUR_DISABLE,tkvx4244_CH_FOUR_ENABLE,tkvx4244_ERROR_CHAN_FOUR);
		   if(retError) return retError;
...
ViInt16 ch5,ViInt16 ch6,ViInt16 ch7, ViInt16 ch8,ViInt16 voltageRange)
{
    ViChar chan[40];
	ViChar		cmd[tkvx4244_CMD_STRLEN];/* storage for instrument command strings */
	ViStatus	retError;			/* Returned error status */
    ...
    /* Validate the command */
	retError = tkvx4244_invalidShortRange (voltageRange,tkvx4244_VOLT_RANGE_200MV,tkvx4244_VOLT_RANGE_20VOLT,tkvx4244_ERROR_INPUT_VOLT);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch5,tkvx4244_CH_FIVE_DISABLE,tkvx4244_CH_FIVE_ENABLE,tkvx4244_ERROR_CHAN_FIVE);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch6,tkvx4244_CH_SIX_DISABLE,tkvx4244_CH_SIX_ENABLE,tkvx4244_ERROR_CHAN_SIX);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch7,tkvx4244_CH_SEVEN_DISABLE,tkvx4244_CH_SEVEN_ENABLE,tkvx4244_ERROR_CHAN_SEVEN);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch8,tkvx4244_CH_EIGHT_DISABLE,tkvx4244_CH_EIGHT_ENABLE,tkvx4244_ERROR_CHAN_EIGHT);
		   if(retError) return retError;
...
ViInt16 ch9,ViInt16 ch10,ViInt16 ch11,ViInt16 ch12,ViInt16 voltageRange)
{
    ViChar chan[40];
	ViChar		cmd[tkvx4244_CMD_STRLEN];/* storage for instrument command strings */
	ViStatus	retError;			/* Returned error status */
	...
	retError = tkvx4244_invalidShortRange (voltageRange,tkvx4244_VOLT_RANGE_200MV,tkvx4244_VOLT_RANGE_20VOLT,tkvx4244_ERROR_INPUT_VOLT);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch9,tkvx4244_CH_NINE_DISABLE,tkvx4244_CH_NINE_ENABLE,tkvx4244_ERROR_CHAN_NINE);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch10,tkvx4244_CH_TEN_DISABLE,tkvx4244_CH_TEN_ENABLE,tkvx4244_ERROR_CHAN_TEN);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch11,tkvx4244_CH_ELEVEN_DISABLE,tkvx4244_CH_ELEVEN_ENABLE,tkvx4244_ERROR_CHAN_ELEVEN);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch12,tkvx4244_CH_TWELVE_DISABLE,tkvx4244_CH_TWELVE_ENABLE,tkvx4244_ERROR_CHAN_TWELVE);
		   if(retError) return retError;
...
ViInt16 ch13,ViInt16 ch14,ViInt16 ch15,ViInt16 ch16,ViInt16 voltageRange)
{
    ViChar chan[40];
	ViChar		cmd[tkvx4244_CMD_STRLEN];/* storage for instrument command strings */
	ViStatus	retError;			/* Returned error status */
	...
	retError = tkvx4244_invalidShortRange (voltageRange,tkvx4244_VOLT_RANGE_200MV,tkvx4244_VOLT_RANGE_20VOLT,tkvx4244_ERROR_INPUT_VOLT);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch13,tkvx4244_CH_THIRTEEN_DISABLE,tkvx4244_CH_THIRTEEN_ENABLE,tkvx4244_ERROR_CHAN_THIRTEEN);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch14,tkvx4244_CH_FOURTEEN_DISABLE,tkvx4244_CH_FOURTEEN_ENABLE,tkvx4244_ERROR_CHAN_FOURTEEN);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch15,tkvx4244_CH_FIFTEEN_DISABLE,tkvx4244_CH_FIFTEEN_ENABLE,tkvx4244_ERROR_CHAN_FIFTEEN);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch16,tkvx4244_CH_SIXTEEN_DISABLE,tkvx4244_CH_SIXTEEN_ENABLE,tkvx4244_ERROR_CHAN_SIXTEEN);
		   if(retError) return retError;
...
ViInt16 allGroups,ViInt32 fdcBufferSize,ViInt32 startingAddress)
{
    ...
    retError = tkvx4244_invalidLongRange (fdcBufferSize, tkvx4244_FDC_BUF_2K,
		   ...
		   if(retError) return retError;
    retError = tkvx4244_invalidLongRange (startingAddress, -262143,
	       262145, tkvx4244_ERROR_MEM_ADDR);
		   if(retError) return retError;
...
ViStatus _VI_FUNC
tkvx4244_getFDCAttribute (ViSession instrumentHandle,ViUInt16 attr[])
{
	ViStatus retError;
...
*/
	...
	retError = viRead (instrumentHandle, (ViPBuf)dataArray, (2*numberOfSamples), (ViPUInt32)&retCnt);
	   if (retError<VI_SUCCESS){viSetAttribute (instrumentHandle, VI_ATTR_IO_PROT,VI_NORMAL );
	   return retError;}
...
ViInt16 ch1,ViInt16 ch2,ViInt16 ch3,ViInt16 ch4,ViInt16 select)
{
	...
	/* Validate the command */
	retError = tkvx4244_invalidShortRange (select,tkvx4244_SELECT_DISABLE,tkvx4244_SELECT_ENABLE,tkvx4244_ERROR_OPEN_CLOSE);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch1,tkvx4244_CH_ONE_DISABLE,tkvx4244_CH_ONE_ENABLE,tkvx4244_ERROR_CHAN_ONE);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (ch2,tkvx4244_CH_TWO_DISABLE,tkvx4244_CH_TWO_ENABLE,tkvx4244_ERROR_CHAN_TWO);
...
ViInt16 ch7,ViInt16 ch8,ViInt16 ch9,ViInt16 ch10,ViInt16 ch11,ViInt16 ch12,ViInt16 ch13,ViInt16 ch14,ViInt16 ch15,ViInt16 ch16,ViInt16 all)
{
	...
	retError = tkvx4244_invalidShortRange (ch16,tkvx4244_CH_SIXTEEN_DISABLE,tkvx4244_CH_SIXTEEN_ENABLE,tkvx4244_ERROR_CHAN_SIXTEEN);
		   if(retError) return retError;
	retError = tkvx4244_invalidShortRange (all,tkvx4244_DONT_SELECT_ALL_CHANNELS,tkvx4244_SELECT_ALL_CHANNELS,tkvx4244_ERROR_ALL_CHAN);
		   if(retError) return retError;

    /* no channels chosen */
    if ((all+ch1+ch2+ch3+ch4+ch5+ch6+ch7+ch8+ch9+ch10+ch11+ch12+ch13+ch14+ch15+ch16)==VI_FALSE)
        return  tkvx4244_NO_CHAN_SELECTED_ERROR;
    /* attempt to enable 3 channels in a group */
    if (((ch1+ch2+ch3+ch4) == 3)||((ch5+ch6+ch7+ch8)==3)||((ch9+ch10+ch11+ch12)==3)||((ch13+ch14+ch15+ch16)==3))
        return  tkvx4244_THREE_CHAN_IN_GP_ERROR;
    /* Parse the ch list */
    tkvx4244_chlst_parse (ch1,ch2,ch3,ch4,ch5,ch6,ch7,ch8,ch9,ch10,ch11,ch12,ch13,ch14,ch15,ch16,all,chan);
    /* Build the command string */
    if (select == 1)
        sprintf(cond, "CLOSE");
    else
...
ViInt16 all ,ViInt16 initiate)
{
    ...
    /* Validate the command */
    retError = tkvx4244_invalidRealRange (*delay,0L,1.0E9,
           tkvx4244_ERROR_INIT_DELAY);
		   if(retError) return retError;
...
ViChar masterClockFrequency[])
{
    ...
    else  if (tkvx4244_invalidRealRange (frequency, 76.59310000000001, 5000000.0,  tkvx4244_FREQ_RANGE_ERROR) != 0)
        return retError;
 ...
 */
ViStatus 
tkvx4244_gplist_parse (ViInt16 gp1,ViInt16 gp2,ViInt16 gp3,
...
ViStatus tkvx4244_findInstruments (ViSession resMgr, ViChar searchString[])
{
	...
	do
	{
		...
		retError = viOpen(resMgr, descriptor, VI_NULL, VI_NULL,
								&session);
		if (retError < VI_SUCCESS)
		{
			viClose(findSession);
