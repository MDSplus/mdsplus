...
class PI_SCT320(Device):
    ...
    for i in range(0,9):
        parts.append({'path':'.GRATINGS.GRATING_%02d'%(i+1), 'type':'structure'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:GROOVE'%(i+1), 'type':'numeric'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:BLAZE'%(i+1), 'type':'text'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:MODEL'%(i+1), 'type':'text'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:OFFSET'%(i+1), 'type':'numeric'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:ADJUST'%(i+1), 'type':'numeric'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:FOCAL'%(i+1), 'type':'numeric'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:INCLUSION'%(i+1), 'type':'numeric'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:DETECTOR'%(i+1), 'type':'numeric'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:DISPERSION'%(i+1), 'type':'signal'})
        parts.append({'path':'.GRATINGS.GRATING_%02d:LAMBDA_NM'%(i+1), 'type':'numeric'})
    ...
    parts.append({'path':'.CALIBRATION.GEOMETRY:GRATING_DISP', 'type':'numeric'})
    ...
    for i in range(0,24):
        ...
        parts.append({'path':'.ROIS.ROI_%02d:LOS_PATH'%(i+1), 'type':'text'})
        parts.append({'path':'.ROIS.ROI_%02d:SPECTRA'%(i+1), 'type':'signal'})
    ...
    def load_calib(self):
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot load geometry calibration file  %s %s'%( spectroCalibFile, str(e) ) )
        ...
        try:
            ...
            intensity_coeff_arr = [c.value for c in np.asarray(matrix).reshape(self.ROI_NUM*self.GRATING_NUM*self.SLIT_NUM*self.WIDTH_NUM*(cal_intensity_degree+1))]
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Error writing geometry calibration comment field ' + str(e) )
            raise mdsExceptions.TclFAILED_ESSENTIAL

        try:
            self.calibration_geometry_date.putData(cal_geometry_date)
        except Exception as e :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Error writing geometry calibration date field ' + str(e) )
            raise mdsExceptions.TclFAILED_ESSENTIAL

        try:
            self.calibration_intensity_date.putData(cal_intensity_date)
        except Exception as e :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Error writing intensity calibration date field ' + str(e) )
            raise mdsExceptions.TclFAILED_ESSENTIAL

        try:
            self.calibration_intensity_comment.putData(cal_intensity_comment)
        except Exception as e :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Error writing intensity calibration comment field ' + str(e) )
            raise mdsExceptions.TclFAILED_ESSENTIAL

        try:
            self.intens_cal.putData(cal_intensity_type)
        except Exception as e :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Error writing intensity calibration type field ' + str(e) )
            raise mdsExceptions.TclFAILED_ESSENTIAL
...
#Read sensor values
        ...
        try:
            ...
            dev = serial.Serial(serialPort, 9600, serial.EIGHTBITS, stopbits=serial.STOPBITS_ONE ,timeout=0, parity=serial.PARITY_NONE)
        except serial.SerialException as e :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot connect to serial port  '+ serialPort + ' ' + str(e) )
            raise mdsExceptions.TclFAILED_ESSENTIAL
        except Exception as e :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot resolve serial port value '+ str(e) )
            raise mdsExceptions.TclFAILED_ESSENTIAL
        ...
        if self.sendCommand(dev, 'MONO-EESTATUS', out) == self.SCT_320_OK :
            ...
            if dev != 0 :
                dev.close()
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot cannot read spectrograph configuration ' )
            raise mdsExceptions.TclFAILED_ESSENTIAL
        ...
        try:
            ...
            if dev != 0 :
                dev.close()
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Cannot resolve GRATING value '+ str(e) )
            raise mdsExceptions.TclFAILED_ESSENTIAL
        ...
        for i in range(self.ROI_NUM):
            try:
                ccdRoiNode = self.__getattr__('%s.SPECTRUM_%02d:DATA'%(detector,i+1))
                getattr(self, 'rois_roi_%02d_spectra'%(i+1) ).putData(ccdRoiNode)
            except Exception as e:
