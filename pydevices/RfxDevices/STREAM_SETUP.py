...
class STREAM_SETUP(Device):
    ...
    for chanId in range(1, 65):
        ...
        parts.append({'path':'.'+chanName+':GAIN', 'type':'numeric', 'value':1})
        parts.append({'path':'.'+chanName+':OFFSET', 'type':'numeric', 'value': 0})
      ...
      for chan in range(0, 64):
        try:
          ...
          try:
            ...
            gain = Float32(1.)
          try:
            offset = getattr(self, 'chan_%02d_offset'%(chan+1)).getData()
          except:
            offset = Float32(0.)
