...
#All there firlds are TreeNode instances and therefore all TreeNode methods such as Data() or putData() can be used.
    ...
    parts = [
        {'path':':ADDR','type':'text'}, {'path':':COMMENT','type':'text'},
    ...
    for i in range(4):
        parts.extend([
            ...
            {'path':'.CHANNEL_%d:START_IDX'%(i),'type':'numeric', 'value':0},
            {'path':'.CHANNEL_%d:END_IDX'%(i),'type':'numeric', 'value':1000},
            {'path':'.CHANNEL_%d:DATA'%(i),'type':'signal', 'options':('no_write_model','compress_on_put')},
        ])
    parts.extend([
