...
class NI6683REC(Device):
    ...
    parts = [
        ...
        {'path': ':TAIUTC_DELAY',   'type': 'numeric',
            'valueExpr': 'Data.compile("37000000000Q")'},
    ...
    NISYNC_READ_NONBLOCKING = c_int(1)
...
# Non-board terminals
    ...
    DevTypeDict = {'UNKNOWN': -1, 'PXI6682': 0,
                   'PCI1588': 1, 'PXI6683': 2, 'PXI6683H': 3}
    trigTermDict = {'PFI0': 0, 'PFI1': 1, 'PFI2': 2, 'TRIG0': 11, 'TRIG1': 12,
                    'TRIG2': 13, 'TRIG3': 14, 'TRIG4': 15, 'TRIG5': 16, 'TRIG6': 17, 'TRIG7': 18}
...
# National Instruments Sync Device Manager
    ...
    def initializeInfo(self):
        ...
        try:
            ...
            self.devFd.append(NI6683REC.NiSyncLib.nisync_open_terminal(
                devType, devNum, trigTerm, self.NISYNC_READ_NONBLOCKING))
            ...
            if self.devFd < 0:
                ...
                ), 'Cannot open terminal %s NiSync Timing Device' % (trigTermName))
                raise mdsExceptions.TclFAILED_ESSENTIAL

            for tr in range(8):
                if getattr(self, 'trig_%d' % (tr+1)).isOn():
                    try:
                        trigTermName = getattr(
                            self, 'trig_%d_term' % (tr+1)).data()
                        trigTerm = c_int(self.trigTermDict[trigTermName])
                    except:
                        Data.execute('DevLogErr($1,$2)', self.getNid(
                        ), 'Invalid terminal for trigger %s ' % (tr+1))
                        continue
                    ...
                    print ("devFd", trigTermName,
                           self.devFd[len(self.devFd)-1])
                else:
                    self.devFd.append(-1)
            ...
            print (str(e))
            Data.execute('DevLogErr($1,$2)', self.getNid(
            ), 'Exception on NiSync Timing Device on open terminal %s ' % (trigTermName))
            raise mdsExceptions.TclFAILED_ESSENTIAL
            ...
            while not self.stopReq:
                ...
                for fd1, event in poolfd:
                    ...
                    for i in range(count):
                        print (1000000000. /
                               (ts[i].nanos-ts_nanos_prev), ts[i].nanos, count)
                        dt = datetime.fromtimestamp(ts[i].nanos // 1000000000)
                        dt_str = dt.strftime('%Y-%m-%d %H:%M:%S') + "." + str(
                            int(((ts[i].nanos-tai_utc_delay) % 1000000000))).zfill(9)
                        print (dt_str)
...
# Configuration check
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(),
                         'Trigger decimation count Undefined')
            raise mdsExceptions.TclFAILED_ESSENTIAL

        try:
            trigEdge = c_int(
                self.trigEdgeDict[self.pulse_time_trig_edge.data()])
        except:
            Data.execute('DevLogErr($1,$2)', self.getNid(),
                         'Pulse time Trigger edge Undefined')
            raise mdsExceptions.TclFAILED_ESSENTIAL
...
# Open device
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(),
                         'Cannot open NiSync IEEE1588 device')
            raise mdsExceptions.TclFAILED_ESSENTIAL

        self.saveInfo()

        status = NI6683REC.NiSyncLib.nisync_enable_timestamp_trigger(
            self.devFd[0], trigEdge, trigDecCnt)
        ...
        if status < 0:
            self.closeInfo()
            Data.execute('DevLogErr($1,$2)', self.getNid(),
                         'Cannot enable pulse time timestamp trigger')
            raise mdsExceptions.TclFAILED_ESSENTIAL

        for tr in range(8):
            if getattr(self, 'trig_%d' % (tr+1)).isOn():
                try:
                    trigEdgeName = getattr(
                        self, 'trig_%d_edge' % (tr+1)).data()
                    trigEdge = c_int(self.trigEdgeDict[trigEdgeName])
                except:
                    Data.execute('DevLogErr($1,$2)', self.getNid(),
                                 'Invalid trigger edge for trigger %s ' % (tr+1))
                    continue

                status = NI6683REC.NiSyncLib.nisync_enable_timestamp_trigger(
                    self.devFd[tr+1], trigEdge, trigDecCnt)
                if status < 0:
                    Data.execute('DevLogErr($1,$2)', self.getNid(
                    ), 'Cannot enable pulse time timestamp for trigger %d' % (tr+1))
                    continue
    ...
    def start(self):
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(),
                         'SPIDER timing device device not initialized')
            raise mdsExceptions.TclFAILED_ESSENTIAL
...
# Module in acquisition check
        ...
        if not self.worker.isAlive():
            Data.execute('DevLogErr($1,$2)', self.getNid(),
                         'SPIDER timing device device not armed')
            raise mdsExceptions.TclFAILED_ESSENTIAL
    ...
    def stop(self):
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(),
                         'SPIDER timing device not initialized')
            raise mdsExceptions.TclFAILED_ESSENTIAL
