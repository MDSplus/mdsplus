    ...
    class AsynchStore(Thread):
        ...
        def run(self):
            ...
            status = BASLERACA.baslerLib.startFramesAcquisition(
                self.device.handle)
            if status < 0:
                BASLERACA.baslerLib.getLastError(
...
###restore info###
    def restoreInfo(self):
        ...
        if self.nid in BASLERACA.handles.keys():
            ...
            self.handle = c_int(-1)
            status = BASLERACA.baslerLib.baslerOpen(
...
# Gain Auto
        try:
            ...
            raise mdsExceptions.TclFAILED_ESSENTIAL
        status = BASLERACA.baslerLib.setGainAuto(
            self.handle, c_char_p(gainAuto))
        if status < 0:
            BASLERACA.baslerLib.getLastError(self.handle, self.error)
            Data.execute('DevLogErr($1,$2)', self.nid,
...
# Gamma Enable
        try:
            ...
            raise mdsExceptions.TclFAILED_ESSENTIAL
        status = BASLERACA.baslerLib.setGammaEnable(
...
# Pixel Format
        try:
            ...
            raise mdsExceptions.TclFAILED_ESSENTIAL
        status = BASLERACA.baslerLib.setPixelFormat(
...
# Frame Rate
        ...
        try:
            ...
            raise mdsExceptions.TclFAILED_ESSENTIAL
        status = BASLERACA.baslerLib.setFrameRate(
...
# Streaming
        ...
        if streamingEnabled:
            ...
            status = BASLERACA.baslerLib.setStreamingMode(self.handle, streamingEnabled,  autoAdjustLimit, c_char_p(
