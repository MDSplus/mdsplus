    ...
    class AsynchStore(Thread):
        ...
        def run(self):
            ...
            while not self.stopped:
                status = self.deviceLib.acquireChunk(c_char_p(self.addr), byref(rawChan), c_int(self.NUM_CHUNK_SAMPLES))
                if status == -1:
                    print ('Acquisition Failed')
                    return
                dataArr = Int16Array(rawChan)
                startTime = Float64(self.currTime)
                endTime = Float64(self.currTime + self.period * self.NUM_CHUNK_SAMPLES)
