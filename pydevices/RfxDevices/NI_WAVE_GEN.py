...
class NI_WAVE_GEN(Device):
    ...
    parts.append({'path':':CMD_EVENT',    'type':'text',    'value':'WAVE_UPDATE'})
	    ...
	    cmdEventName = self.device.cmd_event.data();
		   ...
		   if self.device.restoreInfo(True) != NI_WAVE_GEN.NO_ERROR :
		       ...
		       raise mdsExceptions.DevINV_SETUP
                   ...
                   self.device.__aoConfiguration__(aoConfig, self.device.AO_DAC_POLARITY_BIPOLAR, wavePoint, periodDivisor)

                   for ch in range(4):
                      self.device.configChannel(boardType, boardId, ch, False, wavePoint, chMaxAr[ch], chMinAr[ch], chTypeAr[ch].strip(), chAOXAr[ch], chAOXAr[ch], numPeriod[ch])
...
#############End Inner class AsynchStore
    ...
    def __checkChanneParam__(self, ch, gain):
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Invalid channel %d max value : %s'%(ch, str(ex)))
            raise mdsExceptions.DevBAD_PARAMETER
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Invalid channel %d min value : %s'%(ch, str(ex)))
            raise mdsExceptions.DevBAD_PARAMETER
        ...
        try:
            ...
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Invalid channel %d wave type : %s'%(ch, str(ex)))
            raise mdsExceptions.DevBAD_PARAMETER

        try:
            chFreq = getattr(self, 'ao_%d_freq'%(ch+1)).data()
        except Exception as ex:
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Invalid channel %d waveform frequency : %s'%(ch, str(ex)))
            raise mdsExceptions.DevBAD_FREQ
        if chFreq < (self.MIN_FREQ - 1e-6)  or chFreq > (self.MAX_FREQ + 1e-4) :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Invalid channel %d waveform frequency %f out of limit (%0.2f:%0.2f)'%(ch, chFreq, self.MIN_FREQ, self.MAX_FREQ))
            raise mdsExceptions.DevBAD_FREQ
    ...
    def configChannels(self, reset):
        ...
        for ch in range(4) :
            ...
            self.configChannel(boardType, boardId, ch, reset, wavePoint, self.chMax, self.chMin, self.chType, self.chAOX, self.chAOY, numPeriod[ch])
    ...
    def __aoConfiguration__(self, aoConfig, polarity, wavePoint, periodDivisor):
        ...
        for ch in range(4) :
            ...
            if NI_WAVE_GEN.niLib6259.pxi6259_add_ao_channel(aoConfig, c_uint(ch), polarity) :
                Data.execute('DevLogErr($1,$2)', self.getNid(), "Failed to configure channel! polarity");
                raise mdsExceptions.DevCANNOT_LOAD_SETTINGS
        ...
        for ch in range(4) :
            ...
            if getattr(self, 'ao_%d'%(ch+1)).isOn():
                ...
                if fr < (self.MIN_FREQ - 1e-6)  or fr > (self.MAX_FREQ + 1e-4) :
                    Data.execute('DevLogErr($1,$2)', self.getNid(), 'Invalid channel %d waveform frequency %f out of limit (%0.2f:%0.2f)'%(ch, fr, self.MIN_FREQ, self.MAX_FREQ))
                    raise mdsExceptions.DevBAD_FREQ
...
##########Stop generation
    ...
    def stop_gen(self):
        ...
        if self.restoreInfo(False) != NI_WAVE_GEN.HANDLE_FOUND :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Wavefor generation not initialized')
            raise mdsExceptions.DevINV_SETUP

        self.closeInfo()

        if self.closeUpdateEvent() == NI_WAVE_GEN.ERROR :
            Data.execute('DevLogErr($1,$2)', self.getNid(), 'Wavefor generation not started')
            raise mdsExceptions.DevINV_SETUP
