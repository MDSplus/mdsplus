...
class PLFE(Device):
     ...
     parts = [
          {'path': ':COMMENT', 'type': 'text', 'value': 'Plasma light front end control'},
     ...
     for i in range(6):
          ...
          parts.append({'path': '.BOARD_%02d.CONFIG'%(i + 1), 'type': 'structure'})
          parts.append({'path': '.BOARD_%02d.CONFIG.WR'%(i + 1), 'type': 'structure'})
          parts.append({'path': '.BOARD_%02d.CONFIG.WR:STATUS'%(i + 1), 'type': 'numeric', 'value': 0})
          parts.append({'path': '.BOARD_%02d.CONFIG.WR:CONTROL'%(i + 1), 'type': 'numeric', 'value': 0})
          parts.append({'path': '.BOARD_%02d.CONFIG.RD'%(i + 1), 'type': 'structure'})
          parts.append({'path': '.BOARD_%02d.CONFIG.RD:IDX'%(i + 1), 'type': 'numeric', 'value': -1})
          parts.append({'path': '.BOARD_%02d.CONFIG.RD:STATUS'%(i + 1), 'type': 'numeric', 'value': -1})
          parts.append({'path': '.BOARD_%02d.CONFIG.RD:STATUS_TXT'%(i + 1), 'type': 'text', 'value': 'UNDEF'})
          parts.append({'path': '.BOARD_%02d.CONFIG.RD:CONTROL'%(i + 1), 'type': 'numeric', 'value': -1})
          parts.append({'path': '.BOARD_%02d.CONFIG.RD:CONTROL_TXT'%(i + 1), 'type': 'text', 'value': 'UNDEF'})

          parts.append({'path': '.BOARD_%02d.SETUP'%(i + 1), 'type': 'structure'})

          for j in range(2):

               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d'%(i + 1, j + 1), 'type': 'structure'})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.WR'%(i + 1, j + 1), 'type': 'structure'})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.WR:GAIN'%(i + 1, j + 1), 'type': 'numeric', 'value': 1})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.WR:FILTER'%(i + 1, j + 1), 'type': 'numeric', 'value': 0})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.WR:TRANS'%(i + 1, j + 1), 'type': 'numeric', 'value': 0})

               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.RD'%(i + 1, j + 1), 'type': 'structure'})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.RD:GAIN'%(i + 1, j + 1), 'type': 'numeric', 'value': 1E10})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.RD:FILTER'%(i + 1, j + 1), 'type': 'numeric', 'value': -1})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.RD:FILTER_TXT'%(i + 1, j + 1), 'type': 'text', 'value': 'UNDEF'})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.RD:TRANS'%(i + 1, j + 1), 'type': 'numeric', 'value': -1})
               parts.append({'path': '.BOARD_%02d.SETUP.CHANNEL_%02d.RD:TRANS_TXT'%(i + 1, j + 1), 'type': 'text', 'value': 'UNDEF'})
     ...
     def INIT(self):
          ...
          for i in range(6):
               ...
               self.__getattr__('.BOARD_%02d.CONFIG.RD:STATUS_TXT'%(i + 1)).putData(status_dict[data[i]['status']])
               if data[i]['status']:
                    self.__getattr__('.BOARD_%02d.CONFIG.RD:CONTROL'%(i + 1)).putData(-1)
          ...
          for i in range(6):
               ...
               if data[i]['status']: # anche se locale devo farlo
                    ...
                    w = w + (self.__getattr__('.BOARD_%02d.SETUP.CHANNEL_01.WR:FILTER'%(i + 1)).data() << 4)
          ...
          for i in range(6):
               ...
               self.__getattr__('.BOARD_%02d.SETUP.CHANNEL_01.RD:TRANS'%(i + 1)).putData(int(data2[i]['bits'][2]))
               self.__getattr__('.BOARD_%02d.SETUP.CHANNEL_01.RD:TRANS_TXT'%(i + 1)).putData(trans_dict[int(data2[i]['bits'][2])])
               self.__getattr__('.BOARD_%02d.SETUP.CHANNEL_02.RD:FILTER'%(i + 1)).putData(int(data2[i]['bits'][1]))
