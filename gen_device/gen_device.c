...
int main(int argc, char **argv)
{
  ...
  int treeopen = 0;
  file_name = argv[1];
  if ((descr_file = fopen(file_name, "r")) == 0)
  ...
  for (i = 0, next_arg = 2; i < strlen(options); i++) {
    ...
    case 'x':{
	next_arg++;
	suffix = next_arg < argc ? argv[next_arg] : "";
	break;
      }
    case 'a':
      add_qual = 1;
  ...
  if (!treeopen) {
    ...
    if (path[0])
      fprintf(stderr, "\t$gen_device_path --- (%s)\n", path[0]);
    if (path[1])
...
void DevGenAdd(char *device, char *library, NodeDescr * bottom)
{
  ...
  for (i = 0; i < strlen(device); i++)
    lower_device_name[i] = __TOlower(device[i]);
  lower_device_name[i] = 0;
  upper_device_name = malloc(strlen(device) + 1);
  for (i = 0; i < strlen(device); i++)
    upper_device_name[i] = __TOupper(device[i]);
  ...
  for (; curr_node; curr_node = curr_node->nxt) {
    ...
    case UND:
      fprintf(out_file, "\tADD_NODE(%s, %s)\n", curr_node->name, curr_node->usage);
      break;
    case INT:
      fprintf(out_file, "\tADD_NODE_INTEGER(%s, %s, %s)\n", curr_node->name,
	      curr_node->value, curr_node->usage);
      break;
    case FLO:
      fprintf(out_file, "\tADD_NODE_FLOAT(%s, %s, %s)\n", curr_node->name,
	      curr_node->value, curr_node->usage);
      break;
    case STR:
      fprintf(out_file, "\tADD_NODE_STRING(%s, %s, %s)\n", curr_node->name,
	      curr_node->value, curr_node->usage);
      break;
    case EXP:
      fprintf(out_file, "#define expr \"%s\"\n", curr_node->value);
      fprintf(out_file, "\tADD_NODE_EXPR(%s, %s)\n", curr_node->name, curr_node->usage);
      fprintf(out_file, "#undef expr\n");
      break;
    case ACT:
      if (curr_node->completion)
	fprintf(out_file, "\tADD_NODE_ACTION(%s, %s, %s, %s, \"%s\", %s, %s, %s)\n",
		curr_node->name, curr_node->method, curr_node->phase, curr_node->sequence,
		curr_node->completion, curr_node->timout, curr_node->server, curr_node->usage);
      else
	fprintf(out_file, "\tADD_NODE_ACTION(%s, %s, %s, %s, 0, %s, %s, %s)\n", curr_node->name,
		curr_node->method, curr_node->phase, curr_node->sequence, curr_node->timout,
		curr_node->server, curr_node->usage);
...
void DevGenH(char *device, char *library, NodeDescr * bottom)
{
  ...
  char *file_name = malloc(strlen(lower_device_name) + 7), *curr_char, *routine;
...
void DevGenPartName(char *device, char *library, NodeDescr * bottom)
{
  ...
  static char *test_status = { "\tif (!(status & 1)) return status;" }, *part_name = {
  "_part_name.c"};
...
NodeDescr *DevNewNode(char *level_str, char *name, FlagsDescr * flags)
{
  ...
  if ((level > prev_level + 1) || (level < 1))
    Error("Error in node levels");
...
ActionDescr *DevNewAction(char *method, char *timout, char *sequence, char *phase, char *server,
			  char *completion)
{
...
void DevGenOperation(char *operation, InDescr * input_list, int is_operation)
{
  ...
  char *underscore = "_", *dot_c =
      ".c", low_path[512], *error_messages[512], abc[512], upper_path_name[32];
  char *out_file_name = malloc(strlen(operation) + strlen(lower_device_name) + 9);
  char *curr_char, *low_case_operation = malloc(strlen(operation) + 1), *mixed_case_operation =
      malloc(strlen(operation) + 1);
  char *routine;
  ...
  for (curr_in = input_list, xd_count = 0; curr_in->prv; curr_in = curr_in->prv)
    if (!curr_in->type || (curr_in->type == STR) || (curr_in->type == INT_ARRAY)
	|| (curr_in->type == FLO_ARRAY))
      xd_count++;
  ...
  for (; curr_in; curr_in = curr_in->nxt) {
	...
	fprintf(out_file, "\tcheck_integer_conv_set(%s, %s_convert, %s_t, %d, %s);\n",
		curr_in->path_name, curr_in->path_name, curr_in->path_name, set_count,
		curr_in->error_code);
	break;
      };
      break;
  ...
  for (; curr_in; curr_in = curr_in->nxt) {
    ...
    case FLO_ARRAY:
      ...
      break;
    case STR:
      fprintf(file_h, "\tchar *%s;\n", curr_in->path_name);
      break;
    case NONE:
      fprintf(file_h, "\tstruct descriptor *%s;\n", curr_in->path_name);
      break;
    }
