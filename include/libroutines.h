...
extern int LibFindFile(const mdsdsc_t *const filespec, mdsdsc_t *const result, void **const ctx);
extern int LibFindFileCaseBlind(const mdsdsc_t *const filespec, mdsdsc_t *const result, void **const ctx);
extern int LibFindFileEnd(void **const ctx);
extern int LibFindFileRecurseCaseBlind(const mdsdsc_t *const filespec, mdsdsc_t *const result, void **const ctx);
...
extern int Libgetvm (const uint32_t *const len, void **const vm, ZoneList **const zone);
extern int LibInsertTree(LibTreeNode **const treehead, void *const symbol_ptr, int *const control_flags,
...
extern int LibSpawn(const mdsdsc_t *const cmd, const int waitFlag, const int notifyFlag);
...
extern int LibSysAscTim(length_t *const len, struct descriptor *const str, const int *const time_in);
extern int LibTimeToVMSTime(const time_t *const time_in, int64_t *const time_out);
extern int LibTraverseTree(LibTreeNode **treehead, int (*user_rtn)(), void *user_data);
extern int LibWait(const float *const secs);
extern int libffs(const int *const position, const int *const size, const char *const base, int *const find_position);
