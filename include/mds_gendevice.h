...
#define DEV$_BADPARAM (MSG_MASK + 2015 * MSG_FACNUM_M + 32 * MSG_MSGNUM_M + MSG_ERROR)
#define GEN_DEV$_NODATA (MSG_MASK + 2037 * MSG_FACNUM_M + 1 * MSG_MSGNUM_M + MSG_WARNING)
#define GEN_DEV$_INV_SETUP (MSG_MASK + 2037 * MSG_FACNUM_M + 2 * MSG_MSGNUM_M + MSG_WARNING)
#define GEN_DEV$_X_IO_ERR (MSG_MASK + 2037 * MSG_FACNUM_M + 3 * MSG_MSGNUM_M + MSG_WARNING)
...
#define ADD_NODE_EXPR(name, usage)\
	...
	if(!curr_usage) { switch(comp_expr_xd.pointer->dtype)  {\
	    ...
	    case DTYPE_ACTION : curr_usage = TreeUSAGE_ACTION; break;\
	    case DTYPE_CONGLOM : curr_usage = TreeUSAGE_DEVICE; break;\
	    case DTYPE_PROGRAM :\
	    case DTYPE_METHOD :\
	    case DTYPE_ROUTINE : curr_usage = TreeUSAGE_TASK; break;\
	    case DTYPE_SIGNAL : curr_usage = TreeUSAGE_SIGNAL; break;\
	    case DTYPE_RANGE : curr_usage = TreeUSAGE_AXIS; break;\
	    case DTYPE_WINDOW : curr_usage = TreeUSAGE_WINDOW; break;\
	    case DTYPE_T : curr_usage = TreeUSAGE_TEXT; break;\
	    default : curr_usage = TreeUSAGE_NUMERIC; }}\
...
#define read_descriptor(pos, field)\
    ...
    if(status & 1) in_struct.field = xd.pointer;\
    next_xd
#define read_descriptor_error(pos, field, err)\
    curr_nid = head_nid + pos;\
    status = TreeGetRecord(curr_nid, &xd);\
    if(!(status & 1)) error(head_nid, GEN_DEV$_INV_SETUP, err)\
    in_struct.field = xd.pointer;\
    next_xd
#define read_integer(pos, field)\
    curr_nid = head_nid + pos;\
    status = TdiData(&curr_nid_d, &curr_int_d MDS_END_ARG);\
    if(status & 1) in_struct.field = curr_int;
#define read_integer_error(pos, field, err)\
    curr_nid = head_nid + pos;\
    status = TdiData(&curr_nid_d, &curr_int_d MDS_END_ARG);\
    if(!(status & 1)) error(head_nid, GEN_DEV$_INV_SETUP, err) else\
    in_struct.field = curr_int;
#define check_range(field, min, max, err)\
    if((in_struct.field < min)||(in_struct.field > max)) error(head_nid, GEN_DEV$_INV_SETUP, err);
...
#define read_integer_array_error(pos, field, field_count, err)\
    ...
    if(array_d_ptr->class != CLASS_A) error(head_nid, GEN_DEV$_INV_SETUP, err)\
    else { in_struct.field = (int *)array_d_ptr->pointer;\
...
#define read_float_array_error(pos, field, field_count, err)\
    ...
    array_d_ptr = (struct descriptor_a *)xd.pointer;\
    if(array_d_ptr->class != CLASS_A) error(head_nid, GEN_DEV$_INV_SETUP, err)\
    else { in_struct.field = (float *)array_d_ptr->pointer;\
    in_struct.field_count = array_d_ptr->arsize/array_d_ptr->length; }}\
    next_xd
...
#define read_float(pos, field)\
    ...
    if(status & 1) in_struct.field = curr_float;
...
#define read_string(pos, field)\
    ...
    if(status & 1) in_struct.field = xd.pointer->pointer;\
    next_xd
...
#define build_results_with_xd_and_return(num_xd)\
    in_struct.__xds = (struct descriptor_xd *)malloc(num_xd*sizeof(struct descriptor_xd));\
    in_struct.__num_xds = num_xd;\
    memcpy((char *)in_struct.__xds, (char *)work_xd, num_xd * sizeof(struct descriptor_xd));\
    *in_ptr = in_struct;\
    return error_code
