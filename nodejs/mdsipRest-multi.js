// Restful interface to mdsip + UDP events
// This node.js interfaces provides a HTTP interface listening on port 8081 for remote expression evaluation, i.e. thin client. The mdsip protocol
// is handled directly by the JavaScrip code.
// It serves multiple clients possibly connected to different mdsip servers
// In addition, it provides WebSocket interfaces for receiving MDSplus events (UDP only). Registration to the event is performed 
// via the rest interface, and then ipon event reception, the string <event_name>:<event data> is sent via WebSocket to the connected 
// listeners. Only string raw data associated with evente is currently supported

// URI to call the code:
//
// 1 - http://<host>:8081/connect?ip=<ipAddress[:port]>  
...
// to register for event reception (on all clients connected via WebSocket).
//
