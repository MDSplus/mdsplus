...
static void ResetWave(Widget w)
{
       ...
       (TdiCvt(&times_xd,&float_d,&times_xd MDS_END_ARG) & 1) )
  {
    ...
    if (count >= 2)
    {
      ...
      Widget first_dw = XtNameToWidget(Top(w),d_name(0));
      XtVaGetValues(first_dw, XmNuserData, &saved_times, NULL);
      if (saved_times) XtFree((XtPointer)saved_times);
      XtVaSetValues(first_dw, XmNuserData, memcpy(XtMalloc(sizeof(float)*count),time,sizeof(float)*count), NULL);
      for (i=0;i<4;i++)
      {
        ...
        if ((valid_xd = (struct descriptor_xd *)XmdsXdBoxGetXd(xdbw)) && (TdiData(valid_xd->pointer, &knot_y_xd MDS_END_ARG) & 1) && 
        ...
        if (num_knots && valid_xd && (TdiDimOf(valid_xd->pointer,&knot_x_xd MDS_END_ARG) &1) && 
            (TdiData(&knot_x_xd,&knot_x_xd MDS_END_ARG) & 1) && (TdiCvt(&knot_x_xd,&float_d,&knot_x_xd MDS_END_ARG) & 1))
        {
      ...
      XmdsComplain(w,"Number of samples and/or Trigger and clock information have specified less than 2 samples");
  }
  else
...
static void SetPointerMode(Widget w, int *mode, XmToggleButtonCallbackStruct *cb)
{
  ...
  switch (*mode)
  {
    case XmdsPOINTER_MODE_POINT: help = "Button 1: Position crosshairs"; break;
    case XmdsPOINTER_MODE_ZOOM: help = "Button 1: Zoom in 2x, Button 1 drag: Zoom in box, Button 2: Zoom out"; break;
    case XmdsPOINTER_MODE_DRAG: help = "Button 1: Drag curves (change axis, not curve values)"; break;
    case XmdsPOINTER_MODE_ADD_DELETE: help = 
       "Button 1: Move knot (modifiers: shift=lock X, alt=lock Y, shift-lock=high res), Button 2: Add point (shift=Remove point)";
       break;
    case XmdsPOINTER_MODE_SLIDE_STRETCH: help =
...
static void AddPoint(Widget w, int tag, XmdsWavedrawValueCBStruct *v)
{
  ...
  for (i=0;i<num_knots-1;i++)
    if (v->x[i] >= v->x[i+1]) v->x[i+1] = v->x[i] + (v->x[num_knots-1] - v->x[0])/1E6;
...
static void Move(Widget w, int tag, XmdsWavedrawValueCBStruct *v)
{
  ...
  if (v->reason == XmdsCRMovePointEnd)
  {
    int i;
    for (i=0;i<4;i++)
      if (XtNameToWidget(Top(w),d_name(i)) == w) break;
...
static void SetPoint(Widget w)
{
  ...
  if (channel >= 0 && channel < 4 && num == 2)
  {
    ...
    XtVaGetValues(dw,XmdsNcount,&count,XmdsNxValue,&xptr,XmdsNyValue,&yptr,XmdsNselections,&selected,NULL);
...
static void FitChange(Widget w, int tag, XmRowColumnCallbackStruct *cb)
{
  ...
  for (i=0;i<4;i++)
  {
    ...
    if (submenu == w)
    {
      ...
      Boolean *pendown;
      Widget wave = XtNameToWidget(Top(w),d_name(i));
      XtVaGetValues(wave,XmdsNxValue,&x,XmdsNyValue,&y,XmdsNselections,&selections,XmdsNpenDown,&pendown,XmdsNcount,&count,NULL);
...
static void Spline(int count, float *x, float *y, Boolean *selected, Boolean *pen_down)
{
  ...
  for (i=0;i<num_v;i++) pen_down[num_knots+i] = 1;
}
...
static void Linear(int count, float *knots_x, float *knots_y, Boolean *selected, Boolean *pen_down)
{
  int num_knots = NumKnots(count, selected);
  ...
  for (i=0;i<num_v;i++) pen_down[num_knots+i] = 1;
  for (i=num_v+num_knots-1;i<count;i++) pen_down[i] = 0;
}
...
static void AutoscaleAll(Widget w)
{
  int i;
  for (i=0;i<4;i++)
    XtVaSetValues(XtNameToWidget(Top(w),d_name(i)), XmdsNxMin, 0, XmdsNxMax, 0, XmdsNyMin, 0, XmdsNyMax, 0, NULL);
}
...
static void InitWave(Widget w, int *idx)
{
  ...
  Widget wave = XtNameToWidget(Top(w),d_name(*idx - 1));
  XtVaGetValues(wave, XmdsNcount, &count, XmdsNxValue, &x, XmdsNyValue, &y, XmdsNselections, &selected, XmdsNpenDown, &pendown, 
               NULL);
...
int b5910a___init(struct descriptor_s *niddsc_ptr, InInitStruct *setup)
{
  ...
  int status;
  int chan;
  struct b5910_setup  { unsigned noc    : 3;
	    	        unsigned range  : 2;
	    	        unsigned status : 3;
	    	        unsigned freq   : 3;
	    	        unsigned clock  : 1;
  ...
  {
    int i;
    static float dts[] = {1./2000,1./5000,1./10000,1./20000,1./50000,1./100000,1./200000,1./500000};
  ...
  for (chan = 0; chan < reg.noc; chan++)
  {
    ...
    switch (chan)
    {
      case 0: data = setup->channel_1; break;
      case 1: data = setup->channel_2; break;
      case 2: data = setup->channel_3; break;
      case 3: data = setup->channel_4; break;
    }
    ...
    if (status & 1)
    {
      ...
      switch (range)
      {
        case 0: volts_per_bit = 5.00E-3; max_counts = 2047; min_counts = -2048; break;
        case 1: volts_per_bit = 2.50E-3; max_counts = 2047; min_counts = -2048; break;
        case 2: volts_per_bit = 2.50E-3; max_counts = 4095; min_counts =     0; break;
        case 3: volts_per_bit = 1.25E-3; max_counts = 4095; min_counts =     0; break;
      }
