...
#include "t4012_gen.h"
int t4012__add(struct descriptor *name_d_ptr, struct descriptor *dummy_d_ptr, int *nid_ptr)
{
...
int t4012__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(T4012_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(T4012_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(T4012_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(T4012_N_CHANNELS + 1) : COPY_PART_NAME(:CHANNELS) break;
		case(T4012_N_POST_SAMPLES + 1) : COPY_PART_NAME(:POST_SAMPLES) break;
		case(T4012_N_SMP_PER_CHAN + 1) : COPY_PART_NAME(:SMP_PER_CHAN) break;
		case(T4012_N_DISPLAY_CHAN + 1) : COPY_PART_NAME(:DISPLAY_CHAN) break;
		case(T4012_N_TRIGGER + 1) : COPY_PART_NAME(:TRIGGER) break;
		case(T4012_N_FREQUENCY_1 + 1) : COPY_PART_NAME(:FREQUENCY_1) break;
		case(T4012_N_FREQUENCY_2 + 1) : COPY_PART_NAME(:FREQUENCY_2) break;
		case(T4012_N_SWITCH_MODE + 1) : COPY_PART_NAME(:SWITCH_MODE) break;
		case(T4012_N_SWITCH_TRIG + 1) : COPY_PART_NAME(:SWITCH_TRIG) break;
		case(T4012_N_EXTERN_CLOCK + 1) : COPY_PART_NAME(:EXTERN_CLOCK) break;
		case(T4012_N_CLOCK_OUT + 1) : COPY_PART_NAME(:CLOCK_OUT) break;
		case(T4012_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(T4012_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
...
int t4012__init(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
...
int t4012__trigger(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
...
int t4012__store(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
