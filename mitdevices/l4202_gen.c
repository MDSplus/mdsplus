...
#include "l4202_gen.h"
int l4202__add(struct descriptor *name_d_ptr, struct descriptor *dummy_d_ptr, int *nid_ptr)
{
...
int l4202__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(L4202_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(L4202_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(L4202_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(L4202_N_MEMORY_NAME + 1) : COPY_PART_NAME(:MEMORY_NAME) break;
		case(L4202_N_EXT_CLOCK + 1) : COPY_PART_NAME(:EXT_CLOCK) break;
		case(L4202_N_NUM_SPECTRA + 1) : COPY_PART_NAME(:NUM_SPECTRA) break;
		case(L4202_N_ROUTER_INC + 1) : COPY_PART_NAME(:ROUTER_INC) break;
		case(L4202_N_OFFSET + 1) : COPY_PART_NAME(:OFFSET) break;
		case(L4202_N_RESOLUTION + 1) : COPY_PART_NAME(:RESOLUTION) break;
		case(L4202_N_LOOP_INHIBIT + 1) : COPY_PART_NAME(:LOOP_INHIBIT) break;
		case(L4202_N_SPECTRA + 1) : COPY_PART_NAME(:SPECTRA) break;
		case(L4202_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(L4202_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
...
int l4202__init(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
...
int l4202__store(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
