	       ...
	       int height_in, int xoffset, int yoffset, int debug)
{
  ...
  if (debug > 0)
   ...
   /* set the shutter */
  if (dc1394_feature_set_mode(camera, DC1394_FEATURE_SHUTTER, (shutter==0)?DC1394_FEATURE_MODE_AUTO : DC1394_FEATURE_MODE_MANUAL) != DC1394_SUCCESS) {
  ...
  if (dc1394_get_image_size_from_video_mode(camera, mode, &width, &height) != DC1394_SUCCESS)
   ...
   bytes = (bits_per_pixel <=8) ? 1 : ((bits_per_pixel <= 16) ? 2 : ((bits_per_pixel <= 24) ? 3 : 4));
   ...
   if(debug) printf("Starting thread to take the frames\n");
