...
int mit_clock__add(struct descriptor *name_d_ptr, struct descriptor *dummy_d_ptr, int *nid_ptr)
{
...
int mit_clock__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(MIT_CLOCK_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(MIT_CLOCK_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(MIT_CLOCK_N_FREQUENCY + 1) : COPY_PART_NAME(:FREQUENCY) break;
		case(MIT_CLOCK_N_OUTPUT + 1) : COPY_PART_NAME(:OUTPUT) break;
