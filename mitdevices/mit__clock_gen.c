...
int mit__clock__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(MIT__CLOCK_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(MIT__CLOCK_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(MIT__CLOCK_N_FREQUENCY + 1) : COPY_PART_NAME(:FREQUENCY) break;
		case(MIT__CLOCK_N_DUTY_CYCLE + 1) : COPY_PART_NAME(:DUTY_CYCLE) break;
		case(MIT__CLOCK_N_TSTART + 1) : COPY_PART_NAME(:TSTART) break;
		case(MIT__CLOCK_N_START_LOW + 1) : COPY_PART_NAME(:START_LOW) break;
		case(MIT__CLOCK_N_EDGES_R + 1) : COPY_PART_NAME(:EDGES_R) break;
		case(MIT__CLOCK_N_EDGES_F + 1) : COPY_PART_NAME(:EDGES_F) break;
...
int mit__clock___get_setup(struct descriptor *nid_d_ptr, InGet_setupStruct *in_ptr)
{
