...
int j1819__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(J1819_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(J1819_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(J1819_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(J1819_N_EXT_CLOCK_IN + 1) : COPY_PART_NAME(:EXT_CLOCK_IN) break;
		case(J1819_N_STOP_TRIG + 1) : COPY_PART_NAME(:STOP_TRIG) break;
		case(J1819_N_SAMPLES + 1) : COPY_PART_NAME(:SAMPLES) break;
		case(J1819_N_INPUT + 1) : COPY_PART_NAME(:INPUT) break;
		case(J1819_N_TRUE_PHASE + 1) : COPY_PART_NAME(:TRUE_PHASE) break;
		case(J1819_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(J1819_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
...
int j1819__init(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
...
int j1819__store(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
...
int j1819__trigger(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
