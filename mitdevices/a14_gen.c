...
int a14__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(A14_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(A14_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(A14_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(A14_N_CLOCK_DIVIDE + 1) : COPY_PART_NAME(:CLOCK_DIVIDE) break;
		case(A14_N_EXT_CLOCK_IN + 1) : COPY_PART_NAME(:EXT_CLOCK_IN) break;
		case(A14_N_STOP_TRIG + 1) : COPY_PART_NAME(:STOP_TRIG) break;
		case(A14_N_PTS + 1) : COPY_PART_NAME(:PTS) break;
		case(A14_N_MODE + 1) : COPY_PART_NAME(:MODE) break;
		case(A14_N_DIMENSION + 1) : COPY_PART_NAME(:DIMENSION) break;
		case(A14_N_CLK_POLARITY + 1) : COPY_PART_NAME(:CLK_POLARITY) break;
		case(A14_N_STR_POLARITY + 1) : COPY_PART_NAME(:STR_POLARITY) break;
		case(A14_N_STP_POLARITY + 1) : COPY_PART_NAME(:STP_POLARITY) break;
		case(A14_N_GATED + 1) : COPY_PART_NAME(:GATED) break;
		case(A14_N_INPUT_1 + 1) : COPY_PART_NAME(:INPUT_1) break;
		case(A14_N_INPUT_1_STARTIDX + 1) : COPY_PART_NAME(:INPUT_1:STARTIDX) break;
		case(A14_N_INPUT_1_ENDIDX + 1) : COPY_PART_NAME(:INPUT_1:ENDIDX) break;
		case(A14_N_INPUT_2 + 1) : COPY_PART_NAME(:INPUT_2) break;
		case(A14_N_INPUT_2_STARTIDX + 1) : COPY_PART_NAME(:INPUT_2:STARTIDX) break;
		case(A14_N_INPUT_2_ENDIDX + 1) : COPY_PART_NAME(:INPUT_2:ENDIDX) break;
		case(A14_N_INPUT_3 + 1) : COPY_PART_NAME(:INPUT_3) break;
		case(A14_N_INPUT_3_STARTIDX + 1) : COPY_PART_NAME(:INPUT_3:STARTIDX) break;
		case(A14_N_INPUT_3_ENDIDX + 1) : COPY_PART_NAME(:INPUT_3:ENDIDX) break;
		case(A14_N_INPUT_4 + 1) : COPY_PART_NAME(:INPUT_4) break;
		case(A14_N_INPUT_4_STARTIDX + 1) : COPY_PART_NAME(:INPUT_4:STARTIDX) break;
		case(A14_N_INPUT_4_ENDIDX + 1) : COPY_PART_NAME(:INPUT_4:ENDIDX) break;
		case(A14_N_INPUT_5 + 1) : COPY_PART_NAME(:INPUT_5) break;
		case(A14_N_INPUT_5_STARTIDX + 1) : COPY_PART_NAME(:INPUT_5:STARTIDX) break;
		case(A14_N_INPUT_5_ENDIDX + 1) : COPY_PART_NAME(:INPUT_5:ENDIDX) break;
		case(A14_N_INPUT_6 + 1) : COPY_PART_NAME(:INPUT_6) break;
		case(A14_N_INPUT_6_STARTIDX + 1) : COPY_PART_NAME(:INPUT_6:STARTIDX) break;
		case(A14_N_INPUT_6_ENDIDX + 1) : COPY_PART_NAME(:INPUT_6:ENDIDX) break;
		case(A14_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(A14_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
...
static struct {
	short code;
	int value;} clk_polarity_t[] = {
	0, 0,
	1, 1,
	0, 0 };
static struct {
	short code;
	int value;} str_polarity_t[] = {
	0, 0,
	1, 1,
	0, 0 };
static struct {
	short code;
	int value;} stp_polarity_t[] = {
	0, 0,
	1, 1,
	0, 0 };
static struct {
	...
	check_integer_conv_set(clock_divide, clock_divide_convert, clock_divide_t, 7, A14$_BAD_CLK_DIVIDE);
