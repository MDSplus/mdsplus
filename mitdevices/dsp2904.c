...
{
  int status;
  typedef struct _ModeReg { unsigned gate          : 1;
                            unsigned wrap          : 1;
                            unsigned bits32        : 1;
...
static int ReadChannel(InStoreStruct *setup, int channel, int num, unsigned short *buffer)
{
  ...
  { 
    ...
    pio(8,0,0,16);
    for (try = 0;(try < 20) && (!(CamQ(0)&1)) && (status & 1);try++) {pio(8,0,0,16);}
    pio(10,0,0,16);
    for (points_to_read = num; points_to_read && (status & 1); points_to_read = num - points_read)
    {
      ...
      if (iosb.bytcnt == 0) break;
      points_read += iosb.bytcnt/2;
    }
    if (points_to_read == 0) break;
  }
  if (try_outer >= 3) status = DSP2904$_CHANNEL_READ_ERROR;
  return status;
}
...
{ 
  ...
  return XmdsDeviceSetup(parent, (int *)niddsc->pointer, uids, XtNumber(uids), "DSP2904", uilnames, XtNumber(uilnames),0);
}
