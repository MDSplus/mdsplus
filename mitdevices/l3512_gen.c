...
int l3512__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(L3512_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(L3512_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(L3512_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(L3512_N_MEMORY_NAME + 1) : COPY_PART_NAME(:MEMORY_NAME) break;
		case(L3512_N_ROUTER_NAME + 1) : COPY_PART_NAME(:ROUTER_NAME) break;
		case(L3512_N_NUM_SPECTRA + 1) : COPY_PART_NAME(:NUM_SPECTRA) break;
		case(L3512_N_NUM_CHANNELS + 1) : COPY_PART_NAME(:NUM_CHANNELS) break;
		case(L3512_N_OFFSET + 1) : COPY_PART_NAME(:OFFSET) break;
		case(L3512_N_DURATION + 1) : COPY_PART_NAME(:DURATION) break;
		case(L3512_N_SPECTRA + 1) : COPY_PART_NAME(:SPECTRA) break;
		case(L3512_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(L3512_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
