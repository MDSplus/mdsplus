...
static int arm_init(InInitStruct *setup, int start)
{
  ...
  **********************************************/
  ...
  if (status & 1)
  {
    ...
    for (i=0; i<7; i++) if (freq >= freqs[i]) break;
    csreg.clock = i+1;
  }
...
int incaa6___store(struct descriptor_s *niddsc_ptr, InStoreStruct *setup)
{
  ...
  if (csreg.chans) {start=0; inc = 1;} else {inc = 2; start = (csreg.c_off) ? 1 : 0;}
  ...
  {
    int data_nid = setup->head_nid + INCAA6_N_INPUT_1 + (INCAA6_N_INPUT_2 - INCAA6_N_INPUT_1) * chan;
