...
static int ClockGetSetup(Widget w, int nid, int channel, DecoderSetup *setup_out, unsigned int *event_mask, 
   struct descriptor_xd *edges, struct descriptor_xd *edges_r, struct descriptor_xd *edges_f, String *info);
static int GatedClockGetSetup(Widget w, int nid, int channel, DecoderSetup *setup_out, unsigned int *event_mask, 
   struct descriptor_xd *edges, struct descriptor_xd *edges_r, struct descriptor_xd *edges_f, String *info);
static int GateGetSetup(Widget w, int nid, int channel, DecoderSetup *setup_out, unsigned int *event_mask, 
   struct descriptor_xd *edges, struct descriptor_xd *edges_r, struct descriptor_xd *edges_f, String *info);
static int DualClockGetSetup(Widget w, int nid, int channel, DecoderSetup *setup_out, unsigned int *event_mask, 
   struct descriptor_xd *edges, struct descriptor_xd *edges_r, struct descriptor_xd *edges_f, String *info);
...
int mpb__decoder___init(struct descriptor_s *niddsc_ptr, InInitStruct *setup)
{
    ...
    if (TreeIsOn(dev_nid)&1) {
      ...
      switch (mode)
      {
        case CLOCK: 
          ...
          break;
        case GATED_CLOCK:
          cstat = GatedClockGetSetup(0, setup->head_nid, chan, &regs[chan], (unsigned int *)events[chan], 
                                &edges, &edges_r, &edges_f, 0);
          break;
        case GATE:
        ...
        case DUAL_SPEED_CLOCK:
          cstat = DualClockGetSetup(0, setup->head_nid, chan, &regs[chan], (unsigned int *)events[chan], 
                                &edges, &edges_r, &edges_f, 0);
          break;
        default:
          cstat = 0; break;
  ...
  ****************************/
  ...
  pio(0,16,0xFF5F,"load source CTR 1-5 ")			/* load source CTR 1-5 */
...
{ 
  ...
  static MrmRegisterArg uilnames[] = {{"Reset",(XtPointer)Reset},
                                      {"Show",(XtPointer)Show}};
  XmdsDeviceSetup(parent, (int *)niddsc->pointer, uids, XtNumber(uids), "MPB__DECODER", uilnames, XtNumber(uilnames), &w);
  for (i=0;i<5;i++)
  {
    Widget mode_w;
    Widget pulldown_w;
...
static void Show(Widget w, Widget top, XmAnyCallbackStruct *cb)
{
  ...
  for (top = w; XtParent(top) && top != XtNameToWidget(XtParent(top),"MPB__DECODER"); top = XtParent(top));
  text[0] = 0;
  for (i=0;i<5;i++)
  {
    String info;
...
{
  for (*source = EXT_1MHZ, *period = 1E-6; *period * 65534 < dt && *source <= EXT_100HZ; (*source)++, *period *= 10);
}
   ...
   struct descriptor_xd *edges, struct descriptor_xd *edges_r, struct descriptor_xd *edges_f, String *info)
{
  ...
  if ((!(status & 1)) || (frequency <= 0.0)) return TIMING$_INVCLKFRQ;
  status = GetFloat(w, nid, channel, CHANNEL_P2, &duty_cycle);
  if (!(status & 1)) return TIMING$_INVDUTY;
  dt = 1/frequency;
  ...
  else
  {
    ...
    sprintf(text,"is a single-speed clock with a frequency of %g (Hz)\n and a duty cycle of %g. The output signal starts %s.\n\n",
                      1/dt2,(float)setup.load/(float)(setup.load + setup.hold),setup.start_high ? "high" : "low");
  }
  return status;
}
   ...
   struct descriptor_xd *edges, struct descriptor_xd *edges_r, struct descriptor_xd *edges_f, String *info)
{
  ...
  {
    String text = (*info = malloc(500));
    sprintf(text,"is a gated clock with a frequency of %g (Hz)\n and a duty cycle of %g. The output signal starts %s.\n\n",
                      1/dt2,(float)setup.load/(float)(setup.load + setup.hold),setup.start_high ? "high" : "low");
  }
  return status;
}
   ...
   struct descriptor_xd *edges, struct descriptor_xd *edges_r, struct descriptor_xd *edges_f, String *info)
{
  ...
  if (!(status & 1)) return TIMING$_INVDELDUR;
  ...
  if (!(status & 1)) return TIMING$_INVTRGMOD;
  switch (trigger_mode)
  {
  ...
  if (setup.load < 3) setup.load = 3;
  if (setup.hold < 3) setup.hold = 3;
  ...
  if (nid)
  {
    ...
    static DESCRIPTOR(output_exp,"PACK($1+$2,REPLICATE([1,0],0,SIZE($1)/2)) : PACK($1,REPLICATE([0,1],0,SIZE($1)/2)) : $3");
    int event_nid;
    static DESCRIPTOR_FLOAT(dt1_dsc,&dt1);
    static DESCRIPTOR_FLOAT(dt2_dsc,&dt2);
  ...
  else
  {
    ...
    sprintf(text,"is a gate which goes %s at Event time + %g seconds\nand stays %s for %g seconds\n\n",
   ...
   struct descriptor_xd *edges, struct descriptor_xd *edges_r, struct descriptor_xd *edges_f, String *info)
{
  ...
  if ((!(status & 1)) || (frequency_1 <= 0.0)) return TIMING$_INVCLKFRQ;
  status = GetFloat(w, nid, channel, CHANNEL_P2, &frequency_2);
  if ((!(status & 1)) || (frequency_2 <= 0.0)) return TIMING$_INVCLKFRQ;
  ...
  if (setup.load == 0) setup.load++;
  if (setup.hold == 0) setup.hold++;
  if (nid)
  {
    static int snid;
    ...
    gnid = ChannelNid(nid,channel,CHANNEL_P3);
    rising = 1;
    TdiCompile(&output_exp,&start,&dt1,&dt2,&start_low,&gate,&rising_dsc,edges_r MDS_END_ARG);
    rising = 0;
    TdiCompile(&output_exp,&start,&dt1,&dt2,&start_low,&gate,&rising_dsc,edges_f MDS_END_ARG);
    MdsFree1Dx(edges,0);
    *setup_out = setup;
    status = GetEvent(&event_nid,event_mask);
    if (!(status & 1)) return status;
  ...
  {
    String text = (*info = malloc(500));
    sprintf(text,"is a dual-speed clock with frequencies of %g (Hz)\n and %g. The output signal starts %s.\n\n",
                      1/(setup.load * period * 2),1/(setup.hold * period * 2),setup.start_high ? "high" : "low");
  }
  return status;
}
