...
typedef struct  { unsigned range : 3;
                  unsigned mult  : 4;
                  unsigned fill  : 9;
                } DwellCode;
...
int l3512___init(struct descriptor *niddsc, InInitStruct *setup)
{
  int status;
  float duration;
...
#pragma member_alignment restore
  ...
  for (i = 8196/min(max(setup->num_channels,256),8192), setup_3512.gain = 0; i; setup_3512.gain++,i = i/2);
  ...
  pio(setup->router_name, 17, 1, (short *)&setup->num_spectra, 16);
...
static DwellCode ConvertDwell(float *dwell)
{
  ...
  {
    for (range = 1E-4, i=1; i < 7 && !(*dwell >= range && *dwell < (range * 9.5)); i++,range *= 10.);
    answer.range = i;
...
int l3512___store(struct descriptor *niddsc, InStoreStruct *setup)
{ 
...
int comment_nid = setup->head_nid + L3512_N_COMMENT;
    ...
    pio(setup->memory_name,2,0,&data[32767],16);
  {
    ...
    signal.dimensions[1] = (duration > 0.0 ? (struct descriptor *)&dim2 : (struct descriptor *)&duration_niddsc);
