...
{
  struct _msetup { unsigned pretrig : 1;
		 unsigned clock : 4;
                 unsigned blocks : 4;
                 unsigned fill : 7;
                 } msetup = {0,0,0,0};
...
int h912___store(struct descriptor *niddsc, InStoreStruct *setup)
{
  int status;
  int mstatus;
  struct _status { unsigned mode : 3;
                   unsigned state : 2;
                   unsigned mem : 2;
                   unsigned fill : 3;
                   unsigned blocks : 4;
  ...
  static float freqs[] = {2E-6,5E-6,1E-5,2E-5,5E-5,1E-4,2E-4,5E-4,1E-3,2E-3,5E-3,0.,0.,0.,0.,0.};
  ...
  if (blocks == 0) return DEV$_NOT_TRIGGERED;
  if (hstat.clock > 10)
  {
  ...
  return_on_error(TdiCompile(&timestamps_exp,clock,&blocks_d,&blocksize_d,&trigger,&pts_d,&timestamps MDS_END_ARG));
  ...
  for (chan=1;chan<16;chan++)
  {
    ...
    if ((onstat & 1) || (onstat == TreePARENT_OFF))
    {
      ...
      if (status&1) raw.bounds[0].l = min(samples-1,max(0,raw.bounds[0].l));
      else raw.bounds[0].l = 0;

      status = DevLong(&endidx_nid, (int *)&raw.bounds[0].u);
      if (status&1) raw.bounds[0].u = min(samples-1,max(0,raw.bounds[0].u));
      else raw.bounds[0].u = samples-1;
      ...
      if (raw.m[0] > 0)
      {
        ...
        if (status & 1)
        {
          ...
          return_on_error(TdiCompile(&sig_exp,&raw,&timestamps_nid_d,&start_d,&end_d,&signal MDS_END_ARG));
