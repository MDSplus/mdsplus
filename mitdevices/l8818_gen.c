...
#include "l8818_gen.h"
int l8818__add(struct descriptor *name_d_ptr, struct descriptor *dummy_d_ptr, int *nid_ptr)
{
...
int l8818__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(L8818_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(L8818_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(L8818_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(L8818_N_TRIGGER + 1) : COPY_PART_NAME(:TRIGGER) break;
		case(L8818_N_CLOCK + 1) : COPY_PART_NAME(:CLOCK) break;
		case(L8818_N_EXT_CLOCK + 1) : COPY_PART_NAME(:EXT_CLOCK) break;
		case(L8818_N_PRE_TRIG + 1) : COPY_PART_NAME(:PRE_TRIG) break;
		case(L8818_N_ACTIVE_MEM + 1) : COPY_PART_NAME(:ACTIVE_MEM) break;
		case(L8818_N_INPUT + 1) : COPY_PART_NAME(:INPUT) break;
		case(L8818_N_INPUT_STARTIDX + 1) : COPY_PART_NAME(:INPUT:STARTIDX) break;
		case(L8818_N_INPUT_ENDIDX + 1) : COPY_PART_NAME(:INPUT:ENDIDX) break;
		case(L8818_N_INPUT_OFFSET + 1) : COPY_PART_NAME(:INPUT:OFFSET) break;
		case(L8818_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(L8818_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
...
int l8818__init(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
...
int l8818__trigger(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
...
int l8818__store(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
