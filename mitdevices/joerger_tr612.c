...
struct _pre_mem { unsigned pretrig : 3;
                  unsigned actmem : 3;
                  unsigned not_used : 10;
                };

struct _freq_mode { unsigned freq : 3;
                    unsigned mode : 1;
		    unsigned not_used : 2;
                    unsigned armed : 1;
                    unsigned active : 1;
...
static int ReadChannel(InStoreStruct   *setup,
                int			max_samps,
                int                     chan,
                short			*data_ptr);
...
int joerger_tr612___store(struct descriptor *niddsc, InStoreStruct *setup)
{
  ...
  if (reg2.mode)
  {
    ...
    status = TdiCompile(&burst_dim,&max_samples_d,&trigger_d,dimension.axis,&burst_dimension MDS_END_ARG);
    max_samples++;
