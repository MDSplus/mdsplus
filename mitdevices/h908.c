...
int h908___store(struct descriptor *niddsc, InStoreStruct *setup)
{
  ...
  static float freqs[] = {0.,2.5E-5,5E-5,1E-4,2E-4,5E-4,1E-3,2E-3,5E-3,1E-2,0.,0.,0.,0.,0.,0.};
  ...
  if (validsamps == 0) return DEV$_NOT_TRIGGERED;
      ...
      if (raw.m[0] > 0)
      {
        ...
        if (status & 1)
        {
         static DESCRIPTOR(sig_exp,"BUILD_SIGNAL(BUILD_WITH_UNITS(1.25E-3*$VALUE,'volts'),BUILD_WITH_UNITS($,'counts'),"//
          ...
          return_on_error(TdiCompile(&sig_exp,&raw,&start_d,&end_d,&trigger,clock,&signal MDS_END_ARG));
