...
static int ReadSetup(char *name,
                     float		   *freq_ptr,
                     int                   *polarity);
...
int max_samples;
 ...
 int one = 1;
  ...
  if ((max_samples == 8192) && ((TreeIsOn(c_nids[A12_N_NAME]) & 1) == 0)) max_samples = 32767;
...
static int ReadSetup(char *name,
                     ...
                     int                   *offset)
{
struct a12_setup { unsigned a12_setup_v_offset : 6;      /* Bit on indicates bipolar, 0:10V, else unipolar +5:-5V */
                ...
                short			*data_ptr)
{
  ...
  {
    return_on_error(DevCamChk(CamStopw(name,0,2,*max_samps_ptr,data_ptr,16,0),&one,0),status);
  }
  return 1;
 error:
  return status;
