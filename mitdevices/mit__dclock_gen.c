...
int mit__dclock__add(struct descriptor *name_d_ptr, struct descriptor *dummy_d_ptr, int *nid_ptr)
{
...
int mit__dclock__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(MIT__DCLOCK_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(MIT__DCLOCK_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(MIT__DCLOCK_N_FREQUENCY_1 + 1) : COPY_PART_NAME(:FREQUENCY_1) break;
		case(MIT__DCLOCK_N_FREQUENCY_2 + 1) : COPY_PART_NAME(:FREQUENCY_2) break;
		case(MIT__DCLOCK_N_TSTART + 1) : COPY_PART_NAME(:TSTART) break;
		case(MIT__DCLOCK_N_START_LOW + 1) : COPY_PART_NAME(:START_LOW) break;
		case(MIT__DCLOCK_N_GATE + 1) : COPY_PART_NAME(:GATE) break;
		case(MIT__DCLOCK_N_EDGES_R + 1) : COPY_PART_NAME(:EDGES_R) break;
		case(MIT__DCLOCK_N_EDGES_F + 1) : COPY_PART_NAME(:EDGES_F) break;
...
int mit__dclock___get_setup(struct descriptor *nid_d_ptr, InGet_setupStruct *in_ptr)
{
