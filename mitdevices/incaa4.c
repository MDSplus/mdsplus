...
typedef struct _CSRegister { unsigned clock:  4;
                             unsigned chans:  3;
                             unsigned c_off:  2;
                             unsigned :1;
...
int incaa4___arm(struct descriptor *niddsc, InArmStruct *setup)
{
  int status;
  float freq;
  CSRegister csreg;
  if (setup->int_clk_frq)
  {
    if ((status = TdiGetFloat(setup->int_clk_frq,&freq)) & 1)
    {
      ...
      for (i=0; i<8; i++) if (freq >= freqs[i]) break;
      csreg.clock = i+1;
    }
    else
      return status;
...
int incaa4___init(struct descriptor *niddsc, InInitStruct *setup)
{
...
int incaa4___trigger(struct descriptor *niddsc, InTriggerStruct *setup)
{
  int status;
  pio(25,2,0);
  return status;
...
int incaa4___store(struct descriptor_s *niddsc_ptr, InStoreStruct *setup)
{
...
#define max(a,b) ((a) >= (b)) ? (a) : (b)
  ...
  if (raw.pointer) free(raw.pointer);
  raw.pointer = malloc(samps_per_chan * 2);
  for (chan=0;((chan < active_chans) && (status & 1));chan++)
  {
    int input_nid = setup->head_nid + INCAA4_N_INPUT_1 + chan * (INCAA4_N_INPUT_2 - INCAA4_N_INPUT_1);
    ...
    if (TreeIsOn(input_nid) & 1)
    {
      ...
      if (raw.m[0] > 0)
      {
        ...
        int samps;
        pio(16,0,&addr);
        for (samples_to_read = raw.m[0], data_ptr=(short *)raw.pointer; (samples_to_read > 0) && (status & 1); 
