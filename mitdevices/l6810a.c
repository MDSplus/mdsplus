...
#define CHAN_NID(chan, field) c_nids[L6810A_N_CHANNELS+chan*L6810A_K_NODES_PER_CHANNEL+field]
  ...
  for (chan=0;((chan < 4) && (status & 1));chan++)
  {
    if (TreeIsOn(CHAN_NID(chan,L6810A_N_CHAN_HEAD)) & 1)
    {
	  ...
	  coefficient = coeffs[setup.sensitivity[chan]];
          ...
          offset = *optr * -16;
          status = TreePutRecord(CHAN_NID(chan,L6810A_N_CHAN_HEAD),(struct descriptor *)&signal,0);
        }
      }
    }
  }
