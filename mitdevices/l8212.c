...
*/
  ...
  return_on_error(FreqToClock(chans, setup->noc, setup->head_nid+L8212_04_N_FREQ, &clock), status);
  ...
  if ((status&1)==0) {
    *clock = 0;
    ...
    switch(master_clock) {
      case 0:
        switch(*clock) {
          case 4: if (noc > 8) status = L8212$_FREQ_TO_HIGH; break;
          case 5: if (noc > 4) status = L8212$_FREQ_TO_HIGH; break;
          case 6: if (noc > 2) status = L8212$_FREQ_TO_HIGH; break;
          case 7: if (noc > 1) status = L8212$_FREQ_TO_HIGH; break;
        }
        break;
      case 1:
        switch(*clock) {
          case 4: if (noc > 32) status = L8212$_FREQ_TO_HIGH; break;
          case 5: if (noc > 16) status = L8212$_FREQ_TO_HIGH; break;
          case 6: if (noc > 8 ) status = L8212$_FREQ_TO_HIGH; break;
          case 7: if (noc > 4 ) status = L8212$_FREQ_TO_HIGH; break;
        }
        break;
    }
  }
...
static int NOCToCode(int chans, int noc, int *noc_code)
{
  ...
  if (noc > chans) return L8212$_INVALID_NOC;
  for (*noc_code=0; ((*noc_code<4) && (noc != nocs[model][*noc_code])); (*noc_code)++);
  if (*noc_code==4) (*noc_code)--;
...
int name_nid;
  ...
  return_on_error(ReadSetup(chans, setup->name, setup->memories, setup->header, &samples_per_channel,
                            &min_idx, &max_idx, &frequency, &num_chans),status);
  dimension.axis = frequency == 0 ? (struct descriptor *)&ext_clock_d : (struct descriptor *)&int_clock_d;
  status = 1;
...
static int L8212_SETUP(struct descriptor *niddsc,  Widget parent, int chans)
{
  ...
  static char *clock_strings_fast[] = {" 500 Hz","   2.5 KHz","   5   KHz","  12.5 KHz","  25   KHz","  50   KHz"," 100   KHz"," External"};
  static char *clock_vals_fast_16[] = {".5","2.5","5.0","12.5","25.","50.","0."};
  static char *clock_strings_fast_16[] = {" 500 Hz","   2.5 KHz","   5   KHz","  12.5 KHz","  25   KHz","  50   KHz"," External"};
  ...
  status =  XmdsDeviceSetup(parent, (int *)niddsc->pointer, uids, XtNumber(uids), "L8212", uilnames, XtNumber(uilnames), &dbox);
  reset(dbox);
  return status;
}
extern int TdiText();
static void mems_changed_proc(Widget w, int *tag, XmScaleCallbackStruct *reason)
{
  ...
  struct descriptor_xd *header_xd = XmdsNidOptionMenuGetXd(XtNameToWidget(XtParent(w), "header_menu"));
...
static void L8212$HeaderToPTS(int mems, char *header, int pts[])
{
  ...
  for (pin=0; pin<11; pin++)
    switch(header[pin]) {
      case '9': pins[pin]=0; break;
      case '4': pins[pin]=1; break;
      case '1': pins[pin]=0; lowbit = pin; break;
      case '2': pins[pin]=0; middlebit = pin; break;
      case '3': pins[pin]=0; highbit = pin; break;
    };

  for (i=0; i<8; i++) {
    int ptsc;
    if (lowbit != -1) pins[lowbit] = (i&1);
    if (middlebit != -1) pins[middlebit] = ((i>>1)&1);
    if (highbit != -1) pins[highbit]= ((i>>2)&1);
    ptsc = (pins[1 ]?32768:0)|
           (pins[5 ]?16384:0)|
           (pins[6 ]?8192 :0)|
           (pins[7 ]?4096 :0)|
...
{
  int mems;
  struct descriptor_xd *header_xd = XmdsNidOptionMenuGetXd(XtNameToWidget(XtParent(pts_menu), "header_menu"));
  static char header[11];
  static struct descriptor_s header_dsc = {sizeof(header), DTYPE_T, CLASS_S, header};
  int status = TdiText(header_xd, &header_dsc MDS_END_ARG);
  if (status&1) {  
