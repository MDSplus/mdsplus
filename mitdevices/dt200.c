...
int DT200$DW_SETUP(struct dsc$descriptor *niddsc, struct dsc$descriptor *methoddsc, Widget parent)
{
  ...
  return XmdsDeviceSetup(parent, (int *)niddsc->dsc$a_pointer, uids, XtNumber(uids), "DT200",
...
int dt200__dw_setup(struct descriptor *niddsc, struct descriptor *methoddsc, Widget parent)
{
  ...
  return XmdsDeviceSetup(parent, (int *)niddsc->pointer, uids, XtNumber(uids), "DT200", uilnames,
			 XtNumber(uilnames), 0);
}
#endif
