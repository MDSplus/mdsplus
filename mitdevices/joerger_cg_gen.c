...
int joerger_cg__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(JOERGER_CG_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(JOERGER_CG_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(JOERGER_CG_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(JOERGER_CG_N_CHANNEL_1 + 1) : COPY_PART_NAME(:CHANNEL_1) break;
		case(JOERGER_CG_N_CHANNEL_1_INVERTED + 1) : COPY_PART_NAME(:CHANNEL_1.INVERTED) break;
		case(JOERGER_CG_N_CHANNEL_2 + 1) : COPY_PART_NAME(:CHANNEL_2) break;
		case(JOERGER_CG_N_CHANNEL_2_INVERTED + 1) : COPY_PART_NAME(:CHANNEL_2.INVERTED) break;
		case(JOERGER_CG_N_CHANNEL_3 + 1) : COPY_PART_NAME(:CHANNEL_3) break;
		case(JOERGER_CG_N_CHANNEL_3_INVERTED + 1) : COPY_PART_NAME(:CHANNEL_3.INVERTED) break;
		case(JOERGER_CG_N_CHANNEL_4 + 1) : COPY_PART_NAME(:CHANNEL_4) break;
		case(JOERGER_CG_N_CHANNEL_4_INVERTED + 1) : COPY_PART_NAME(:CHANNEL_4.INVERTED) break;
		case(JOERGER_CG_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
