...
int mit_dclock__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(MIT_DCLOCK_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(MIT_DCLOCK_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(MIT_DCLOCK_N_FREQUENCY_1 + 1) : COPY_PART_NAME(:FREQUENCY_1) break;
		case(MIT_DCLOCK_N_FREQUENCY_2 + 1) : COPY_PART_NAME(:FREQUENCY_2) break;
		case(MIT_DCLOCK_N_GATE + 1) : COPY_PART_NAME(:GATE) break;
		case(MIT_DCLOCK_N_OUTPUT + 1) : COPY_PART_NAME(:OUTPUT) break;
