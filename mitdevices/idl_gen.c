...
int idl__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(IDL_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(IDL_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(IDL_N_COMMANDS + 1) : COPY_PART_NAME(:COMMANDS) break;
		case(IDL_N_EXEC_ACTION + 1) : COPY_PART_NAME(:EXEC_ACTION) break;
