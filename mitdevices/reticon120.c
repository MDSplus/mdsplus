...
int reticon120___init(struct descriptor_s *niddsc_ptr, InInitStruct *setup)
{
...
#pragma member_alignment restore
  ReturnOnError(TdiEq(setup->pixel_select,&zero_d,&pixels_d MDS_END_ARG),RETICON$_BAD_PIXEL_SELECT);
  for (s=0;s<setup->num_states;s++)
  {
    ...
    if (frames > 2048) return RETICON$_BAD_FRAMES;
    ...
    if (i == 7) return RETICON$_BAD_FRAME_SELECT;
...
int reticon120__dw_setup(struct descriptor *niddsc, struct descriptor *methoddsc, Widget parent)
{ 
...
static void Reset(Widget w)
{
  ...
  for (i=0;i<256;i++,t_w++)
  {
    while (!XmIsToggleButtonGadget(*t_w)) t_w++;
...
static int Apply(Widget w)
{
  ...
  for (state=0;state<num_states;state++)
  {
    ...
    {
      XmdsComplain(XtParent(w),"State %d contains invalid integration period for %d pixel groups selected\n"\
                       "Maximum of 60,121 and 249 pixel groups for .5,1, and 2 msec integration periods",state,num_pix);
      return 0;
    }
    else
    {
      xd = (struct descriptor_xd *)XmdsExprGetXd(XtNameToWidget(XtParent(w),fname));
      if (!xd || (!(TdiGetLong(xd->pointer,&num_frames) & 1)) || (num_frames > 2047) || !num_frames)
      {
        XmdsComplain(XtParent(w),"Invalid frame count specified for state %d\n specify a number between 1 and 2047",state);
        return 0;
      }
      else
...
int reticon120___store(struct descriptor_s *niddsc_ptr, InStoreStruct *setup)
{
  int status;
  float period;
  static int pixels = 0;
...
#pragma member_alignment save
#pragma nomember_alignment
typedef  struct { unsigned  __attribute__ ((packed)) value     : 12   ;
...
buf *buffer;
  ...
  if (words)
  {
    ...
    for (i=0,is_time=0,frame_count=0,pixel_count=0,frame_size=0,latch_time=0,j=0;i<words && (status & 1);i++)
    {
      ...
      {
        if (buffer[i].event_latch && (latch_time != 0.0)) latch_time = times[frame_count-1];
        if (frame_count)
...
int reticon120___trigger(struct descriptor_s *niddsc_ptr, InTriggerStruct *setup)
{
  int status;
