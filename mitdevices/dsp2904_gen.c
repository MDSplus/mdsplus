...
int dsp2904__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(DSP2904_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(DSP2904_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(DSP2904_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(DSP2904_N_TRAQ_NAME + 1) : COPY_PART_NAME(:TRAQ_NAME) break;
		case(DSP2904_N_TIMER + 1) : COPY_PART_NAME(:TIMER) break;
		case(DSP2904_N_TIMER_GATE + 1) : COPY_PART_NAME(:TIMER.GATE) break;
		case(DSP2904_N_TIMER_WRAP + 1) : COPY_PART_NAME(:TIMER.WRAP) break;
		case(DSP2904_N_TIMER_BITS_32 + 1) : COPY_PART_NAME(:TIMER.BITS_32) break;
		case(DSP2904_N_TIMER_PRESET + 1) : COPY_PART_NAME(:TIMER:PRESET) break;
		case(DSP2904_N_TIMER_CHANNEL + 1) : COPY_PART_NAME(:TIMER:CHANNEL) break;
		case(DSP2904_N_COUNTER + 1) : COPY_PART_NAME(:COUNTER) break;
		case(DSP2904_N_COUNTER_GATE + 1) : COPY_PART_NAME(:COUNTER.GATE) break;
		case(DSP2904_N_COUNTER_WRAP + 1) : COPY_PART_NAME(:COUNTER.WRAP) break;
		case(DSP2904_N_COUNTER_BITS_32 + 1) : COPY_PART_NAME(:COUNTER.BITS_32) break;
		case(DSP2904_N_COUNTER_PRESET + 1) : COPY_PART_NAME(:COUNTER:PRESET) break;
		case(DSP2904_N_COUNTER_CHANNEL + 1) : COPY_PART_NAME(:COUNTER:CHANNEL) break;
		case(DSP2904_N_IDX_DIVIDER + 1) : COPY_PART_NAME(.IDX_DIVIDER) break;
		case(DSP2904_N_TIME_OF_CNT + 1) : COPY_PART_NAME(.TIME_OF_CNT) break;
		case(DSP2904_N_RATE + 1) : COPY_PART_NAME(:RATE) break;
		case(DSP2904_N_CLOCK_OUT + 1) : COPY_PART_NAME(:CLOCK_OUT) break;
		case(DSP2904_N_EXT_CLK_IN + 1) : COPY_PART_NAME(:EXT_CLK_IN) break;
		case(DSP2904_N_START + 1) : COPY_PART_NAME(:START) break;
		case(DSP2904_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(DSP2904_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
