...
#include "j412_gen.h"
int j412__add(struct descriptor *name_d_ptr, struct descriptor *dummy_d_ptr, int *nid_ptr)
{
...
int j412__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(J412_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(J412_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(J412_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(J412_N_DT + 1) : COPY_PART_NAME(:DT) break;
		case(J412_N_START_TRIG + 1) : COPY_PART_NAME(:START_TRIG) break;
		case(J412_N_SET_POINTS + 1) : COPY_PART_NAME(:SET_POINTS) break;
		case(J412_N_OUTPUT + 1) : COPY_PART_NAME(:OUTPUT) break;
		case(J412_N_NUM_CYCLES + 1) : COPY_PART_NAME(:NUM_CYCLES) break;
		case(J412_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
...
int j412__init(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr)
{
