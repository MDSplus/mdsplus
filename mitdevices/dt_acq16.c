...
#ifdef __VMS
int DT_ACQ16$DW_SETUP(struct dsc$descriptor *niddsc, struct dsc$descriptor *methoddsc,
		      Widget parent)
{
  ...
  static NCI_ITM nci[] =
      { {4, NCI$_CONGLOMERATE_NIDS, (unsigned char *)&nid, 0}, {0, NCI$_END_OF_LIST, 0, 0} };
  TREE$GETNCI((int *)niddsc->dsc$a_pointer, nci);
  uilnames[0].value = (XtPointer) nid;
  return XmdsDeviceSetup(parent, (int *)niddsc->dsc$a_pointer, uids, XtNumber(uids), "DT_ACQ16",
