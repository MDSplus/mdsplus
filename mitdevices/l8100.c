...
int l8100___init(struct descriptor *niddsc, InInitStruct *setup)
{
  ...
  status=InitChannel(setup, 0, 
                     ...
                     setup->input_1_mult_convert, 
                     setup->input_1_offset);
  status=InitChannel(setup, 1, 
                     setup->input_2_gain_convert,
                     setup->input_2_mult_convert, 
                     setup->input_2_offset);
  return status;
}
...
static int StoreChannel(InStoreStruct *setup, int chan)
{
  ...
  if (IsOn(input_nid))
  {
    ...
    SignalError(TdiCompile(&expression,&out_nid_d,&offset_d,&gain_d,&mult_d,&value MDS_END_ARG));
