...
int b2408__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(B2408_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(B2408_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(B2408_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(B2408_N_TRIGGER + 1) : COPY_PART_NAME(:TRIGGER) break;
		case(B2408_N_EXT_CLOCK + 1) : COPY_PART_NAME(:EXT_CLOCK) break;
		case(B2408_N_TIMEBASE + 1) : COPY_PART_NAME(:TIMEBASE) break;
		case(B2408_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(B2408_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
