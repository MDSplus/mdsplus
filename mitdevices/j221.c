...
{ 
  ...
  struct j221_registers { unsigned j221_setup_v_output   : 1;      
  ...
  if (num)
  {
    ...
    if (status & 1) {
      ...
      if (i > 1) printf ("******J221 needed %d tries on the first STOPW******\n", i);
    }
    if (status & 1) {
      int i;
      for (i=0,status=0; (i<10) && !(status&1); i++) {
      ...
      if (i > 1) printf ("******J221 needed %d tries on the second STOPW******\n", i);
    }
...
static int merge_data(int nid, int **data, int **times, int *ndata)
{
  ...
  *ndata = 0;
  for (i=0; i<12; i++)
  {
    ...
    int chan_nid = nid + i*(J221_N_OUTPUT_02 - J221_N_OUTPUT_01) + J221_N_OUTPUT_01_SET_POINTS - J221_N_OUTPUT_01;
    int mode_nid = nid + i*(J221_N_OUTPUT_02 - J221_N_OUTPUT_01) + J221_N_OUTPUT_01_MODE - J221_N_OUTPUT_01;
    int width = 0;
    wave_arsize[i] = 0;
        ...
        if (TdiLong(&xd,&xd MDS_END_ARG) & 1)
        {
		  ...
		  wave_arsize[i] = -1;
		    ...
		    wave_pointer[i][j+1] = width +
		      (wave_pointer[i][j] = *(int *)(((struct descriptor_a *)xd.pointer)->pointer+j*2));
                wave_pointer[i][xdsize] = 16777215;
                *ndata += xdsize;
              }
            }
          }
          else wave_arsize[i] = -1;
        }
        else wave_arsize[i] = -1;
      }
      else wave_arsize[i] = -1;
  ...
  *times = (int *)setpoint;
  ...
  while (next < 16777215) {
    next = 16777215;
    for (i=0; i<12; i++) if (wave_arsize[i] && *wave_pointer[i] < next) next = *wave_pointer[i];
    if (next < 16777215) {
      for (i=0; i<12; i++) if (wave_arsize[i] && *wave_pointer[i] == next) {
...
int j221___add(int *head_nid)
{
  ...
  for (i=0;i<12;i++)
  {
    ...
    static DESCRIPTOR(gates,"I_TO_X(BUILD_DIM(,$1),($2 > 0 ? SPREAD([0,$2],1,SIZE($3))+SPREAD($3,0,2) : $3) ) * $4 + $5");
    static DESCRIPTOR(signal,"2*BUILD_SIGNAL([0,REPLICATE([1,0],0,ESIZE($1)/2),ESIZE($1) MOD 2 ? [1,1] : 0],\
    ...
    TdiCompile(&trigs,&clock_nid_d,&mode_nid_d,&set_points_nid_d,&divide_nid_d,&start_trig_nid_d,&xd MDS_END_ARG);
    TreePutRecord(triggers_nid,(struct descriptor *)&xd,0);
    TdiCompile(&gates,&clock_nid_d,&mode_nid_d,&set_points_nid_d,&divide_nid_d,&start_trig_nid_d,&xd MDS_END_ARG);
