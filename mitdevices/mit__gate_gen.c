...
int mit__gate__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(MIT__GATE_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(MIT__GATE_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(MIT__GATE_N_TRIGGER_MODE + 1) : COPY_PART_NAME(:TRIGGER_MODE) break;
		case(MIT__GATE_N_START_LOW + 1) : COPY_PART_NAME(:START_LOW) break;
		case(MIT__GATE_N_TRIGGER + 1) : COPY_PART_NAME(:TRIGGER) break;
		case(MIT__GATE_N_EVENT + 1) : COPY_PART_NAME(:EVENT) break;
		case(MIT__GATE_N_DURATION + 1) : COPY_PART_NAME(:DURATION) break;
		case(MIT__GATE_N_PULSE_TIME + 1) : COPY_PART_NAME(:PULSE_TIME) break;
		case(MIT__GATE_N_GATE_OUT + 1) : COPY_PART_NAME(:GATE_OUT) break;
		case(MIT__GATE_N_EDGES_R + 1) : COPY_PART_NAME(:EDGES_R) break;
		case(MIT__GATE_N_EDGES_F + 1) : COPY_PART_NAME(:EDGES_F) break;
