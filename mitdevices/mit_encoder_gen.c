...
int mit_encoder__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(MIT_ENCODER_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(MIT_ENCODER_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(MIT_ENCODER_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(MIT_ENCODER_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(MIT_ENCODER_N_CHANNEL_1 + 1) : COPY_PART_NAME(:CHANNEL_1) break;
		case(MIT_ENCODER_N_CHANNEL_1_EVENT + 1) : COPY_PART_NAME(:CHANNEL_1:EVENT) break;
		case(MIT_ENCODER_N_CHANNEL_2 + 1) : COPY_PART_NAME(:CHANNEL_2) break;
		case(MIT_ENCODER_N_CHANNEL_2_EVENT + 1) : COPY_PART_NAME(:CHANNEL_2:EVENT) break;
		case(MIT_ENCODER_N_CHANNEL_3 + 1) : COPY_PART_NAME(:CHANNEL_3) break;
		case(MIT_ENCODER_N_CHANNEL_3_EVENT + 1) : COPY_PART_NAME(:CHANNEL_3:EVENT) break;
		case(MIT_ENCODER_N_CHANNEL_4 + 1) : COPY_PART_NAME(:CHANNEL_4) break;
		case(MIT_ENCODER_N_CHANNEL_4_EVENT + 1) : COPY_PART_NAME(:CHANNEL_4:EVENT) break;
		case(MIT_ENCODER_N_CHANNEL_5 + 1) : COPY_PART_NAME(:CHANNEL_5) break;
		case(MIT_ENCODER_N_CHANNEL_5_EVENT + 1) : COPY_PART_NAME(:CHANNEL_5:EVENT) break;
		case(MIT_ENCODER_N_CHANNEL_6 + 1) : COPY_PART_NAME(:CHANNEL_6) break;
		case(MIT_ENCODER_N_CHANNEL_6_EVENT + 1) : COPY_PART_NAME(:CHANNEL_6:EVENT) break;
		case(MIT_ENCODER_N_SOFT_CHANNEL + 1) : COPY_PART_NAME(:SOFT_CHANNEL) break;
		case(MIT_ENCODER_N_SOFT_CHANNEL_EVENT + 1) : COPY_PART_NAME(:SOFT_CHANNEL:EVENT) break;
...
int mit_encoder___set_event(struct descriptor *nid_d_ptr, InSet_eventStruct *in_ptr)
{
