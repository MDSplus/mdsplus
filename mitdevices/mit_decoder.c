...
int mit_decoder___init(struct descriptor *niddsc_ptr, InInitStruct *setup)
{
  ...
  *****************************/
  for (chan=0; chan < 5; chan++)
  {
    ...
    unsigned short temp;
    int c_nid = setup->head_nid + MIT_DECODER_N_CHANNEL_0 + (MIT_DECODER_N_CHANNEL_1 - MIT_DECODER_N_CHANNEL_0) * chan;
  ...
  *********************************/
  ...
  while (OctaFFS(&all_events,&event_bit))
  {
    ...
    for (chan = 0; chan < 5+2; chan++)
    {
      ...
      {
	event_reg |= (1 << (7 + chan));
        if (chan < 4) event_reg |= 0x1000; /* Start scalers on all events */
      }
    }
...
static int GetPseudoDevNid( struct descriptor_xd *pseudo_xd, int *dev_nid)
{
  ...
  while (d_ptr && d_ptr->dtype == DTYPE_DSC) d_ptr = (struct descriptor *)d_ptr->pointer;
  if (d_ptr) {
    status = 1;
    switch (d_ptr->dtype) {
      case DTYPE_NID : *dev_nid = *(int *)d_ptr->pointer; break;
    case DTYPE_PATH : { char *name = strncpy(malloc(d_ptr->length+1),d_ptr->pointer,d_ptr->length);
                        name[d_ptr->length]=0;
                        status = TreeFindNode(name, dev_nid);
                        free(name);
                        break;
                      }
      default : status = TIMING$_INVPSEUDODEV;
    }
  } else return TIMING$_NOPSEUDODEV;
  return status;
}
...
static int GetEvent(char *name, EventMask *mask)
{
  ...
  status = TdiExecute(&expr,&name_dsc,&xd);
  if (status&1) {
...
{ 
  ...
  status = TdiExecute(&expression,&nid_dsc,&xd);
  if (status & 1)
  {
