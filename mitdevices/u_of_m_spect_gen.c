...
int u_of_m_spect__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(U_OF_M_SPECT_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(U_OF_M_SPECT_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(U_OF_M_SPECT_N_GO_FILE + 1) : COPY_PART_NAME(:GO_FILE) break;
		case(U_OF_M_SPECT_N_DATA_FILE + 1) : COPY_PART_NAME(:DATA_FILE) break;
		case(U_OF_M_SPECT_N_TRIGGER + 1) : COPY_PART_NAME(:TRIGGER) break;
		case(U_OF_M_SPECT_N_SETUP + 1) : COPY_PART_NAME(.SETUP) break;
		case(U_OF_M_SPECT_N_SETUP_WAVE_LENGTH + 1) : COPY_PART_NAME(.SETUP:WAVE_LENGTH) break;
		case(U_OF_M_SPECT_N_SETUP_SLIT_WIDTH + 1) : COPY_PART_NAME(.SETUP:SLIT_WIDTH) break;
		case(U_OF_M_SPECT_N_SETUP_NUM_SPECTRA + 1) : COPY_PART_NAME(.SETUP:NUM_SPECTRA) break;
		case(U_OF_M_SPECT_N_SETUP_GRATING_TYPE + 1) : COPY_PART_NAME(.SETUP:GRATING_TYPE) break;
		case(U_OF_M_SPECT_N_SETUP_EXPOSURE + 1) : COPY_PART_NAME(.SETUP:EXPOSURE) break;
		case(U_OF_M_SPECT_N_SETUP_FILTER + 1) : COPY_PART_NAME(.SETUP:FILTER) break;
		case(U_OF_M_SPECT_N_WAVE_LENGTH + 1) : COPY_PART_NAME(:WAVE_LENGTH) break;
		case(U_OF_M_SPECT_N_SLIT_WIDTH + 1) : COPY_PART_NAME(:SLIT_WIDTH) break;
		case(U_OF_M_SPECT_N_GRATING_TYPE + 1) : COPY_PART_NAME(:GRATING_TYPE) break;
		case(U_OF_M_SPECT_N_EXPOSURE + 1) : COPY_PART_NAME(:EXPOSURE) break;
		case(U_OF_M_SPECT_N_FILTER + 1) : COPY_PART_NAME(:FILTER) break;
		case(U_OF_M_SPECT_N_SPECTRA + 1) : COPY_PART_NAME(:SPECTRA) break;
		case(U_OF_M_SPECT_N_ORDER_SORTER + 1) : COPY_PART_NAME(:ORDER_SORTER) break;
		case(U_OF_M_SPECT_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(U_OF_M_SPECT_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
