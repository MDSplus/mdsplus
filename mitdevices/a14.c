...
typedef struct { unsigned clock_speed : 3;
...
int a14___init(struct descriptor *niddsc, InInitStruct *setup)
{
  ...
  {
    case 2:
    case 1: pts = (setup->pts + 2047) / 2048; break;
    default: pts = (setup->pts + 1023) / 1024; break;
  }
                       ...
                       short *buffer,int *samples_read)
{
  ...
  *samples_read = 0;
  ...
  while (status & 1 && *samples_read < samples)
  {
    ...
    int smps;
    if (samples_to_read > 1 && ( ((address + samples_to_read) & 0xff) == 0xff)) samples_to_read--;
    pio(17,2,address);
...
/*                           0-2.5     0-5      0-5       0-10    +/- 2.5   +/- 5     +/- 5     +/- 10     */
  static float coeffs[] =  {2.5/4096, 5./4096, 5./4096, 10./4096, 5./4096, 10./4096, 10./4096, 20./4096};
  ...
  switch (sr.mode)
  {
    ...
    case 4:
	     min_idx = 0;
             ...
             start_addr = 0;
             break;
    case 1:  min_idx = pts-memsize_idx;
             max_idx = pts-1;
  ...
  for (i=0;i<6 && status & 1;i++, range = range >> 3)
  {
    ...
    if (TreeIsOn(chan_nid) & 1)
    {
      ...
      {
        int samples_read;
        status = ReadChannel(setup, start_addr,raw.bounds[0].l,raw.m[0],i,(short *)raw.pointer,&samples_read);
        if (status & 1)
        {
