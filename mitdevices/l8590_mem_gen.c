...
int l8590_mem__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(L8590_MEM_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(L8590_MEM_N_NAME + 1) : COPY_PART_NAME(:NAME) break;
		case(L8590_MEM_N_COMMENT + 1) : COPY_PART_NAME(:COMMENT) break;
		case(L8590_MEM_N_LATCH + 1) : COPY_PART_NAME(:LATCH) break;
		case(L8590_MEM_N_INIT_ACTION + 1) : COPY_PART_NAME(:INIT_ACTION) break;
		case(L8590_MEM_N_STORE_ACTION + 1) : COPY_PART_NAME(:STORE_ACTION) break;
