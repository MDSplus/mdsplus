...
int dc1394Init(int cam, int width, int height, int max_frames, int trigger_mode, int shutter, int gain, int trig_on, int mode, int frame_rate, int debug)
{
  ...
  if (dc1394_dma_setup_capture(Cameras[cam].handle,camera_nodes[0],
			       0,
				FORMAT_VGA_NONCOMPRESSED+((mode==101) || (mode==103)),
				mode,
				SPEED_400,
				frame_rate,
				3, /* frames */
...
static void *AcquireFrames(void *arg) 
{
  ...
  for (Cameras[cam].next_frame=0; Cameras[cam].next_frame < Cameras[cam].max_frames; Cameras[cam].next_frame++) {
    ...
    memcpy((void *)Cameras[cam].frames+Cameras[cam].next_frame*Cameras[cam].width*Cameras[cam].height*Cameras[cam].bpp, Cameras[cam].camera.capture_buffer, Cameras[cam].width*Cameras[cam].height*Cameras[cam].bpp);
...
int dc1394ReadFrames(int cam, char *data) {
  ...
  if (CheckCamBuf(Cameras[cam].frames, Cameras[cam].width*Cameras[cam].height*Cameras[cam].max_frames*Cameras[cam].bpp) != 0) {
  ...
  if (Cameras[cam].frames != NULL)
    if (Cameras[cam].bpp == 2) {
      swab(Cameras[cam].frames, data, Cameras[cam].width*Cameras[cam].height*Cameras[cam].next_frame*Cameras[cam].bpp);
    }
    else {
      memmove(data, Cameras[cam].frames, Cameras[cam].width*Cameras[cam].height*Cameras[cam].next_frame*Cameras[cam].bpp);
    }
  else
