...
#define max(a,b) ((a) >= (b)) ? (a) : (b)
  ...
  {
    int input_nid = setup->head_nid + INCAA16_N_INPUT_01 + chan * (INCAA16_N_INPUT_02 - INCAA16_N_INPUT_01);
    ...
    if (TreeIsOn(input_nid) & 1)
    {
      ...
      if (raw.m[0] > 0)
      {
        ...
        for (samples_to_read = raw.m[0], data_ptr=(short *)raw.pointer,
             addr = (unsigned int)(start_addr + chan + (raw.bounds[0].l - min_idx) * active_chans) % memsize;
