...
int hv4032a1__add(struct descriptor *name_d_ptr, struct descriptor *dummy_d_ptr, int *nid_ptr)
{
...
int hv4032a1__part_name(struct descriptor *nid_d_ptr, struct descriptor *method_d_ptr, struct descriptor *out_d)
{
	...
	status = TreeGetNci(*(int *)nid_d_ptr->pointer, nci_list);
		if (!(status & 1)) return status;
	switch(element)  {
		case(HV4032A1_N_HEAD + 1) : StrFree1Dx(out_d); break;
		case(HV4032A1_N_VOLTAGE_1 + 1) : COPY_PART_NAME(:VOLTAGE_1) break;
		case(HV4032A1_N_VOLTAGE_2 + 1) : COPY_PART_NAME(:VOLTAGE_2) break;
		case(HV4032A1_N_VOLTAGE_3 + 1) : COPY_PART_NAME(:VOLTAGE_3) break;
		case(HV4032A1_N_VOLTAGE_4 + 1) : COPY_PART_NAME(:VOLTAGE_4) break;
...
int hv4032a1___get_settings(struct descriptor *nid_d_ptr, InGet_settingsStruct *in_ptr)
{
