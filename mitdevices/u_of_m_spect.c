...
int u_of_m_spect___init(struct descriptor *niddsc_ptr, InInitStruct *setup)
{
  ...
  if (file)
  {
    fprintf(file,"%d\n%d\n%d\n%d\n%d\n%d\n\"Y\"\n", setup->setup_wave_length,setup->setup_slit_width,setup->setup_num_spectra,
            setup->setup_grating_type,((setup->setup_exposure+15)/30) * 30,setup->setup_filter);
    fclose(file);
    strcpy(cmd,"rm ");
...
int u_of_m_spect___store(struct descriptor *niddsc_ptr, InStoreStruct *setup)
{
  ...
  {
    fscanf(file, "%d %d %d %d %d %d %d\n", &wavelength, &slit, &num_frames, &grating, &exposure, &filter, &np);
    dt = exposure/1000.;
    ...
    for(frame=0; frame<num_frames; frame++) {
      ...
      for (row=0; row<rows; row++) {
        ...
        fgets(line, 131, file);
        sscanf(line, "%d %d %d %d %d %d %d %d %d %d", &buffer[offset], &buffer[offset+1], &buffer[offset+2], 
...
static int StoreSpectra(int head_nid, int np, int num_frames, int *buffer, int p1, float dt)
{
  ...
  static ARRAY_BOUNDS(int, 2) array = {sizeof(int), DTYPE_L, CLASS_A, 0, 0, 0, {0,0,1,1,1}, 2}; 
  ...
  return_on_error(TdiCompile(&dim, &zero_dsc, &num_frames_dsc, &trigger_dsc, &dt_dsc, &second_dim MDS_END_ARG), status);
