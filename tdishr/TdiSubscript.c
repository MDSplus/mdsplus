...
int Tdi1Subscript(int opcode, int narg, struct descriptor *list[],
                  struct descriptor_xd *out_ptr)
{
    ...
    if (!(status & 1)) {
        if (dat[0].pointer && dat[0].pointer->dtype == DTYPE_DICTIONARY) {
            ...
            for (idx = 0; idx < ((apd->arsize / apd->length) - 1); idx += 2) {
                if ((TdiEq(apd->pointer[idx], list[1], &ans MDS_END_ARG) & 1) ==
                    1 && check == 1) {
    ...
    for (dim = 0; dim < dimct; ++dim) {
        ...
        if (status & 1) {
            ...
            if (psig && dim < psig->ndesc - 2 &&
                ...
                if (status & 1)
                    status =
                        TdiXtoI(pdim, xx[dim].pointer, &ii[dim] MDS_END_ARG);
                if (status & 1)
                    status =
                        TdiItoX(pdim, ii[dim].pointer, &xx[dim] MDS_END_ARG);
                if (status & 1) {
                    EMPTYXD(xd);
                    ...
                    if (tmp_status & 1) {
                        ...
                        if (pwin && pwin->startidx)
                            status =
                                TdiSubtract(&ii[dim], pwin->startidx,
                                            &ii[dim] MDS_END_ARG);
                    }
                    MdsFree1Dx(&xd, NULL);
                }
                ...
                highdim = dim + 1;
                       ...
                       && list[dim + 1]) {
                status =
                    TdiCull(pdat, &ddim, list[dim + 1], &xx[dim] MDS_END_ARG);
                if (status & 1)
                ...
                if (dim + 1 < narg && list[dim + 1]) {
                    ...
                    TdiThreadStatic()->TdiRANGE_PTRS[0] =
                        (struct descriptor *)&dleft;
                    TdiThreadStatic()->TdiRANGE_PTRS[1] =
                        (struct descriptor *)&dright;
                    ...
                    status =
                        TdiDtypeRange(&dleft, &dright, &ii[dim] MDS_END_ARG);
            }
        }
    ...
    if (highest <= 0) {
        ...
        status =
            MdsGet1DxA((struct descriptor_a *)&arr, &pdat->length, &pdat->dtype,
                       out_ptr);
        if (!(status & 1))
            goto badsub;
        if (arr.arsize == 0)
 ...
 inner:for (j = 0; j < row; j += sizeof(int))
            _MOVC3(len, pin + len * *(int *)((char *)px[0] + j), pout), pout +=
                len;
                /******************************************************
        ...
        for (j = 0; ++j < highest;) {
            if (++count[j] < ((int)(arr.m[j]))) {
                pin +=
                    stride[j] * (*(px[j] + count[j]) - *(px[j] + count[j] - 1));
                goto inner;
            }
...
*/
int Tdi1Map(int opcode, int narg, struct descriptor *list[],
            struct descriptor_xd *out_ptr)
{
    int status = 1;
