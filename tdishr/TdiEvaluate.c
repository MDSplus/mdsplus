...
int Tdi1Evaluate(int opcode, int narg, struct descriptor *list[],
	...
	*****************************/
	...
	case DTYPE_DSC:
		...
		default:
		    if (out_ptr->l_length &&
			(out_ptr->pointer !=
			 (struct descriptor *)list[0]->pointer))
...
*****************************************************************************/
	...
	case DTYPE_CALL:
	    pfun = (struct descriptor_function *)list[0];
	    status =
		TdiCall(pfun->length ? *(unsigned char *)pfun->
			pointer : DTYPE_L, pfun->ndesc, pfun->arguments,
			out_ptr);
	    break;
	default:
			/***********************
