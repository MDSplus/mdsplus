...
int Tdi1Compile(int opcode, int narg, struct descriptor *list[],
                struct descriptor_xd *out_ptr)
{
    ...
    if (TdiThreadStatic()->compiler_recursing == 1) {
        fprintf(stderr,
                "Error: Recursive calls to TDI Compile is not supported");
        return 0;
    }
    ...
    if (status & 1) {
        if (text_ptr->length > 0) {
            ...
            if (!TdiRefZone.l_zone)
                status =
                    LibCreateVmZone(&TdiRefZone.l_zone, 0, 0, 0, 0, 0, 0, 0, 0,
                                    0, &compile_zone);
        ...
        ****************************************/
            ...
            TdiRefZone.a_begin = TdiRefZone.a_cur =
                memcpy(malloc(text_ptr->length), text_ptr->pointer,
                       text_ptr->length);
        ...
        ************************/
            if (status & 1) {
                if (TdiRefZone.a_result == 0)
                    ...
                    status =
                        MdsCopyDxXd((struct descriptor *)TdiRefZone.a_result,
                                    out_ptr);
            }
...
*/
int Tdi1Execute(int opcode, int narg, struct descriptor *list[],
                struct descriptor_xd *out_ptr)
{
    int status = 1;
