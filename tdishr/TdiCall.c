...
int TdiCall(int opcode, int narg, struct descriptor *list[],
            struct descriptor_xd *out_ptr)
{
        ...
        if (pfun && pfun->dtype == DTYPE_FUNCTION) {
            ...
            if (code == OpcDescr) {
                ...
                if (status & 1 && xd.pointer
                    && xd.pointer->dtype == DTYPE_POINTER) {
 ...
 fort:      tmp[ntmp] = EMPTY_XD;
    ...
    if (status & 1)
        status =
            TdiInterlude(opcode, newdsc, routine, (void *)LibCallg,
                         (void **)result, &max);
    if (!out_ptr)
        goto skip;
    if (status & 1)
