...
STATIC_CONSTANT DESCRIPTOR(compile_err,
...
***************************************************/
int TdiTrace(int opcode, int narg, struct descriptor *list[],
...
int TRACE(int opcode, int narg, struct descriptor *list[],
          struct descriptor_xd *out_ptr)
{
    int status = 1;
    int j;
...
int TdiIntrinsic(int opcode, int narg, struct descriptor *list[],
                 struct descriptor_xd *out_ptr)
{
    int status = 1;
    ...
    if (status & 1
        || status == TdiBREAK
        || status == TdiCONTINUE || status == TdiGOTO || status == TdiRETURN) {
    ...
    if (opcode == OpcCompile && message->length < MAXMESS) {
        char *cur =
            MINMAX(TdiRefZone.a_begin, TdiRefZone.a_cur, TdiRefZone.a_end);
...
*/
int Tdi1Debug(int opcode, int narg, struct descriptor *list[],
              struct descriptor_xd *out_ptr)
{
    int status = 1;
    int option = -1;
    ...
    if (option & 1 && mess_stat != 1) {
        ...
        StrConcat((struct descriptor *)message, &dmsg, &newline,
                  message MDS_END_ARG);
    }
    if (message->length) {
        if (option & 2)
...
STATIC_ROUTINE struct descriptor *FixedArray(struct descriptor *in)
{
    ...
    int dsize =
        sizeof(struct descriptor_a) + sizeof(int) + 3 * sizeof(int) * a->dimct;
    int i;
