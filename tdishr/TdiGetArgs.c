...
{
    STATIC_CONSTANT unsigned char omitsu[] =
	{ DTYPE_SIGNAL, DTYPE_WITH_UNITS, 0 };
	...
	case DTYPE_SIGNAL:
	    ...
	    *data_ptr = EMPTY_XD;
		...
		(struct descriptor_xd *)signal_ptr->pointer;
	    status =
		TdiGetData(omitu,
			   ((struct descriptor_signal *)signal_ptr->pointer)->
			   data, data_ptr);
	    if (status & 1)
		switch (data_ptr->pointer->dtype) {
		case DTYPE_WITH_UNITS:
		    ...
		    *data_ptr = EMPTY_XD;
		    status =
			TdiData(((struct descriptor_with_units *)tmp.pointer)->
				units, units_ptr MDS_END_ARG);
		    if (status & 1)
			status =
			    TdiData(((struct descriptor_with_units *)tmp.
				     pointer)->data, data_ptr MDS_END_ARG);
		    MdsFree1Dx(&tmp, NULL);
		    break;
		default:
...
void UseNativeFloat(struct TdiCatStruct *cat)
{
	...
	if (((TdiREF_CAT[k].cat & ~(0x800)) == (cat->out_cat & ~(0x800))) &&
	    (k == DTYPE_NATIVE_FLOAT ||
	     ...
	     k == DTYPE_FLOAT_COMPLEX || k == DTYPE_DOUBLE_COMPLEX)) {
	    cat->out_dtype = k;
	       ...
	       struct descriptor_xd dat[], struct TdiCatStruct cats[])
{
	...
	******************************/
	...
	for (nd = 0, nc = 0, j = 0; j < narg; ++j) {
	    ...
	    cptr->out_cat =
		(unsigned short)(((cptr->in_cat = TdiREF_CAT[jd].cat) | i1) &
				 i2);
	    if (jd != DTYPE_MISSING)
		nc |= cptr->out_cat;
