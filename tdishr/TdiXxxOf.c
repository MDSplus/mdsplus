...
*/
int Tdi1ArgOf(int opcode, int narg, struct descriptor *list[],
	      struct descriptor_xd *out_ptr)
{
	...
	case DTYPE_CALL:
		...
		status =
		    MdsCopyDxXd(((struct descriptor_call *)tmp.pointer)->
				arguments[iarg], out_ptr);
	    break;
	case DTYPE_CONDITION:
	    status =
		MdsCopyDxXd(((struct descriptor_condition *)tmp.pointer)->
			    condition, out_ptr);
	    break;
	case DTYPE_DEPENDENCY:
	    status =
		MdsCopyDxXd(((struct descriptor_dependency *)tmp.pointer)->
			    arguments[iarg], out_ptr);
	    break;
	case DTYPE_FUNCTION:
	    status =
		MdsCopyDxXd(((struct descriptor_function *)tmp.pointer)->
			    arguments[iarg], out_ptr);
	    break;
	case DTYPE_METHOD:
	    status =
		MdsCopyDxXd(((struct descriptor_method *)tmp.pointer)->
			    arguments[iarg], out_ptr);
	    break;
	case DTYPE_PROCEDURE:
	    status =
		MdsCopyDxXd(((struct descriptor_procedure *)tmp.pointer)->
			    arguments[iarg], out_ptr);
	    break;
	case DTYPE_ROUTINE:
	    status =
		MdsCopyDxXd(((struct descriptor_routine *)tmp.pointer)->
			    arguments[iarg], out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
*/
int Tdi1AxisOf(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_DIMENSION:
	    status =
		MdsCopyDxXd(((struct descriptor_dimension *)tmp.pointer)->axis,
			    out_ptr);
	    break;
	case DTYPE_SLOPE:
	case DTYPE_RANGE:
...
*/
int Tdi1BeginOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_RANGE:
		...
		status =
		    MdsCopyDxXd(((struct descriptor_range *)tmp.pointer)->begin,
				out_ptr);
	    break;
	case DTYPE_SLOPE:
	    if (narg > 1)
		...
		status = TdiBAD_INDEX;
	    if (status & 1)
		status = MdsCopyDxXd(((struct descriptor_slope *)tmp.pointer)->
				     segment[n].begin, out_ptr);
	    break;
	case DTYPE_WINDOW:
	    if (narg > 1)
		status = TdiEXTRA_ARG;
		...
		status =
		    MdsCopyDxXd(((struct descriptor_window *)tmp.pointer)->
				startidx, out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
/*--------------------------------------------------------------
	Return the class of its argument, but not of the XD.
		byte = CLASS_OF(any)
*/
int Tdi1ClassOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
...
/*--------------------------------------------------------------
	Evaluate and then return the class of an argument.
		byte = CLASS(any)
*/
int Tdi1Class(int opcode, int narg, struct descriptor *list[],
	      struct descriptor_xd *out_ptr)
{
    int status = 1;
...
*/
int Tdi1CompletionOf(int opcode, int narg, struct descriptor *list[],
		     struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
    ...
    if (status & 1)
	status =
	    MdsCopyDxXd(((struct descriptor_dispatch *)tmp.pointer)->completion,
			out_ptr);
    MdsFree1Dx(&tmp, NULL);
    return status;
}
			    ...
			    struct descriptor_xd *out_ptr)
{
	...
	case DTYPE_ACTION:
	    status =
		MdsCopyDxXd(((struct descriptor_action *)tmp.pointer)->
			    completion_message, out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
	    break;
...
*/
int Tdi1ConditionOf(int opcode, int narg, struct descriptor *list[],
		    struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
...
*/
int Tdi1DimOf(int opcode, int narg, struct descriptor *list[],
	      struct descriptor_xd *out_ptr)
{
    int status = 1;
    array_bounds *pa;
	...
	**************************************/
	...
	case CLASS_R:
		...
		if (index > ((struct descriptor_signal *)pa)->ndesc - 3
		    || ((struct descriptor_signal *)pa)->dimensions[index] ==
		    0) {
		    struct descriptor_s index_dsc =
			{ sizeof(int), DTYPE_L, CLASS_S, 0 };
		    index_dsc.pointer = (char *)&index;
		    status = TdiGetData(&omits[1], &tmp, &tmp);
		    if (status & 1)
			status =
			    TdiDimOf(&tmp, &index_dsc, out_ptr MDS_END_ARG);
		} else
		    status =
			MdsCopyDxXd(((struct descriptor_signal *)pa)->
				    dimensions[index], out_ptr);
		break;
	    default:
		status = TdiINVDTYDSC;
		break;
...
*/
int Tdi1DispatchOf(int opcode, int narg, struct descriptor *list[],
		   struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_ACTION:
	    status =
		TdiDispatchOf(((struct descriptor_action *)tmp.pointer)->
			      dispatch, out_ptr MDS_END_ARG);
	    break;
	case DTYPE_DISPATCH:
	    MdsFree1Dx(out_ptr, NULL);
	    *out_ptr = tmp;
...
*/
int Tdi1DscptrOf(int opcode, int narg, struct descriptor *list[],
		 struct descriptor_xd *out_ptr)
{
    int status = 1;
...
/*--------------------------------------------------------------
	Evaluate and then return a descriptor pointer of an argument.
		byte = DSCPTR(any)
*/
int Tdi1Dscptr(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
...
/*--------------------------------------------------------------
	Return the data type of its argument, but not of the XD.
		byte = KIND_OF(any)
*/
int Tdi1KindOf(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor *px = list[0];
...
/*--------------------------------------------------------------
	Evaluate and then return the dtype of an argument.
		byte = KIND(any)
*/
int Tdi1Kind(int opcode, int narg, struct descriptor *list[],
	     struct descriptor_xd *out_ptr)
{
    int status = 1;
...
*/
int Tdi1EndOf(int opcode, int narg, struct descriptor *list[],
	      struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_RANGE:
		...
		status =
		    MdsCopyDxXd(((struct descriptor_range *)tmp.pointer)->
				ending, out_ptr);
	    break;
	case DTYPE_SLOPE:
	    if (narg > 1)
...
*/
int Tdi1ErrorOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
	...
	case DTYPE_WITH_ERROR:
	    status =
		MdsCopyDxXd(((struct descriptor_with_error *)tmp.pointer)->
			    error, out_ptr);
	    break;
	default:
...
*/
int Tdi1ErrorlogsOf(int opcode, int narg, struct descriptor *list[],
		    struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_ACTION:
	    {
		struct descriptor_action *act =
		    (struct descriptor_action *)tmp.pointer;
...
*/
int Tdi1HelpOf(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_PARAM:
	    status =
		MdsCopyDxXd(((struct descriptor_param *)tmp.pointer)->help,
			    out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
*/
int Tdi1IdentOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
    ...
    if (status & 1)
	status =
	    MdsCopyDxXd(((struct descriptor_dispatch *)tmp.pointer)->ident,
			out_ptr);
    MdsFree1Dx(&tmp, NULL);
    return status;
}
...
*/
int Tdi1ImageOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_CALL:
	    status =
		MdsCopyDxXd(((struct descriptor_call *)tmp.pointer)->image,
			    out_ptr);
	    break;
	case DTYPE_CONGLOM:
	    status =
		MdsCopyDxXd(((struct descriptor_conglom *)tmp.pointer)->image,
			    out_ptr);
	    break;
	case DTYPE_ROUTINE:
	    status =
		MdsCopyDxXd(((struct descriptor_routine *)tmp.pointer)->image,
			    out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
*/
int Tdi1InterruptOf(int opcode, int narg, struct descriptor *list[],
		    struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
...
*/
int Tdi1LanguageOf(int opcode, int narg, struct descriptor *list[],
		   struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
...
*/
int Tdi1MethodOf(int opcode, int narg, struct descriptor *list[],
		 struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_METHOD:
	    status =
		MdsCopyDxXd(((struct descriptor_method *)tmp.pointer)->method,
			    out_ptr);
	    break;
	case DTYPE_OPAQUE:
	    status =
		MdsCopyDxXd(((struct descriptor_opaque *)tmp.pointer)->
			    opaque_type, out_ptr);
	default:
	    status = TdiINVDTYDSC;
	    break;
...
*/
int Tdi1ModelOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_CONGLOM:
	    status =
		MdsCopyDxXd(((struct descriptor_conglom *)tmp.pointer)->model,
			    out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
*/
int Tdi1NameOf(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_CONGLOM:
	    status =
		MdsCopyDxXd(((struct descriptor_conglom *)tmp.pointer)->name,
			    out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
*/
int Tdi1NdescOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_r *pr = (struct descriptor_r *)list[0];
...
/*--------------------------------------------------------------
	...
	WARNING, will not work for DTYPE_FUNCTION.
		byte = NDESC(any)
*/
int Tdi1Ndesc(int opcode, int narg, struct descriptor *list[],
	      struct descriptor_xd *out_ptr)
{
    int status = 1;
...
*/
int Tdi1ObjectOf(int opcode, int narg, struct descriptor *list[],
		 struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_METHOD:
	    status =
		MdsCopyDxXd(((struct descriptor_method *)tmp.pointer)->object,
			    out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
*/
int Tdi1PhaseOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
    ...
    if (status & 1)
	status =
	    MdsCopyDxXd(((struct descriptor_dispatch *)tmp.pointer)->phase,
			out_ptr);
    MdsFree1Dx(&tmp, NULL);
    return status;
}
...
*/
int Tdi1ProcedureOf(int opcode, int narg, struct descriptor *list[],
		    struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
...
*/
int Tdi1ProgramOf(int opcode, int narg, struct descriptor *list[],
		  struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_PROGRAM:
	    status =
		MdsCopyDxXd(((struct descriptor_program *)tmp.pointer)->program,
			    out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
*/
int Tdi1QualifiersOf(int opcode, int narg, struct descriptor *list[],
		     struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_CONGLOM:
	    status =
		MdsCopyDxXd(((struct descriptor_conglom *)pd)->qualifiers,
			    out_ptr);
	    break;
	case DTYPE_CALL:
	case DTYPE_CONDITION:
...
/*--------------------------------------------------------------
	Return raw data portion of a SIGNAL.
		...
		data = RAW_OF(other)
*/
int Tdi1RawOf(int opcode, int narg, struct descriptor *list[],
	      struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_SIGNAL:
	    status =
		MdsCopyDxXd(((struct descriptor_signal *)tmp.pointer)->raw,
			    out_ptr);
	    break;
	default:
	    MdsFree1Dx(out_ptr, NULL);
...
*/
int Tdi1RoutineOf(int opcode, int narg, struct descriptor *list[],
		  struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_CALL:
	    status =
		MdsCopyDxXd(((struct descriptor_call *)tmp.pointer)->routine,
			    out_ptr);
	    break;
	case DTYPE_ROUTINE:
	    status =
		MdsCopyDxXd(((struct descriptor_routine *)tmp.pointer)->routine,
			    out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
*/
int Tdi1SlopeOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
	...
	case DTYPE_RANGE:
		...
		MdsCopyDxXd(((struct descriptor_r *)tmp.pointer)->dscptrs[2],
			    out_ptr);
	    break;
	case DTYPE_SLOPE:
...
/*--------------------------------------------------------------
	Return taskule information.
		...
		same = TASK_OF(method)
*/
int Tdi1TaskOf(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_ACTION:
	    status =
		TdiTaskOf(((struct descriptor_action *)tmp.pointer)->task,
			  out_ptr MDS_END_ARG);
	    break;
	case DTYPE_METHOD:
	case DTYPE_PROCEDURE:
...
*/
int Tdi1TimeoutOf(int opcode, int narg, struct descriptor *list[],
		  struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_METHOD:
	    status =
		MdsCopyDxXd(((struct descriptor_method *)tmp.pointer)->time_out,
			    out_ptr);
	    break;
	case DTYPE_PROCEDURE:
	    status =
	...
	case DTYPE_PROGRAM:
	    status =
		MdsCopyDxXd(((struct descriptor_program *)tmp.pointer)->
			    time_out, out_ptr);
	    break;
	case DTYPE_ROUTINE:
	    status =
...
*/
int Tdi1UnitsOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    STATIC_CONSTANT DESCRIPTOR(none, " ");
	...
	case DTYPE_WITH_UNITS:
	    status =
		MdsCopyDxXd(((struct descriptor_with_units *)tmp.pointer)->
			    units, out_ptr);
	    break;
	default:
	    status = MdsCopyDxXd((struct descriptor *)&none, out_ptr);
...
*/
int Tdi1ValidationOf(int opcode, int narg, struct descriptor *list[],
		     struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_PARAM:
	    status =
		MdsCopyDxXd(((struct descriptor_param *)tmp.pointer)->
			    validation, out_ptr);
	    break;
	default:
	    status = TdiINVDTYDSC;
...
/*--------------------------------------------------------------
	Return data portion of signal or parameter.
		...
		data = VALUE_OF(other)	
*/
int Tdi1ValueOf(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_DIMENSION:
	    status =
		TdiValueOf(((struct descriptor_dimension *)tmp.pointer)->window,
			   out_ptr MDS_END_ARG);
	    break;
	case DTYPE_PARAM:
	    status =
		MdsCopyDxXd(((struct descriptor_param *)tmp.pointer)->value,
			    out_ptr);
	    break;
	case DTYPE_SIGNAL:
	    status =
		MdsCopyDxXd(((struct descriptor_signal *)tmp.pointer)->data,
			    out_ptr);
	    break;
	case DTYPE_WINDOW:
	    status =
		MdsCopyDxXd(((struct descriptor_window *)tmp.pointer)->
			    value_at_idx0, out_ptr);
	    break;
	case DTYPE_WITH_UNITS:
	    status =
		MdsCopyDxXd(((struct descriptor_with_units *)tmp.pointer)->data,
			    out_ptr);
	    break;
	case DTYPE_OPAQUE:
	    status =
		MdsCopyDxXd(((struct descriptor_opaque *)tmp.pointer)->data,
			    out_ptr);
	default:
	    MdsFree1Dx(out_ptr, NULL);
	    *out_ptr = tmp;
...
*/
int Tdi1WhenOf(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
    ...
    if (status & 1)
	status =
	    MdsCopyDxXd(((struct descriptor_dispatch *)tmp.pointer)->when,
			out_ptr);
    MdsFree1Dx(&tmp, NULL);
    return status;
}
...
*/
int Tdi1WindowOf(int opcode, int narg, struct descriptor *list[],
		 struct descriptor_xd *out_ptr)
{
    int status = 1;
    struct descriptor_xd tmp = EMPTY_XD;
	...
	case DTYPE_DIMENSION:
	    status =
		MdsCopyDxXd(((struct descriptor_dimension *)tmp.pointer)->
			    window, out_ptr);
	    break;
	case DTYPE_WINDOW:
	    MdsFree1Dx(out_ptr, NULL);
