...
int Tdi1Vector(int opcode, int narg, struct descriptor *list[],
               struct descriptor_xd *out_ptr)
{
    int status = 1;
    array miss =
        { sizeof(char), DTYPE_MISSING, CLASS_A, (char *)0, 0, 0, {0, 1, 1, 0,
                                                                  0}, 1, 0 };
    array_coeff arr =
        { sizeof(char), DTYPE_BU, CLASS_A, (char *)0, 0, 0, {0, 1, 1, 1, 0},
        MAXDIM, 0 };
    struct descriptor_xd (*psig)[], (*puni)[] = 0, (*pdat)[] = 0;
    struct TdiCatStruct (*pcats)[] = 0;
    int cmode = -1, j, n, (*pnelem)[] = 0, jd, mind = MAXDIM, maxd = 0, nmiss =
        0;
    int virt =
    ...
    if (status & 1)
        status =
            TdiGetArgs(opcode, narg, list, (*psig), (*puni), (*pdat), (*pcats));
