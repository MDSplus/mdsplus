...
int Tdi1Bound(int opcode, int narg, struct descriptor *list[],
	      struct descriptor_xd *out_ptr)
{
	...
	if (status & 1)
	    status = MdsGet1DxA((struct descriptor_a *)&adsc, &size_l,
				&dtype_l, out_ptr);
    }
    if (status & 1 && rank > 0)
...
*/
int Tdi1Ebound(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
    array_bounds *pa = 0;
	...
	if (status & 1)
	    status =
		(*TdiRefFunction[opcode].f3) (sig[0].pointer, pa, dim, out_ptr);
    } else {
	if (status & 1 && rank > MAXDIM)
	    status = TdiNDIM_OVER;
	else {
	    for (dim = 0; status & 1 && dim < rank; ++dim) {
		outs[dim] = EMPTY_XD;
		status =
		    (*TdiRefFunction[opcode].f3) (sig[0].pointer, pa, dim,
						  &outs[dim]);
		new[dim] = (struct descriptor *)&outs[dim];
	    }
	    if (status & 1)
		status = TdiIntrinsic(OpcVector, rank, new, out_ptr);
	    if (opcode == OpcEsize && status & 1)
		...
		array_bounds * pa, int dim, struct descriptor_xd *pout)
{
	...
	status =
	    TdiItoX(psig->dimensions[dim], TdiItoXSpecial, pout MDS_END_ARG);
	if (status & 1) {
