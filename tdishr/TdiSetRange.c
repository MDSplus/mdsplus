		...
		***************************/
		if (status & 1 && (defhi || deflo)) {
			if (j >= ndim) status = TdiINVDTYDSC;
				...
				if (deflo) {
					if (bounds) lo = pa->m[dimct + 2*j];
					else lo = 0;
				}
				if (coeff) hi = pa->m[j] - 1 + lo;
				else hi = (int)pa->arsize / (int)pa->length - 1 + lo;
			}
			else {
				if (coeff) lo = hi + 1 - pa->m[j];
				else lo = hi + 1 - (int)pa->arsize / (int)pa->length;
			}
		}
