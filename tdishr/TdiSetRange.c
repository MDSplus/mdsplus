...
int Tdi1SetRange(int opcode, int narg, struct descriptor *list[],
                 struct descriptor_xd *out_ptr)
{
    ...
    int bounds = 0, coeff = 0, defhi, deflo, dimct, hi, j, lo, ndim = 0, cmode =
        0;
    ...
    if (status & 1)
        status =
            MdsGet1DxA((struct descriptor_a *)&arr, &pa->length, &pa->dtype,
                       out_ptr);
