...
int Tdi1Same(int opcode, int narg, struct descriptor *list[],
	     struct descriptor_xd *out_ptr)
{
	...
	******************************************/
    if (status & 1)
	status =
	    (*fun_ptr->f2) (narg, uni, dat, cats, &routine, fun_ptr->o1,
			    fun_ptr->o2);
	...
	******************/
    if (status & 1)
	status =
	    TdiGetShape(narg, dat, cats[narg].digits, cats[narg].out_dtype,
			&cmode, out_ptr);
	...
	********************************/
	...
	if (routine == &Tdi3undef || routine == 0) {
	    ...
	    *out_ptr = dat[0];
	    ...
	    case 2:
		status =
		    (*routine) (dat[0].pointer, dat[1].pointer,
				out_ptr->pointer);
		break;
	    case 3:
		status =
		    (*routine) (dat[0].pointer, dat[1].pointer, dat[2].pointer,
				out_ptr->pointer);
		break;
	    }
