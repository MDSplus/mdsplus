...
int Tdi1Decompile(int opcode, int narg, struct descriptor *list[],
...
STATIC_ROUTINE int tdi_vector(struct descriptor *in_ptr,
                              int level,
                              char **item_ptr_ptr, struct descriptor_d *out_ptr)
{
    ...
    if (level > 0)
        for (j = n; --j >= 0 && status & 1;) {
            ...
            if (j > 0 && status & 1)
                status = StrAppend(out_ptr, &COMMA_SPACE);
    } else {
        ...
        for (j = 0; status & 1 && j < n; pitem += length) {
            ...
            if (++j < n && status & 1)
                status = StrAppend(out_ptr, &COMMA);
        }
        *item_ptr_ptr = pitem;
    }
    if (status & 1)
        status = StrAppend(out_ptr, &RIGHT_BRACKET);
    return status;
}
...
STATIC_ROUTINE int noblanks(struct descriptor *cdsc_ptr)
{
    ...
    return 1;
        ...
        *****************************/
STATIC_ROUTINE int closeup(char repl,
                           struct descriptor *pfloat, struct descriptor *pdc)
{
...
int Tdi0Decompile(struct descriptor *in_ptr,
                  int prec, struct descriptor_d *out_ptr)
{
    ...
    case CLASS_D:
        ...
        case DTYPE_T:
            {
                ...
                for (j = in_ptr->length; --j >= 0; cptr++) {
                    ...
                    if (status & 1
                        && (t2.length =
                            (unsigned short)(cptr - t2.pointer)) > 0)
                        status = StrAppend(out_ptr, &t2);
                    t2.pointer = cptr + 1;
                    c0[0] = '\\';
                    if (status & 1)
                        status = StrAppend(out_ptr, &cdsc);
                    cdsc.length = 2;
                }
                if (status & 1
                    && (t2.length = (unsigned short)(cptr - t2.pointer)) > 0)
                    status = StrAppend(out_ptr, &t2);
                if (status & 1)
                    status = StrAppend(out_ptr, &QUOTE);
                break;
            ...
            if (status & 1) {
                ...
                status =
                    StrConcat((struct descriptor *)out_ptr,
                              (struct descriptor *)out_ptr, &cdsc,
                              &sdsc MDS_END_ARG);
            }
            break;
        ...
        case DTYPE_OU:
            ...
            cdsc.length = (unsigned short)(cptr - cdsc.pointer);
            {
                ...
                status =
                    StrConcat((struct descriptor *)out_ptr,
                              (struct descriptor *)out_ptr, &HEX, &cdsc,
                              &sdsc MDS_END_ARG);
            }
            break;
        ...
        case DTYPE_FT:
            cdsc.length =
                (unsigned short)((in_ptr->length - 1) * 8 * .30103 + 6.8);
        ...
        case DTYPE_DSC:
            status =
                Tdi0Decompile((struct descriptor *)in_ptr->pointer, prec,
                              out_ptr);
            break;
        ...
        case DTYPE_MISSING:
            if (in_ptr->class != CLASS_XD)
                status = StrAppend(out_ptr, &MISSING);
            break;
        case DTYPE_IDENT:
            t2 = *in_ptr;
            t2.dtype = DTYPE_T;
        ...
        case DTYPE_PATH:
                        ...
                        *****************************************************/
            {
                ...
                if (status & 1) {
                    ...
                    status = StrAppend(out_ptr, in_ptr);
            }
            break;
        case DTYPE_EVENT:
            t2 = *in_ptr;
            ...
            status =
                StrConcat((struct descriptor *)out_ptr,
                          (struct descriptor *)out_ptr, &BUILD_EVENT, &t2,
                          &CLOSE_EVENT MDS_END_ARG);
            break;
        case DTYPE_POINTER:{
                char outstr[256];
                struct descriptor out = { 0, DTYPE_T, CLASS_S, outstr };
                out.length =
                    sprintf(outstr, "Pointer(%p)", *(void **)in_ptr->pointer);
    ...
    case CLASS_A:
        {
            ...
            if (a_ptr->aflags.bounds) {
                ...
                for (j = 0; j < dimct; ++j) {
                    ...
                    if (status & 1)
                        status = StrAppend(out_ptr, &COLON);
                    if (status & 1)
                        status =
                            TdiSingle(a_ptr->m[dimct + 2 * j + 1], out_ptr);
                    if (status & 1)
                        status = StrAppend(out_ptr, &COMMA);
                }
            }
                ...
                ******************************************/
                ...
                for (j = 0; j < dimct; ++j) {
                    if (status & 1)
                        status =
                            TdiSingle(coeff ? a_ptr->m[j] : count, out_ptr);
                    if (status & 1)
                        status = StrAppend(out_ptr, &COMMA);
                }
            }
            ...
            if (count > TdiDECOMPILE_MAX) {
                ...
                if (status & 1)
                    status = Tdi0Decompile(&one, P_ARG, out_ptr);
                if (status & 1)
                    status = StrAppend(out_ptr, &MORE);
            } else {
                char *pitem = (char *)a_ptr->pointer;
                status =
                    tdi_vector((struct descriptor *)a_ptr, dimct - 1, &pitem,
                               out_ptr);
            }
            if (more && status & 1)
                status = StrAppend(out_ptr, &RIGHT_PAREN);
            if (bptr && status & 1)
                status = StrAppend(out_ptr, &RIGHT_PAREN);
        }
        break;
