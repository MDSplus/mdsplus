	       ...
	       struct descriptor_xd dat[1], struct TdiCatStruct cats[1])
{
	...
	*************************/
	...
	if (cptr->out_cat != cptr->in_cat) {
		...
		if (TdiREF_CAT[cptr->out_dtype].cat != cptr->out_cat) {
			...
			for (jd = 0;
			     jd < TdiCAT_MAX
			     && TdiREF_CAT[jd].cat != cptr->out_cat; ++jd) {;
			}
			if (jd >= TdiCAT_MAX)
			    status = TdiINVDTYDSC;
			else
	...
	********************/
	...
	if (cptr->in_dtype == cptr->out_dtype
	    && cptr->digits == aptr->length) {;
	}
		...
		********************************************/
		...
		status =
		    TdiGetShape(1, &dat[j], cptr->digits, cptr->out_dtype,
				&cmode, &xd);
		if (status & 1)
