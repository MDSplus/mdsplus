...
int	n, status = 1;
	...
	N_ELEMENTS(px, n);
	if (status & 1) for (; --n >= 0; ++pi)
		if (*pi >= left && *pi <= right) *po++ = *pi;
	if (pi != po) {
		/********************************
		Scalars must be nulled elsewhere.
		********************************/
		if (px->class == CLASS_A) px->arsize = (char *)po - px->pointer;
		status = -1;
	}
	return status;
}
...
quadw *qptr;
	...
	case 1 : for (cptr = (char *)po; --n >= 0; pi += 1) if (*pm++) *cptr++ = *((char *)pi);  po = (char *)cptr; break;
	case 2 : for (sptr = (short *)po; --n >= 0; pi += 2) if (*pm++) *sptr++ = *((short *)pi); po = (char *)sptr; break;
	case 4 : for (lptr = (int *)po; --n >= 0; pi += 4) if (*pm++) *lptr++ = *((int *)pi);  po = (char *)lptr; break;
	case 8 : for (qptr = (quadw *)po; --n >= 0; pi += 8) if (*pm++) *qptr++ = *((quadw *)pi);  po = (char *)qptr; break;
	default : for (; --n >= 0; pi += len) if (*pm++) _MOVC3(len, pi, po), po += len;	break;
	}
	if (pi != po) {
		status = -1;
		/********************************
...
int	len = px->length, n, status = 1;
	...
	N_ELEMENTS(px, n);
	if (status & 1) switch (len) {
	case 1 : for (; --n >= 0; pi += 1) if (!*pm++) *(char *)pi	= *((char *)pn);	break;
	case 2 : for (; --n >= 0; pi += 2) if (!*pm++) *(short *)pi	= *((short *)pn);	break;
	case 4 : for (; --n >= 0; pi += 4) if (!*pm++) *(int *)pi	= *((int *)pn);	break;
	case 8 : for (; --n >= 0; pi += 8) if (!*pm++) *(quadw *)pi	= *((quadw *)pn);	break;
	default : for (; --n >= 0; pi += len) if (!*pm++) _MOVC3(len, pn, pi);	break;
	}
	return status;
}
/**********************************************
...
static unsigned char omitd[] = {DTYPE_WITH_UNITS,DTYPE_DIMENSION,0};
	...
	if (status & 1 && in.pointer->dtype == DTYPE_WITH_UNITS) {
		...
		if (status & 1) status = TdiGetData(&omits[1] ,&in, &in);
	}
	if (status & 1 && narg > 1 && list[1]) status = TdiGetLong(list[1], &dim);
	else dim = 0;
	...
	*****************************/
	if (status & 1 && in.pointer->dtype == DTYPE_SIGNAL) {
		...
		if (dim >= psig->ndesc - 2) status = TdiBAD_INDEX;
		else status = TdiGetData(omitd, psig->dimensions[dim], &in);
		if (status & 1 && in.pointer->dtype == DTYPE_WITH_UNITS) {
			status = TdiUnits(in.pointer, &units MDS_END_ARG);
			if (status & 1) status = TdiGetData(&omitd[1] ,&in, &in);
		}
		else	MdsFree1Dx(&units, NULL);
		dim = 0;
	}
	/*************************
	Get dimension information.
	*************************/
	if (status & 1) switch (in.pointer->dtype) {
	case DTYPE_MISSING :
		if (narg > 2 && list[2]) status = MdsCopyDxXd(list[2], out_ptr);
		else MdsFree1Dx(out_ptr, NULL);
		break;
	case DTYPE_T :
		if (!(narg > 2 && list[2])) {
			if (status & 1) status = TdiData(&in, out_ptr MDS_END_ARG);
			break;
		}
		...
		if (status & 1) status = TdiIsIn(dat[0].pointer, &in, &tmp MDS_END_ARG);
		if (status & 1) status = rroutine(dat[0].pointer, (struct descriptor_a *)tmp.pointer, 
                                                                        (struct descriptor_a *)dat[0].pointer);
		if (status == -1) status = TdiRecull(&dat[0]);
		MdsFree1Dx(&sig[0], NULL);
		MdsFree1Dx(&uni[0], NULL);
		if (status & 1) *out_ptr = dat[0];
		break;
	default :
		if (!(narg > 2 && list[2])) {
			if (status & 1) status = TdiData(&in, out_ptr MDS_END_ARG);
		...
		********************************/
		...
		if (status & 1) {
			...
			if (pwu->dtype == DTYPE_WITH_UNITS) dx0 = *pwu->data;
			else dx0 = *tmp.pointer;
			dx0.class = CLASS_S;
			dx1 = dx0;
		...
		************************************************************/
		...
		if (status & 1 && units.pointer == 0) status = MdsCopyDxXd((struct descriptor *)&uni[0], &units);
		if (status & 1) status = TdiCvtArgs(3, dat, cats);
		if (status & 1) status = TdiGe(dat[2].pointer, dat[0].pointer, &tmp MDS_END_ARG);
		if (status & 1) s1 = status = rroutine(dat[0].pointer, (struct descriptor_a *)tmp.pointer, 
									     (struct descriptor_a *)dat[2].pointer);
		if (status & 1) status = TdiLe(dat[2].pointer, dat[1].pointer, &tmp MDS_END_ARG);
		if (status & 1) s1 |= status = rroutine(dat[1].pointer, (struct descriptor_a *)tmp.pointer, 
									      (struct descriptor_a *)dat[2].pointer);
		if (status & 1 && s1 == -1) status = TdiRecull(&dat[2]);
		MdsFree1Dx(&sig[0], NULL);
		...
		if (status & 1) *out_ptr = dat[2];
		break;
	}
	MdsFree1Dx(&tmp, NULL);
	MdsFree1Dx(&in, NULL);
	if (status & 1 && units.pointer) status = TdiMasterData(0, sig, &units, &cmode, out_ptr);
