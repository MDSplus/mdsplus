...
STATIC_CONSTANT DESCRIPTOR_RANGE(EMPTY_RANGEE, 0, 0,
			...
			struct descriptor_xd *out_ptr)
{
	...
	{ DTYPE_DIMENSION, DTYPE_SIGNAL, DTYPE_DIMENSION, 0 };
    STATIC_CONSTANT unsigned char omitd[] =
	{ DTYPE_WITH_UNITS, DTYPE_DIMENSION, 0 };
	...
	*************************/
	...
	default:
		...
		************************************************************/
	    ...
	    if (status & 1)
		status =
		    TdiGe(dat[2].pointer, dat[0].pointer, &tmp MDS_END_ARG);
	    if (status & 1)
		s1 = status =
		    rroutine(dat[0].pointer, (struct descriptor_a *)tmp.pointer,
	    ...
	    if (status & 1)
		status =
		    TdiLe(dat[2].pointer, dat[1].pointer, &tmp MDS_END_ARG);
	    if (status & 1)
		s1 |= status =
...
/***********************************************/
int Tdi1Cull(int opcode, int narg, struct descriptor *list[],
	     struct descriptor_xd *out_ptr)
{
    int status = 1;
...
/***********************************************/
int Tdi1Extend(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
