...
STATIC_CONSTANT struct descriptor zero =
    { sizeof(zero_val), DTYPE_BU, CLASS_S, (char *)&zero_val };
STATIC_CONSTANT struct descriptor one =
    { sizeof(one_val), DTYPE_BU, CLASS_S, (char *)&one_val };

int Tdi1Trans(int opcode, int narg, struct descriptor *list[],
	      struct descriptor_xd *out_ptr)
{
	...
	****************************/
	...
	status =
	    MdsGet1DxA((struct descriptor_a *)pa, &digits, &out_dtype, out_ptr);
	if (status & 1)
	...
	***************************/
	...
	arr.arsize *= ncopies;
	status =
	    MdsGet1DxA((struct descriptor_a *)&arr, &digits, &out_dtype,
		       out_ptr);
    }
	/*************************************
	...
	***********/
    status =
	(*pfun->f3) (pa, pmask, pd, count_dim, count_bef, count_aft, step_dim,
		     step_bef, step_aft);
    if (!(status & 1))
	goto err;
	    ...
	    int count0, int count1, int count2, int step0, int step1, int step2)
{
    ...
    int j0, j1, j2;
    int leni = in_ptr->length, stepi0 = leni * step0, stepi1 =
	leni * step1, stepi2 = leni * step2;
...
int Tdi3Count(struct descriptor *in_ptr,
	      ...
	      struct descriptor *out_ptr,
	      int count0,
	      int count1, int count2, int step0, int step1, int step2)
{
