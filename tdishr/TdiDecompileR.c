...
{
    newline.length =
	(unsigned short)(((TdiIndent += step) < 8 ? TdiIndent : 8) + 1);
    return StrAppend(pout, &newline);
}

STATIC_ROUTINE int OneStatement(struct descriptor_r *pin,
...
STATIC_ROUTINE int MultiStatement(int nstmt,
...
int Tdi0Decompile_R(struct descriptor_r *pin,
		    int prec, struct descriptor_d *pout)
{
 ...
 cannot:	status = Append(fun_ptr->name, pout);
		...
		****************/
		...
		if (status & 1 && prec <= newone)
		    status = Append("(", pout);
		if (status & 1)
		    status =
			Tdi0Decompile(pin->dscptrs[0], newone + lorr, pout);
		if (status & 1 && prec <= newone)
		    status = Append(")", pout);
		if (status & 1 && lorr < 0)
		    status = Append(pop->symbol, pout);
 ...
 first:	for (pop = binary; pop->opcode; pop++)
		...
		if (opcode == OpcConditional) {
		    if (status & 1)
			status =
			    Tdi0Decompile(r_ptr->dscptrs[2], newone - lorr,
					  pout);
		    if (status & 1)
			status = Append(pop->symbol, pout);
		    if (status & 1)
		    ...
		    if (status & 1)
			status =
			    Tdi0Decompile(r_ptr->dscptrs[1], newone + lorr,
					  pout);
		} else {
		    if (status & 1)
			status =
			    Tdi0Decompile(r_ptr->dscptrs[0], newone - lorr,
					  pout);
		    for (m = 1; m < narg; m++) {
			if (status & 1)
			    status = Append(pop->symbol, pout);
			if (status & 1 && pin != r_ptr)
			    status = Append("= ", pout);
			if (status & 1)
			    status =
				Tdi0Decompile(r_ptr->dscptrs[m], newone + lorr,
					      pout);
		    }
		}
		if (status & 1 && prec <= newone)
		...
		*****************************************/
		...
		if (status & 1)
		    status =
			MultiStatement(narg - 1,
				       (struct descriptor_r **)&pin->dscptrs[1],
				       pout);
		if (prec < P_STMT && status & 1)
		    status = Append(")", pout);
		break;
		...
		if (status & 1)
		    status =
			MultiStatement(narg,
				       (struct descriptor_r **)&pin->dscptrs[0],
				       pout);
		if (prec < P_STMT && status & 1)
		    status = Append(")", pout);
		break;
		...
		if (status & 1)
		    status =
			Append((opcode == OpcIf) ? "If (" : "Where (", pout);
		if (status & 1)
		    status = Tdi0Decompile(pin->dscptrs[0], P_STMT, pout);
		if (status & 1)
		    status = Append(") ", pout);
		...
		if (status & 1 && narg >= 3) {
		    status =
			Append((opcode == OpcIf) ? " Else " : " ElseWhere ",
			       pout);
		    if (status & 1)
			status =
			    CompoundStatement(1,
		...
		if (status & 1)
		    status =
			Tdi0Decompile(pin->ndesc ? pin->dscptrs[0] : 0, P_STMT,
				      pout);
		if (status & 1)
		    status = Append(")", pout);
		if (status & 1)
		...
		if (status & 1)
		    status =
			Append((opcode == OpcSwitch) ? "Switch (" : "While (",
			       pout);
		if (status & 1)
		    status = Tdi0Decompile(pin->dscptrs[0], P_STMT, pout);
		if (status & 1)
