...
STATIC_CONSTANT struct descriptor dmbig =
{ sizeof(mbig), DTYPE_F, CLASS_S, (char *)&mbig };
STATIC_CONSTANT struct descriptor dbig =
{ sizeof(big), DTYPE_F, CLASS_S, (char *)&big };
STATIC_CONSTANT struct descriptor dmhuge =
{ sizeof(mhuge), DTYPE_L, CLASS_S, (char *)&mhuge };
STATIC_CONSTANT struct descriptor dhuge =
{ sizeof(lhuge), DTYPE_L, CLASS_S, (char *)&lhuge };
STATIC_CONSTANT struct descriptor dmone =
{ sizeof(mone), DTYPE_L, CLASS_S, (char *)&mone };
STATIC_CONSTANT struct descriptor done =
{ sizeof(one), DTYPE_L, CLASS_S, (char *)&one };
...
int Tdi1ItoX(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
	...
	struct descriptor_xd dimen = EMPTY_XD, window = EMPTY_XD, axis =
			EMPTY_XD, xat0 = EMPTY_XD;
	...
	if (status & 1)
		...
		case DTYPE_DIMENSION:
			...
			if (status & 1 && pwin && pwin->dtype == DTYPE_WITH_UNITS) {
				status =
						TdiEvaluate(((struct descriptor_with_units *)pwin)->data,
								&tmp MDS_END_ARG);
				if (status & 1)
			...
			if (status & 1 && paxis && paxis->dtype == DTYPE_WITH_UNITS) {
				status =
						TdiEvaluate(((struct descriptor_with_units *)paxis)->data,
								&tmp MDS_END_ARG);
				if (status & 1)
	...
	if (status & 1) {
		...
		case DTYPE_SLOPE:
			...
			case 0:
				...
				break;
			default:
				*(struct descriptor_function *)(fake.begin =
						(struct descriptor *)&vec[0]) =
								vector0;
				*(struct descriptor_function *)(fake.ending =
						(struct descriptor *)&vec[1]) =
								vector0;
				*(struct descriptor_function *)(fake.deltaval =
						(struct descriptor *)&vec[2]) =
								vector0;
				...
				for (jseg = nseg; --jseg >= 0;)
					if ((vec[0].arguments[jseg] =
							pslope->segment[jseg].begin) == 0) {
						vec[0].arguments[jseg] = &dmbig;
					}
				for (jseg = nseg; --jseg >= 0;)
					if ((vec[1].arguments[jseg] =
							pslope->segment[jseg].ending) == 0)
						vec[1].arguments[jseg] = &dbig;
				for (jseg = nseg; --jseg >= 0;)
					if ((vec[2].arguments[jseg] =
							pslope->segment[jseg].slope) == 0)
			 ...
			 *********************/
			case DTYPE_RANGE:
				status = TdiGetArgs(opcode, nran =
						paxis->ndesc, &paxis->begin, sig, uni, dat,
						cats);
				nseg = HUGE;
				...
				big_beg = cats[0].in_dtype == DTYPE_MISSING
						|| (cats[0].in_cat & TdiCAT_FLOAT
								&& ROPRAND_TYPED(dat[beg].pointer->dtype,
				...
				if (status & 1)
					status =
							TdiSubtract(dat[end].pointer, dat[beg].pointer,
									&cnt MDS_END_ARG);
				if (tslo && status & 1)
				...
				if (nseg > 1 || !big_beg) {
					if (status & 1)
						status =
								TdiSubtract(dat[beg].pointer, xat0.pointer,
										&tmp MDS_END_ARG);
					if (tslo && status & 1)
						status =
								TdiDivide(&tmp, dat[delta].pointer, &tmp MDS_END_ARG);
					if (!(TdiFaultClear(0) & 1) && status & 1)
					...
					if (kseg == 0 && left > 0)
						...
						if (nseg == 1) {
							...
							if (left <= -HUGE)
								...
								if (tslo && status & 1)
									status =
											TdiMultiply(&tmp, dat[delta].pointer,
													&tmp MDS_END_ARG);
								if (status & 1)
									status =
											TdiSubtract(&dat[beg], &tmp, &xat0 MDS_END_ARG);
								if (!big_end)
				...
				if (status & 1) {
					if (special) {
						...
						status =
								MdsCopyDxXd((struct descriptor *)&dlimits, out_ptr);
					} else if (arg1) {
						struct descriptor_range *rptr =
								(struct descriptor_range *)list[1];
						MdsFree1Dx(out_ptr, NULL);
						TdiThreadStatic()->TdiRANGE_PTRS[0] = &dk0;
						TdiThreadStatic()->TdiRANGE_PTRS[1] = &dk1;
						TdiThreadStatic()->TdiRANGE_PTRS[2] =
								flag ? 0 : dimen.pointer;
						/******************************************************
						 ...
						 ******************************************************/
						 if (!flag && (rptr->dtype == DTYPE_RANGE)
							 ...
							 rptr = &fake_range;
							 status =
									 TdiGetArgs(opcode, 1, &rptr, &sig1, &uni1, out_ptr,
											 cats);
						 } else
							 status =
									 TdiGetArgs(opcode, 1, &rptr, &sig1, &uni1, out_ptr,
				...
				if (nseg > 1 && (flag || arg1)) {
					if (status & 1)
						status = TdiBsearch(out_ptr->pointer,
								flag ? cnt.pointer : dat[beg].pointer,
										&done, &tmp MDS_END_ARG);
					if (status & 1 && dat[beg].pointer->class == CLASS_A)
						status =
								TdiMap(&dat[beg], tmp.pointer, &dat[beg] MDS_END_ARG);
					if (status & 1 && dat[end].pointer->class == CLASS_A)
						status =
								TdiMap(&dat[end], tmp.pointer, &dat[end] MDS_END_ARG);
					...
					if (tslo && status & 1)
						status =
								TdiMultiply(&cnt, dat[delta].pointer, &cnt MDS_END_ARG);
					if (status & 1)
				...
				if (flag) {
					if (tslo && status & 1)
						status =
								TdiMultiply(out_ptr, &dat[delta], out_ptr MDS_END_ARG);
					if (status & 1)
					...
					if (tslo && status & 1)
						status =
								TdiDivide(out_ptr, &dat[delta], out_ptr MDS_END_ARG);
				}
				MdsFree1Dx(&cnt, NULL);
				MdsFree1Dx(&uni1, NULL);
				...
				if (status & 1)
					status =
							TdiMasterData(narg > 1, &sig1, &units, &cmode, out_ptr);
			...
			if (status & 1) {
				if (special) {
					if (flag) {
						...
						status =
								MdsGet1DxA((struct descriptor_a *)&duo, &num,
										&paxis->dtype, out_ptr);
						if (status & 1) {
							_MOVC3(j1, (char *)window.pointer->pointer,
									out_ptr->pointer->pointer);
							_MOVC3(j1,
									(char *)window.pointer->pointer + j1 * (k1 -
											k0),
											out_ptr->pointer->pointer + j1);
						}
					} else {
						int limits[2];
						array_int dlimits = *(array_int *) & duo;
