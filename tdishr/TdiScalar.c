...
int Tdi1Scalar(int opcode, int narg, struct descriptor *list[],
               struct descriptor_xd *out_ptr)
{
    ...
    if (status & 1)
        ...
        case 2:
            status =
                (*routine) (dat[0].pointer, dat[1].pointer, out_ptr->pointer);
            break;
        }
...
int Tdi3Epsilon(struct descriptor *x_ptr, struct descriptor *out_ptr)
{
    int status = 1;
    double digits_d;
    struct descriptor digits =
        { sizeof(digits_d), DTYPE_NATIVE_DOUBLE, CLASS_S, 0 };
...
STATIC_CONSTANT const unsigned int OU_HUGE[] =
    { 0xffffffff, 0xffffffff, 0xffffffff, 0xffffffff };
STATIC_CONSTANT const unsigned int O_HUGE[] =
...
*/
int Tdi3SelectedRealKind(struct descriptor *pdp,
                         struct descriptor *pdr, struct descriptor *pout)
{
