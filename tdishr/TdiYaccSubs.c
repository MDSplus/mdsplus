...
int TdiYacc_ARG(struct marker *mark_ptr)
{
	...
	*******************************/
    status =
	MdsCopyDxXdZ(ptr, &junk, &TdiRefZone.l_zone, NULL, NULL, NULL, NULL);
    if (status & 1)
		  ...
		  struct marker *arg2, struct marker *arg3, struct marker *arg4)
{
    ...
    int dsc_size =
	sizeof(struct descriptor_function) +
	sizeof(struct descriptor *) * (ndesc - 1);
...
int TdiYacc_IMMEDIATE(struct descriptor_xd **dsc_ptr_ptr)
{
	...
	*******************/
    if (status & 1)
	status =
	    MdsCopyDxXdZ(xd.pointer, &junk, &TdiRefZone.l_zone, NULL, NULL,
			 NULL, NULL);
    if (status & 1)
...
int TdiYacc_RESOLVE(struct descriptor_function **out_ptr_ptr)
{
 ...
 doit:
    if ((TdiRefZone.l_status =
	 TdiYacc_IMMEDIATE((struct descriptor_xd **)out_ptr_ptr)) & 1)
	return 0;
    return 1;
}
