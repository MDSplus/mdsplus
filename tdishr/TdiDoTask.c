...
STATIC_ROUTINE int Doit(struct descriptor_routine *ptask,
			struct descriptor_xd *out_ptr)
{
    ...
    switch (ptask->dtype) {
    case DTYPE_METHOD:{
	    struct descriptor_method *pmethod =
		(struct descriptor_method *)ptask;
	    int nid;
    ...
    case DTYPE_PROGRAM:
	{
	    struct descriptor_program *prog_task =
		(struct descriptor_program *)ptask;
...
int Tdi1DoTask(int opcode, int narg, struct descriptor *list[],
	       struct descriptor_xd *out_ptr)
{
    int status = 1;
    quadw dt = { 0, 0 };
