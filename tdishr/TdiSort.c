...
int Tdi1Bsearch(int opcode, int narg, struct descriptor *list[],
    ...
    if (status & 1) {
        ...
        for (; --ni >= 0; pinput += len) {
            ...
            if (lo < -1 || lo >= nt) {
                ...
                inc = 1;
                while (hi = lo, (lo -= inc) >= 0
                       && (*gtr) (ptable + len * lo, pinput, len) > 0)
                    inc <<= 1;
                if (lo < 0)
                    lo = -1;
            }
...
*/
int Tdi1Sort(int opcode, int narg, struct descriptor *list[],
             struct descriptor_xd *out_ptr)
{
    int status = 1;
    ...
    for (;;) {
        ...
        while (r - l < 11) {
            for (j = l; ++j <= r;) {
                ...
                for (i = j;
                     --i >= l
                     && (*gtr) (pinput + len * *(ndx + i), pkeep, len) > 0;)
                    *(ndx + i + 1) = *(ndx + i);
                *(ndx + i + 1) = keep;
            }
                        /**************************
...
*/
int Tdi1SortVal(int opcode, int narg, struct descriptor *list[],
                struct descriptor_xd *out_ptr)
{
    int status = 1;
    ...
    if (status & 1)
        status =
            TdiSort(tmp.pointer, narg > 1 ? list[1] : 0, out_ptr MDS_END_ARG);
    if (status & 1)
...
*/
int Tdi1Union(int opcode, int narg, struct descriptor *list[],
              struct descriptor_xd *out_ptr)
{
    int status = 1;
    int j, n, len;
    ...
    if (status & 1) {
        ...
        struct descriptor_a *pdo =
            pdwu->dtype ==
            DTYPE_WITH_UNITS ? (struct descriptor_a *)pdwu->
            data : (struct descriptor_a *)pdwu;
        char *cptri, *cptro;
        short *sptri, *sptro;
        int *lptri, *lptro;
...
*/
int Tdi1IsIn(int opcode, int narg, struct descriptor *list[],
             struct descriptor_xd *out_ptr)
{
    int status = 1;
