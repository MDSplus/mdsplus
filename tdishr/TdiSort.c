...
struct TdiCatStruct		cats[3];
	...
	if (status & 1)
	if (ni <= 0) ;					/* null output */
	else if (nt <= 0) status = TdiNULL_PTR;	/* cannot lookup */
	else switch (dat[0].pointer->dtype) {
	case DTYPE_BU :	neq = (int (*)())&NEQ_BU;	gtr = (int (*)())&GTR_BU; break;
	case DTYPE_WU :	neq = (int (*)())&NEQ_WU;	gtr = (int (*)())&GTR_WU; break;
	case DTYPE_NID :
	case DTYPE_LU :	neq = (int (*)())&NEQ_LU;	gtr = (int (*)())&GTR_LU; break;
	case DTYPE_QU :	neq = (int (*)())&NEQ_QU;	gtr = (int (*)())&GTR_QU; break;
	case DTYPE_OU :	neq = (int (*)())&NEQ_OU;	gtr = (int (*)())&GTR_OU; break;

	case DTYPE_B :	neq = (int (*)())&NEQ_B;	gtr = (int (*)())&GTR_B; break;
	case DTYPE_W :	neq = (int (*)())&NEQ_W;	gtr = (int (*)())&GTR_W; break;
	case DTYPE_L :	neq = (int (*)())&NEQ_L;	gtr = (int (*)())&GTR_L; break;
	case DTYPE_Q :	neq = (int (*)())&NEQ_Q;	gtr = (int (*)())&GTR_Q; break;
	case DTYPE_O :	neq = (int (*)())&NEQ_O;	gtr = (int (*)())&GTR_O; break;

	case DTYPE_F :	neq = (int (*)())&NEQ_F;	gtr = (int (*)())&GTR_F; break;
	case DTYPE_FS :	neq = (int (*)())&NEQ_FS;	gtr = (int (*)())&GTR_FS; break;
	case DTYPE_D :	neq = (int (*)())&NEQ_D;	gtr = (int (*)())&GTR_D; break;
	case DTYPE_G :	neq = (int (*)())&NEQ_G;	gtr = (int (*)())&GTR_G; break;
	case DTYPE_FT :	neq = (int (*)())&NEQ_FT;	gtr = (int (*)())&GTR_FT; break;

	case DTYPE_FC :	neq = (int (*)())&NEQ_FC;	gtr = (int (*)())&GTR_FC; break;
	case DTYPE_FSC :	neq = (int (*)())&NEQ_FSC;	gtr = (int (*)())&GTR_FSC; break;
	case DTYPE_DC :	neq = (int (*)())&NEQ_DC;	gtr = (int (*)())&GTR_DC; break;
	case DTYPE_GC :	neq = (int (*)())&NEQ_GC;	gtr = (int (*)())&GTR_GC; break;
	case DTYPE_FTC :	neq = (int (*)())&NEQ_FTC;	gtr = (int (*)())&GTR_FTC; break;

	case DTYPE_PATH :
	case DTYPE_EVENT :
	case DTYPE_T :	neq = (int (*)())&NEQ_T;	gtr = (int (*)())&GTR_T;
		if (narg > 3) status = TdiGetLong(list[3], &upcase);
		if (upcase & 1) {
			if (status & 1) status = StrUpcase(dat[0].pointer, dat[0].pointer);
			if (status & 1) status = StrUpcase(dat[1].pointer, dat[1].pointer);
		}
		break;
	default : if (ni > 0) status = TdiINVDTYDSC; break;
	}
	if (status & 1) {
		...
		for (; --ni >= 0; pinput += len) {
			...
			*****************/
			while (hi - lo > 1) {
				mid = (hi + lo) >> 1;
				if ((*gtr)(ptable+len*mid, pinput, len) <= 0) lo = mid;
				else hi = mid;
			}
			/**************
			Equality check.
			**************/
			if (mode > 0 || (lo >= 0 && (*neq)(pinput, ptable+len*lo, len) == 0)) *poutput++ = lo;
			else if (mode == 0) *poutput++ = -1;
			else *poutput++ = lo + 1;
		}
	}
	if (sig[1].pointer) MdsFree1Dx(&sig[1], NULL);
	if (uni[1].pointer) MdsFree1Dx(&uni[1], NULL);
...
int				stack[64];
	...
	case DTYPE_BU :	gtr = (int (*)())&GTR_BU; break;
	case DTYPE_WU :	gtr = (int (*)())&GTR_WU; break;
	case DTYPE_LU :	gtr = (int (*)())&GTR_LU; break;
	case DTYPE_QU :	gtr = (int (*)())&GTR_QU; break;
	case DTYPE_OU :	gtr = (int (*)())&GTR_OU; break;

	case DTYPE_B :	gtr = (int (*)())&GTR_B; break;
	case DTYPE_W :	gtr = (int (*)())&GTR_W; break;
	case DTYPE_L :	gtr = (int (*)())&GTR_L; break;
	case DTYPE_Q :	gtr = (int (*)())&GTR_Q; break;
	case DTYPE_O :	gtr = (int (*)())&GTR_O; break;

	case DTYPE_F :	gtr = (int (*)())&GTR_F; break;
	...
	case DTYPE_GC :	gtr = (int (*)())&GTR_GC; break;
	case DTYPE_FTC :	gtr = (int (*)())&GTR_FTC; break;

	case DTYPE_T :	gtr = (int (*)())&GTR_T;
		if (narg > 1) status = TdiGetLong(list[1], &upcase);
		if (status & 1 & upcase & 1) status = StrUpcase(dat[0].pointer, dat[0].pointer);
		break;
	...
	******************/
	...
	for (;;) {
		...
		************************************************/
		for (;;) {
			while (i < j && (*gtr)(pinput+len* *(ndx+j), pkeep, len) > 0) --j;
			if (i == j) break;
			*(ndx+i) = *(ndx+j);
			++i;
			while (i < j && (*gtr)(pkeep, pinput+len* *(ndx+i), len) > 0) ++i;
			if (i == j) break;
			*(ndx+j) = *(ndx+i);
			--j;
		}
	...
	MdsFree1Dx(&dat[0], NULL);
	if (sig[0].pointer) {
		if (status & 1) status = TdiMasterData(1, &sig[0], uni, &cmode, out_ptr);
...
struct descriptor_xd	tmp = EMPTY_XD;
	...
	if (status & 1) status = TdiSort(tmp.pointer, narg > 1 ? list[1] : 0, out_ptr MDS_END_ARG);
	if (status & 1) status = TdiMap(&tmp, out_ptr, out_ptr MDS_END_ARG);
	if (tmp.pointer) MdsFree1Dx(&tmp, NULL);
	return status;
}
/*--------------------------------------------------------------
...
char	*pi, *po;
	...
	if (status & 1) status = TdiSortVal(out_ptr, out_ptr MDS_END_ARG);
	if (status & 1) {
	struct descriptor_with_units *pdwu = (struct descriptor_with_units *)out_ptr->pointer;
	struct descriptor_a *pdo = pdwu->dtype == DTYPE_WITH_UNITS ? 
                                       (struct descriptor_a *)pdwu->data : (struct descriptor_a *)pdwu;
        char *cptri, *cptro;
        short *sptri, *sptro;
        int *lptri, *lptro;
		if (pdo->class != CLASS_A) return status;
		len = pdo->length;
		po = pi = pdo->pointer;
		n = (int)pdo->arsize / (int)pdo->length;
		switch (len) {
		case 1 : for (cptri = (char  *)pi, cptro = (char  *)po; --n > 0;) if (*++cptri != *cptro) *++cptro = *cptri; 
                         po = (char *)cptro; break;
		case 2 : for (sptri = (short *)pi, sptro = (short *)po; --n > 0;) if (*++sptri != *sptro) *++sptro = *sptri; 
                         po = (char *)sptro; break;
		case 4 : for (lptri = (int  *)pi, lptro = (int  *)po; --n > 0;) if (*++lptri != *lptro) *++lptro = *lptri; 
                         po = (char *)lptro; break;
		default :
			switch (len & 3) {
			default : for (; --n > 0;) {
					pi += len;
					for (j = 0; j < len; j += 1) if (*(char *)(pi+j) != *(char *)(po+j)) break;
					if (j < len) {po += len; _MOVC3(len, pi, po);}
				} break;
			case 2 : for (; --n > 0;) {
					pi += len;
					for (j = 0; j < len; j += 2) if (*(short *)(pi+j) != *(short *)(po+j)) break;
					if (j < len) {po += len; _MOVC3(len, pi, po);}
				} break;
			case 0 : for (; --n > 0;) {
					pi += len;
					for (j = 0; j < len; j += 4) if (*(int *)(pi+j) != *(int *)(po+j)) break;
					if (j < len) {po += len; _MOVC3(len, pi, po);}
				} break;
			} break;
		}
...
struct descriptor *pupcase = narg > 2 ? list[2] : 0;
	...
	if (status & 1) status = TdiBsearch(list[0], out_ptr, 0, pupcase, out_ptr MDS_END_ARG);
	if (status & 1) status = TdiGe(out_ptr, 0, out_ptr MDS_END_ARG);
	return status;
}
