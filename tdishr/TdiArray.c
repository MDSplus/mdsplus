                     ...
                     struct descriptor_xd *out_ptr)
{
    int status = 1;
    array_coeff arr =
        { 1, DTYPE_B, CLASS_A, (char *)0, 0, 0, {0, 1, 1, 1, 0}, MAXDIM, 0 };
    array_int cvt =
        { sizeof(int), DTYPE_L, CLASS_A, (int *)0, 0, 0, {0, 1, 1, 0, 0}, 1,
        0 };
    ...
    if (status & 1) {
        if (arr.class == CLASS_A) {
            for (arr.arsize = 1, j = ndim; --j >= 0;)
                arr.arsize *= arr.m[j];
            status =
                MdsGet1DxA((struct descriptor_a *)&arr, &length, &dtype,
                           out_ptr);
        } else
...
int Tdi3Ramp(struct descriptor *out_ptr)
{
    ...
    STATIC_CONSTANT struct descriptor con0 =
        { sizeof(int), DTYPE_L, CLASS_S, (char *)&i0 };
    STATIC_CONSTANT struct descriptor con1 =
        { sizeof(int), DTYPE_L, CLASS_S, (char *)&i1 };
    int status = 1, n;
    int i;
...
#define LoadRandomFloat(dtype,type,value) { type *ptr = (type *)out_ptr->pointer; double val; \
    ...
    case DTYPE_D:
        LoadRandomFloat(DTYPE_D, double,
                        (Tdi_RandomSeed * norm + Tdi_RandomSeed) * norm + half);
        break;
