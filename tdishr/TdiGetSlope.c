                ...
                struct descriptor_xd *out_ptr)
{
    ...
    if ((ndesc < 2 || !slope_ptr->segment[0].begin) && (ndesc < 3 ||
        ...
        *********************************/
        ...
        struct descriptor_xd (*pbegin)[], (*pend)[] = 0, (*pslope)[] =
            0, *(*pvlist)[] = 0, tmp1;
        int (*pcnt)[] = 0;
        int virt =
        ...
        for (jseg = 0; jseg < nseg; ++jseg) {
            ...
            if (status & 1)
                status =
                    TdiEvaluate(slope_ptr->segment[jseg].slope,
                                &(*pslope)[jseg]);
            if (status & 1 && jseg * 3 + 1 < ndesc)
                status =
                    TdiEvaluate(slope_ptr->segment[jseg].begin,
                                &(*pbegin)[jseg]);
            if (status & 1 && jseg * 3 + 2 < ndesc)
                status =
                    TdiEvaluate(slope_ptr->segment[jseg].ending,
                                &(*pend)[jseg]);
            ...
            if (status & 1)
                status =
                    TdiSubtract((*pend)[jseg].pointer, (*pbegin)[jseg].pointer,
                                &tmp1);
            if (status & 1)
