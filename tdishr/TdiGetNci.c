...
STATIC_CONSTANT struct item {
    ...
    "CONGLOMERATE_NIDS", 0, NciNUMBER_OF_ELTS, NciCONGLOMERATE_NIDS,
            DTYPE_NID, 0}, {
    ...
    "NO_WRITE_MODEL", NciM_NO_WRITE_MODEL, NciM_NO_WRITE_MODEL,
            NciGET_FLAGS, DTYPE_BU, 1}, {
    ...
    "PATH_REFERENCE", NciM_PATH_REFERENCE, NciM_PATH_REFERENCE,
            NciGET_FLAGS, DTYPE_BU, 1}, {
    ...
    "USAGE_COMPOUND_DATA", 0xff, TreeUSAGE_COMPOUND_DATA, NciUSAGE,
            DTYPE_BU, 1}, {
...
"WRITE_ONCE", NciM_WRITE_ONCE, NciM_WRITE_ONCE, NciGET_FLAGS, DTYPE_BU,
            1},};
...
STATIC_ROUTINE int compare(struct descriptor *s1, struct item s2[1])
{
    ...
    char c0;
        ...
        ****************************************/
    if ((cmp =
         strncmp(s1->pointer, s2[0].item_name,
...
int Tdi1GetNci(int opcode, int narg, struct descriptor *list[],
               struct descriptor_xd *out_ptr)
{
    ...
    if (list[0]) {
        ...
        if (status & 1) {
            ...
            if (status & 1 && class != CLASS_A)
                switch (dtype) {
                case DTYPE_T:
                case DTYPE_PATH:
                    {
                        struct descriptor path_d =
                            { 0, DTYPE_PATH, CLASS_S, 0 };
                        char *path;
    ...
    if (status & 1) {
        step =
            key_ptr->item_length ==
            0 ? sizeof(struct descriptor_xd) : key_ptr->item_length;
        status =
            MdsGet1DxA((struct descriptor_a *)&arr0, &step,
 ...
 more:  switch (dtype) {
        ...
        if ((unsigned int)(outcount * step) >= holda_ptr->arsize) {
            ...
            if (status & 1)
                status =
                    MdsGet1DxA(holda_ptr, &step, &key_ptr->item_dtype, &tmp);
            if (status & 1) {
...
*/
            ...
            *(struct descriptor_d *)hold_ptr = EMPTY_D;
            ...
            if (status & 1 && texted[0].pointer) {
                unsigned short len =
                    (unsigned short)strlen((char *)texted[0].pointer);
                if (maxlen < len)
                    maxlen = len;
                StrCopyR((struct descriptor *)hold_ptr, &len,
                         texted[0].pointer);
 ...
 skip:  if (status & 1 && wild)
    ...
    if (status & 1) {
        ...
        if (key_ptr->item_length) {
            ...
            status =
                MdsCopyDxXd((struct descriptor *)holda_ptr->pointer, out_ptr);
        else if (maxlen > 0) {
            array arr = *(array *) & arr0;
            unsigned char dtype = DTYPE_T;
            arr.arsize = outcount;
            status =
                MdsGet1DxA((struct descriptor_a *)&arr, &maxlen, &dtype,
                           (struct descriptor_xd *)out_ptr);
            if (status & 1) {
                char *p;
                ...
                for (j = 0, p = out_ptr->pointer->pointer, dp =
                     (struct descriptor_xd *)holda_ptr->pointer; j < outcount;
                     j++, p += maxlen, dp++) {
            ...
            if (status & 1) {
                ...
                for (j = outcount; --j >= 0; hold_ptr += step)
                    *xd_ptr++ = hold_ptr;
                status =
                    Tdi1Vector(OpcVector, outcount, tmp.pointer->pointer,
                               out_ptr);
            }
            MdsFree1Dx(&tmp, NULL);
        }
