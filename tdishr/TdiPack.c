...
struct TdiCatStruct		cats[4];
	...
	if (status & 1 && dat[0].pointer->class != CLASS_A) status = TdiINVCLADSC;
	if (status & 1) status = Tdi2Pack(narg, uni, dat, cats, 0);
	...
	struct descriptor_a * parr = (struct descriptor_a *)dat[0].pointer;
		...
		lena = parr->length;
		if (narg > 2) {
			if (dat[2].pointer->class != CLASS_A) numv = -2;
	...
	*******************************************************************************/
		if (dat[1].pointer->class == CLASS_A) {
			...
			if (numv >= 0 && numm > numv) numm = numv;
			for (; --numm >= 0; pm += lenm, pi += lena) if (*pm & 1) {_MOVC3(lena, pi, po); po += lena;};
			bytes = po - dat[0].pointer->pointer;
		}
		else if (!(*dat[1].pointer->pointer & 1)) bytes = 0;
