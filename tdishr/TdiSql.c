...
STATIC_ROUTINE void StoreAnswer(int idx, struct descriptor *dst, int type)
{
    ...
    case SYBMONEY:{
		...
		if (bufs[idx].size < 32767 * num) {
		    src.pointer = bufs[idx].vptr =
			realloc(bufs[idx].vptr, 32767 * num);
...
*/
		...
		case SYBDATETIME:				/***convert date to double***/
			...
			int status =
			    SYB_dbconvert(dbproc, type, buf, bufs[j].len,
					  SYBCHAR, (unsigned char *)ddate,
					  sizeof(ddate) - 1);

#ifdef VMS
			quadword big_time;
			struct dsc$descriptor_s date_dsc =
			    { 22, DSC$K_DTYPE_T, DSC$K_CLASS_S, (char *)ddate };
			len = sizeof(d_null);
			dtype = DTYPE_FT;
			ddate[20] = '.';
			ddate[3] = ddate[0];
			...
			break;
#else
			STATIC_CONSTANT char *moname =
			    "JanFebMarAprMayJunJulAugSepOctNovDec";
			STATIC_CONSTANT int day[] =
	...
	334 };
			...
			leap -= mo < 2 && yr % 4 == 0 && (yr % 100 != 0
							  || yr % 400 == 0);
			d_ans =
			    (double)(yr * 365 + day[mo] + da + leap - 678941);
			d_ans +=
			    (double)(th +
				     1000 * (se +
					     60 * (mi + 60 * hr))) / 86400000.;
			buf = (char *)&d_ans;
#endif
#else
			d_ans = d_null;
			if (bufs[j].len == 8) {
				...
				((double)((unsigned int *)buf)[1]) / 300. /
				60. / 60. / 24.;
			} else if (bufs[j].len == 4) {
			    d_ans =
				(double)(((unsigned short *)buf)[0] + 15020) +
				((double)((unsigned short *)buf)[1]) / 60. /
				24.;
			}
			buf = (char *)&d_ans;
			len = sizeof(d_ans);
...
ARGLIST *arg;
{
    struct descriptor_xd **argv = (struct descriptor_xd **)arg->v, tmp =
	EMPTY_XD;
...
/*********************************************************/
int Tdi1Dsql(int opcode, int narg, struct descriptor *list[],
...
/*********************************************************/
int Tdi1Isql(int opcode, int narg, struct descriptor *list[],
	     struct descriptor_xd *out_ptr)
{
    int status = 1;
    ARGLIST user_args = { 0 };
...
/*********************************************************/
int Tdi1IsqlSet(int opcode, int narg, struct descriptor *list[],
		struct descriptor_xd *out_ptr)
{
    int status = 1;
    if (narg >= 1 && list[0])
