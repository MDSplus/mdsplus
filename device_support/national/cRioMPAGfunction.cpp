...
NiFpga_Status crioMpagInit(NiFpga_Session *session, const char *cRioId, size_t fifoDepthSize)
{
     ...
     /* opens a session, downloads the bitstream, and runs the FPGA */
      ...
      if (NiFpga_IsNotError(status))
      {
           ...
           status = crioMpagInitFifo(*session, "FIFO A" , NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_A, fifoDepthSize );
	   if ( NiFpga_IsError(status) ) {
        	NiFpga_Close(*session, 0);
		return -1;
	   }
           ...
           status = crioMpagInitFifo(*session, "FIFO B" , NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_B, fifoDepthSize );
	   if ( NiFpga_IsError(status) ) {
        	NiFpga_Close(*session, 0);
		return -1;
	   }
           ...
           status = crioMpagInitFifo(*session, "FIFO C" , NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_C, fifoDepthSize );
 	   if ( NiFpga_IsError(status) ) {
        	NiFpga_Close(*session, 0);
		return -1;
	   }
...
NiFpga_Status closeMpagFpgaSession(NiFpga_Session session)
{
   ...
   //NiFpga_Status status = NiFpga_Initialize();
    ...
    NiFpga_MergeStatus(&status, NiFpga_WriteBool(session,
	                                         NiFpga_MainFPGA_9159_ControlBool_Stop,
	                                          1));
    ...
    status = NiFpga_StopFifo(session,  NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_A );
    if ( NiFpga_IsError(status) ) {
	printf("Host SLAVE A stop fifo error\n");
    }

    status = NiFpga_StopFifo(session,  NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_B );
    if ( NiFpga_IsError(status) ) {
	printf("Host SLAVE B stop fifo error\n");
    }

    status = NiFpga_StopFifo(session,  NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_C );
    if ( NiFpga_IsError(status) ) {
...
NiFpga_Status setMpagAcqParam( NiFpga_Session session, uint8_t clockMode, float freq, uint16_t highStrobeTick, const int32_t *chanAOmap) 
{
    NiFpga_Status status = NiFpga_Status_Success;
    uint16_t acqPeriod;
    ...
    */
    NiFpga_MergeStatus(&status, NiFpga_WriteBool(session,
	                                         NiFpga_MainFPGA_9159_ControlBool_ForceLocal,
	                                         clockMode));
    ...
    {   
        ...
        for( int chan = 0; chan <  NUM_SLAVE_CHANNEL ; chan++)
        {
            if ( chanState[chan + slaveIdx * NUM_SLAVE_CHANNEL ] )
            {
                 ...
                 for(int smp = 0; smp < bufSize; smp++){
                     //if(smp == 0)printf("%s %d buffers_s[%d] = data[%d]\n",fifoName, chanIdx,(chanIdx + smp * NUM_SLAVE_CHANNEL), (chan + smp * NUM_SLAVE_CHANNEL));
...
*/
	        if(resampledNid)
                    saveList->addItem( buffers_s[chanIdx], 
		                   bufReadChanSmp[chanIdx], sampleToRead, SHORT , segmentSize, 
		                   readChanSmp[chanIdx], dataNid[chan], clockNid, timeIdx0, treePtr, shot, streamFactor, streamNames[chan], 
				   streamGains[chan], streamOffsets[chan], period, gains[slaveCh], coeffs[slaveCh], 
                                   numCoeffs[slaveCh], resampledNid[chan]);
                else
		    saveList->addItem( buffers_s[chanIdx], 
		                   bufReadChanSmp[chanIdx], sampleToRead, SHORT, segmentSize, 
		                   readChanSmp[chanIdx], dataNid[chan], clockNid, timeIdx0, treePtr, shot, streamFactor, streamNames[chan], 
...
int mpag_readAndSaveAllChannels(NiFpga_Session session, int nChan, int *chanState, int bufSize, int segmentSize, 
                                int numSamples, void *dataNidPtr, int clockNid, float timeIdx0, float period, 
...
int readMpagFifoData(NiFpga_Session session, const char *fifoName, NiFpgaEx_DmaFifo fifoId, int16_t *data, size_t slaveBufSize, void *stopAcq )
{
    NiFpga_Status status = NiFpga_Status_Success;
    size_t currSize = 0;
    uint32_t count = 0;
...
/**************************************************************/
    ...
    status = crioMpagResetFifo(session, "FIFO A" , NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_A);
    status = crioMpagResetFifo(session, "FIFO B" , NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_B);
    status = crioMpagResetFifo(session, "FIFO C" , NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_C);
...
/**************************************************************/
    ...
    {   
        ...
        readElem = readMpagFifoData( session, "SLAVE A",  NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_A, &data[0], slaveDataSamples );
        ...
        readElem = readMpagFifoData( session, "SLAVE B",  NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_B, &data[slaveDataSamples], slaveDataSamples );
        if( readElem != -2 && ( readElem == -1 || readElem != slaveDataSamples) )
        {
               if( readElem != slaveDataSamples )
        ...
        readElem = readMpagFifoData( session, "SLAVE C",  NiFpga_MainFPGA_9159_TargetToHostFifoI16_FIFOTOPC_C, &data[2*slaveDataSamples], slaveDataSamples );
        if( readElem != -2 && ( readElem == -1 || readElem != slaveDataSamples) )
        {
               if( readElem != slaveDataSamples )
		...
		{
                      for(int smp = 0; smp < bufSize; smp++){
                             //if(smp == 0)printf("%d %d buffers_s[%d] = data[%d]\n",slave, chan,(chanIdx + slave * NUM_SLAVE_CHANNEL), (chan + smp * NUM_SLAVE_CHANNEL + slave * slaveDataSamples));
                      	     buffers_s[chanIdx][smp] = data[chan + smp * NUM_SLAVE_CHANNEL + slave * slaveDataSamples];
                      }
                      chanIdx++;
                }
            }
