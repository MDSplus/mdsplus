...
enum callback_id {CB_UNKNOWN,CB_AUTOSCALE,CB_CROSSHAIRS,CB_LIMITS,CB_MOVE,CB_STRETCH,CB_SET_AT_LIMITS,CB_ADD_POINT,
...
static void /*XtActionProc*/MoveVerticalPane(Widget w, XButtonEvent *event, String *params, Cardinal *num_params);
...
static EventInfo *NewEvent(Widget w, unsigned long stub, XEvent *event, int reason,enum callback_id callback_id);
...
static char *IndexedName(char *name_in,int idx)
{
  static char name[20];
  int i;
  for (i=0;name_in[i];i++) name[i] = name_in[i];
  name[i] = '0' + idx;
  name[i+1] = 0;
  return name;
}
      ...
      && (stub_rec = IDL_WidgetStubLookup(*stub_id)))
  {
    ...
    if (num_extra > 0) {
      for (i=0; i<num_extra; i++) {
        Widget b;
        static XtCallbackRec user_button_callback_list[] = {{(XtCallbackProc)UserButton, (XtPointer)0},{0,0}};
    ...
    for (i=0;i< (*cols - 1);i++)
    {
      ...
      MrmFetchWidgetOverride(hierarchy,"sash",*plots,IndexedName("sash",i),arglist,XtNumber(arglist),&sash,&class);
    }
    for (i=0;i< *cols;i++)
    {
      Widget pane;
      ...
      int numargs = lsash && rsash ? XtNumber(args3) : (lsash ? XtNumber(args4) : (rsash ? XtNumber(args2) : XtNumber(args1)));
      Widget waveform = 0;
      Widget first_waveform;
      args2[2].value = args3[4].value = (long)rsash;
      args3[1].value = args4[1].value = (long)lsash;
      MrmFetchWidgetOverride(hierarchy,"pane",*plots,IndexedName("pane",i),arglist,numargs, &pane, &class);
      for (j=0;j<rows[i];j++)
      {
        Arg arglist[] = {{XmdsNpanWith,0}};
        arglist[0].value = (long)waveform;
        MrmFetchWidgetOverride(hierarchy,"wave",pane,IndexedName("wave",j),arglist,XtNumber(arglist),&waveform,&class);
...
static void /*XtActionProc*/MoveVerticalPane(Widget w, XButtonEvent *event, String *params, Cardinal *num_params)
{
  ...
  XtVaSetValues(separator, XmNleftOffset, min(max_offset, max(min_offset, event->x_root - main_x_root)), NULL);
...
static void Autoscale(Widget w, int stub, XmPushButtonCallbackStruct *cb)
{
  ...
  if (wave)
  {
    ...
    XtVaGetValues(wave, XmdsNxMin, &xminval, XmdsNxMax, &xmaxval, XmdsNyMin, &yminval, XmdsNyMax, &ymaxval, NULL);
    e->values[0] = *xminval;
    e->values[1] = *xmaxval;
    e->values[2] = *yminval;
    e->values[3] = *ymaxval;
...
static void Cut(Widget w, int stub, XmAnyCallbackStruct *callback_struct)
{
  ...
  else if (XtOwnSelection(w, XA_PRIMARY, XtLastTimestampProcessed(XtDisplay(w)), (XtConvertSelectionProc)ConvertSelection, 
                LoseSelection, NULL))
  {
    SelectedWidget = w;
...
static void Fit(Widget w, int stub, XmdsWavedrawFitCBStruct *cb)
{
  ...
  XtVaGetValues(w,XmdsNxIncreasing,&xincreasing,XmdsNyIncreasing,&yincreasing,
       XmdsNlowX,&lowx,XmdsNhighX,&highx,XmdsNlowY,&lowy,XmdsNhighY,&highy,XmdsNclosed,&closed,NULL);
    ...
    if (spline_points > 2)
    {
      ...
      else
      {
        ...
        if (closed)
        {
          ...
          memcpy(&spline_x[0],&spline_x[knotid+spline_points]-offset,offset * sizeof(float));
          memcpy(&spline_y[0],&spline_y[knotid+spline_points]-offset,offset * sizeof(float));
...
static void Move(Widget w,int stub,XmdsWavedrawValueCBStruct *cb)
{
  if (cb->event)
  {
    ...
    e = NewEvent(w, stub, cb->event, cb->reason,pointermode > XmdsPOINTER_MODE_SET_PEN ? CB_STRETCH : CB_MOVE);
    e->moved_idx = cb->idx;
    e->values[0] = cb->oldx;;
    e->values[1] = cb->newx;
    e->values[2] = cb->oldy;
...
static void SetAtLimits(Widget w, int stub, XmPushButtonCallbackStruct *cb)
{
  ...
  if (wave)
  {
    ...
    float *yminval;
    float *ymaxval;
    XtVaGetValues(wave, XmdsNlowX, &xminval, XmdsNhighX, &xmaxval, XmdsNlowY, &yminval, XmdsNhighY, &ymaxval, NULL);
    XtVaSetValues(wave, XmdsNxMin, xminval, XmdsNxMax, xmaxval, XmdsNyMin, yminval, XmdsNyMax, ymaxval, NULL);
    XtVaGetValues(wave, XmdsNxMin, &xminval, XmdsNxMax, &xmaxval, XmdsNyMin, &yminval, XmdsNyMax, &ymaxval, NULL);
    e->values[0] = *xminval;
    e->values[1] = *xmaxval;
    e->values[2] = *yminval;
