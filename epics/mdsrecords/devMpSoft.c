...
extern int writeMds(int nodeId, double *vals, int dtype, int preTrigger, int nVals, int dim1,
		    int dim2, int dataIdx, double period, double trigger, long epicsTime,
...
static long init_record(mdsputRecord * prec)
{
  ...
  case CA_LINK:
    ...
    break;
  default:
    recGblRecordError(S_db_badField, (void *)prec, "devMpSoft (init_record) Illegal INP field");
...
static long asynchRead(CALLBACK * pcallback)
{
  ...
  if (nRequest > 0) {
    prec->nord = nRequest;
    if (prec->tsel.type == CONSTANT && prec->tse == epicsTimeEventDeviceTime)
...
//extern int writeMds(int nodeId, double *vals, int nVals, int dim1, int dim2, int dataIdx, int isEpicsTime, long epicsTime, int preTrigger, double period, double trigger, char *errMsg);
    ...
    currTime =
	(unsigned long)prec->time.nsec + ((unsigned long)prec->time.secPastEpoch * 1000000000L);

    prec->errs =
	writeMds(prec->nid, prec->bptr, prec->ftvl, prec->pre, nRows, prec->dim1, prec->dim2,
		 prec->didx, prec->prd, prec->trig, currTime, prec->errm, prec->dbg);
...
static long read_mp(mdsputRecord * prec)
{
  ...
  epicsThreadCreate("MDSPUT_THREAD", epicsThreadPriorityHigh,
