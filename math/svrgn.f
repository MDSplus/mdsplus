...
!-----------------------------------------------------------------------
      ...
      Real      ra(*)      !input, [n] vector to be sorted
      Real      rb(*)      !output, [n] vector, may be ra sort without permutations
      Integer iperm(1)
      Call SVRGP(n,ra,rb,.false.,iperm)
      End
!-----------------------------------------------------------------------
      Subroutine SVRGP(n,ra,rb,perm,iperm)
      Integer      n      !input, number of elements in array
      Real      ra(*)      !input, [n] vector to be sorted
      Real      rb(*)      !output, [n] vector, may be ra
...
!+sort absolute without permutations
      Call SVRBP(n,ra,rb,.false.,iperm)
      End
!-----------------------------------------------------------------------
      Subroutine SVRBP(n,ra,rb,perm,iperm)
      Integer      n      !input, number of elements in array
      Real      ra(*)      !input, [n] vector to be sorted
      Real      rb(*)      !output, [n] vector, may be ra
        Logical perm    !input, true if iperm provided
