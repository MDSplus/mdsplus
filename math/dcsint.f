...
!-----------------------------------------------------------------------
	...
	Real*8	xdata(*)	!input, [ndata] abscissae
	Real*8	fdata(*)	!input, [ndata] ordinates
...
!-----------------------------------------------------------------------
	...
	Real*8	dleft	!input, derivative at endpoint
	Integer	iright	!input, end type: 0 = not-a-knot 1 = f', 2 = f''
	Real*8	dright	!input, derivative at endpoint
	Real*8	break(*)!output, [ndata] breakpoints
...
! Carl de Boor "A practical Guide to Splines" Springer-Verlag p.57
	...
	n = ndata
	If (n.le.0) Return
	Call DC9INI(ndata,xdata,fdata,fdata,break,c)
! dx and df/dx secant
	Do m = 2,n
...
30	Do j = n-1,1,-1
	...
	Enddo
	Call DC9FIN(ndata,break,c)
	End
!-----------------------------------------------------------------------
	Subroutine DCSHER(ndata,xdata,fdata,dfdata,break,c)
	Integer	ndata	!input, number of data points
	Real*8	xdata(*)!input, [ndata] abscissae
	Real*8	fdata(*)!input, [ndata] ordinates
	Real*8	dfdata(*)!input, [ndata] derivatives
	Real*8	break(*)!output, [ndata] breakpoints
	Real*8	c(4,*)	!output, [4,ndata] coefficients, f f' f'' f'''
!+Hermite cubic spline, given derivatives everywhere
! O(n) operations, O(step**4) approximation on uniform mesh
	Call DC9INI(ndata,xdata,fdata,dfdata,break,c)
	Call DC9FIN(ndata,break,c)
	End
!-----------------------------------------------------------------------
	Subroutine DCSAKM(ndata,xdata,fdata,break,c)
	Integer	ndata	!input, number of data points
	Real*8	xdata(*)!input, [ndata] abscissae
	Real*8	fdata(*)!input, [ndata] ordinates
	Real*8	break(*)!output, [ndata] breakpoints
...
! This method is C2 because DC9FIN makes it so.
	    ...
	    wp = abs(secant(3) - g)
	    c(2,2) = wp*gm + wm*g
	    If (wm + wp .gt. 0) c(2,2) = c(2,2) / (wm + wp)
	    c(2,1) = (3*gm - c(2,2))/2
	    gm = secant(n-1)
	    g = secant(n-2)
	    wm = abs(g - gm)
	    wp = abs(secant(n-3) - g)
	    c(2,n-1) = wp*gm + wm*g
	    If (wm + wp .gt. 0) c(2,n-1) = c(2,n-1) / (wm + wp)
	    c(2,n) = (3*gm - c(2,n-1))/2
	Endif
	Call DC9FIN(ndata,break,c)
	End
!-----------------------------------------------------------------------
	Subroutine DCSBES(ndata,xdata,fdata,break,c)
	Integer	ndata	!input, number of data points
	Real*8	xdata(*)!input, [ndata] abscissae
	Real*8	fdata(*)!input, [ndata] ordinates
	Real*8	break(*)!output, [ndata] breakpoints
...
! df/dx1=c2+2*c3*x=c2+2*(df(1)/dx1-c2)
	...
	End
!-----------------------------------------------------------------------
	Subroutine DC9FIN(ndata,break,c)
	Integer	ndata	!input, number of data points
	Real*8	break(*)!output, [ndata] breakpoints
	Real*8	c(4,*)	!modify, [4,ndata] coefficients, f f' f'' f'''
!+Internal, generate cubic coefficients, derivatives at left endpoint
! Assumes c(1,*) and c(2,*) have been set
	Integer	j,k,n
	Real*8	divdf1,divdf3,dtau
...
! last column is normally dummy, fill it too
	...
	End
!-----------------------------------------------------------------------
	Subroutine DC9INI(ndata,xdata,fdata,dfdata,break,c)
	Integer	ndata	!input, number of data points
	Real*8	xdata(*)!input, [ndata] abscissae
	Real*8	fdata(*)!input, [ndata] ordinates
	Real*8	dfdata(*)!input, [ndata] derivatives or dummy
	Real*8	break(*)!output, [ndata] breakpoints
	Real*8	c(4,*)	!output, [4,ndata] coefficients, f f' f'' f'''
!+Internal, initialize spline vectors
	...
	Do j = 1,ndata
		igot(i+j) = j
	Enddo
	Call DSVRGP(ndata,xdata,break,igot(igot(1)+1))
	Do j = 1,ndata
		k = igot(i+j)
		c(1,k) = fdata(j)
		c(2,k) = dfdata(j)
