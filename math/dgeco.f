...
30	CONTINUE
	...
	S=ABS(WK)
	SM=ABS(WKM)
	IF(ABS(ABD(M,K)).EQ.0.0D0)GOTO40
	WK=WK/(ABD(M,K))
	WKM=WKM/(ABD(M,K))
	GOTO50
40	CONTINUE
...
160	CONTINUE
	...
	DET(1)=ABD(M,I)*DET(1)
	IF(ABS(DET(1)).EQ.0.0D0)GOTO60
10	IF(ABS(DET(1)).GE.1.0D0)GOTO20
	DET(1)=TEN*DET(1)
	DET(2)=DET(2)-(1.0D0)
	GOTO10
20	CONTINUE
...
50	CONTINUE
	...
	IPVT(K)=L+K-M
	IF(ABS(ABD(L,K)).EQ.0.0D0)GOTO100
	IF(L.EQ.M)GOTO60
	T=ABD(L,K)
	ABD(L,K)=ABD(M,K)
	ABD(M,K)=T
...
120	CONTINUE
	...
	IF(ABS(QRAUX(J)).EQ.0.0D0)GOTO150
...
40	CONTINUE
	...
	DO60JJ=1,JU
	J=JU-JJ+1
	IF(ABS(QRAUX(J)).EQ.0.0D0)GOTO50
	TEMP=X(J,J)
	X(J,J)=QRAUX(J)
...
70	CONTINUE
	IF(.NOT.CQTY)GOTO100
	DO90J=1,JU
	IF(ABS(QRAUX(J)).EQ.0.0D0)GOTO80
	TEMP=X(J,J)
	X(J,J)=QRAUX(J)
...
190	CONTINUE
	...
	DO230JJ=1,JU
	J=JU-JJ+1
	IF(ABS(QRAUX(J)).EQ.0.0D0)GOTO220
	TEMP=X(J,J)
	X(J,J)=QRAUX(J)
	IF(.NOT.CR)GOTO200
...
30	CONTINUE
	DET(1)=D*DET(1)
	IF(ABS(DET(1)).EQ.0.0D0)GOTO80
40	IF(ABS(DET(1)).GE.1.0D0)GOTO50
	DET(1)=TEN*DET(1)
	DET(2)=DET(2)-(1.0D0)
	GOTO40
50	CONTINUE
...
70	CONTINUE
	...
	IF(ABS(E(L)).EQ.0.0D0)GOTO80
	IF(ABS(E(LP1)).NE.0.0D0)E(L)=SIGN(E(L),E(LP1))
	CALL DSCAL(P-L,1.0D0/E(L),E(LP1),1)
	E(LP1)=(1.0D0)+E(LP1)
80	CONTINUE
	E(L)=-(E(L))
	IF(LP1.GT.N.OR.ABS(E(L)).EQ.0.0D0)GOTO120
	DO90I=LP1,N
	WORK(I)=(0.0D0)
90	CONTINUE
	DO100J=LP1,P
...
180	CONTINUE
	...
	INTEGER CASE,J,JJ
	DO10INFO=1,N
	IF(ABS(T(INFO,INFO)).EQ.0.0D0)GOTO150
10	CONTINUE
	INFO=0
	CASE=1
