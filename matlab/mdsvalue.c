...
/* make_mdsvalue.m */
% make_mdsvalue.m
% Make the mdsvalue routine (not shared version)
% make sure ../mdsplus points to current version of mdsplus distribution
if(exist('debug','var'))
mex -DDEBUG mdsvalue.c ../mdsplus/mdstcpip/mdsipshr.c ../mdsplus/mdstcpip/mdsiputil.c -I../mdsplus/include -I../mdsplus/mdstcpip
else
mex mdsvalue.c ../mdsplus/mdstcpip/mdsipshr.c ../mdsplus/mdstcpip/mdsiputil.c -I../mdsplus/include -I../mdsplus/mdstcpip
end
#endif
...
#define int32 int
#endif
/* In this version, calls to mdsvalue is replaced */
...
cc/g_float/inc=matlab:[extern.include]/defi=V4 mdsvalue.c
my_cmexg :== @my_cmexg.com
my_cmexg mdsvalue sock serv servip
#endif
...
#define CONST const
#ifdef HOW_TO_COMPILE
mex -v mdsvalue.opt mdsvalue.c
mex -DDEBUG -v mdsvalue.opt mdsvalue.c
/* mdsvalue.opt */
...
/* use the class mdscvt to find if there is a conversion */
   if(mxIsClass(prhs[i],"mdscvt"))  {
      ...
      if(Tarray == NULL)
      	mexErrMsgTxt("mdsvalue: Conversion type in class mdscvt not given");
      else {
	 ...
	 switch(*convS)  {
	  ...
	  case 'q' :
	  case 'Q' :
	    mexErrMsgTxt("mdsvalue: Quadword not yet supported");
	    break;
#endif
	  default :
	    mexErrMsgTxt("mdsvalue: Conversion in mdscvt class unknown");
	 }
      }
      Tarray = mxGetField(prhs[i], 0, "x");
      if(Tarray == NULL)
      	mexErrMsgTxt("mdsvalue: No data in mdscvt class");
   } else
     Tarray = (mxArray *)prhs[i];      /* copy array pointer with CONST cast */
...
/* if not a normal matlab variable and conv was specified, signal alarm */
	 if(!mxIsClass(Tarray,"double"))
	   mexErrMsgTxt("mdsvalue: mdscvt specified but data was not double precision");
      }
	 ...
	 conv = -DTYPE_ULONG;
      	...
      	mexErrMsgTxt("mdsvalue: Can not handle this matlab data type, YET!");
...
/* clear the matricies */
	 ...
	 for (j = len; j--;) {
	    if (!mxIsChar(mxGetCell(Tarray, j)) || mxGetM(mxGetCell(Tarray, j)) > 1)
	      mexErrMsgTxt("mdsvalue: Cell array can only contain 1D character Arrays");
...
/* Remove a second dimension if 1 to reduce rank in remote server */
     ...
     mexErrMsgTxt("mdsvalue: Imaginary numbers not wanted");
#endif
   else
     mexErrMsgTxt("mdsvalue: Untreated data type encountered");
}
/* mdsput ======================================================================*/
static void Bmdsput(int nlhs,mxArray *plhs[],int nrhs,CONST mxArray *prhs[]) {
   int i;
...
/*     mexErrMsgTxt("mdsvalue: Connection to MDS server not open");*/
...
/* Check is string */
   ...
   if(!mxIsChar(prhs[2]))
     mexErrMsgTxt("mdsvalue: Third argument must be a string");
...
/* Check is string */
   if(!mxIsChar(prhs[opt]))
     mexErrMsgTxt("mdsvalue: First argument must be a string");
...
% This is a dummy file to call mdsvalue.mex##; calls the mex directly; rename mex if you want to use this function
% eg : mdsvalue('_a=2+3');
