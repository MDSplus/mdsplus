...
static void ResetWindow(String geometry, String title, String icon, int cols, int rows, Boolean bigfont, int btnwidth);
...
static    Boolean ConvertSelection(Widget w, Atom *selection, Atom *target, Atom *type, XtPointer *value,
							              unsigned int *length, int *format);
...
static void PasteComplete(Widget w, XtPointer cdata, Atom *selection, Atom *type, XtPointer value,
						              unsigned int *length, int *format);
static void PasteTypesComplete(Widget w, XtPointer cdata, Atom *selection, Atom *type, XtPointer value,
						              unsigned int *length, int *format);
/*------------------------------------------------------------------------------
...
/*------------------------------------------------------------------------------
 ...
 Executable:                                                                  */
  ...
  TopWidget = XtVaAppInitialize(&app_ctx, "DwPad", options, XtNumber(options), &argc, argv, fallback_resources,
                                XmNallowShellResize, 1, NULL);
  XtGetApplicationResources(TopWidget, &defaultfile, resources, XtNumber(resources), (Arg *) NULL, 0);
  if (defaultfile) defaultfile = XtNewString(defaultfile);
...
static void ApplyCustomizeWindow(Widget w, int *tag, XtPointer callback_data)
{
  ...
  Boolean   bigfont = XmToggleButtonGetState(XtNameToWidget(CustomizeWindowWidget, "font_box.big_font"));
  int       btnwidth;
  int       rows;
  int       cols;
  ApplyStatus = 0;
...
static void ApplyLabel(Widget w, int *tag, XtPointer callback_data)
{
  ...
  if (ReplaceString(&info->pad_label, XmTextGetString(XtNameToWidget(PadLabelWidget, "pad_label_expression")), 1))
  {
...
static void SetPadVideo(Widget w, Boolean reverse)
{
  if (w)
  {
    ...
    XtVaSetValues(w, XmNbackground, reverse ? fore : back, XmNforeground, reverse ? back : fore, NULL);
  }
}
...
static void MakeWaves(int cols, int rows)
{
  ...
  if (old_numwaves < num_waves)
  {
    for (i = old_numwaves; i < num_waves; i++)
    {
      ...
      XtVaSetValues(info->w, XmNrecomputeSize, 0, XmNhighlightThickness, 0, XtNtranslations, translations, XmNuserData, info, NULL);
    }
  }
  Columns = cols;
  Rows = rows;
...
static void ResetWindow(String geometry, String title, String icon, int cols, int rows, Boolean bigfont, int btnwidth)
{
  ...
  for (i = 0; i < num; i++)
  {
    ...
    XtVaSetValues(widgets[i], XmNwidth, width, XmNheight, height, XmNfontList, fontlist[bigfont & 1], NULL);
  ...
  if (!geometry || reset_dimensions)
    XtVaSetValues(MainWidget, XtNwidth, min(1024, XtWidth(PadWidget) + 6),
		  XtNheight, min(1024, XtHeight(PadWidget) + XtHeight(XtNameToWidget(MainWidget, "menu_bar")) + 6), NULL);
...
static void RestoreDatabase(String dbname)
{
  ...
  ResetWindow(GetResource(paddb, "Pad.geometry", "50x50+100+100"), GetResource(paddb, "Pad.title", "MDS Pad"),
	      GetResource(paddb, "Pad.icon_name", "Pad"), cols, rows, atoi(GetResource(paddb, "Pad.font", "0")),
						              ...
						              unsigned int *length, int *format)
{
  ...
  XtGetSelectionValue(w, XA_PRIMARY, req_type, (XtSelectionCallbackProc)PasteComplete, 0, XtLastTimestampProcessed(XtDisplay(w)));
  if (value)
    XtFree(value);
}
