...
static void ResetErrors()
{
  ...
  static struct dsc$descriptor const clear_messages =
      { 4, DSC$K_DTYPE_L, DSC$K_CLASS_S, (char *)&four };
...
static Boolean Error(Boolean brief, String topic, String * error, struct dsc$descriptor_xd *xd1,
		     struct dsc$descriptor_xd *xd2)
{
  if (brief)
    *error = XtNewString(topic);
    ...
    static struct dsc$descriptor const get_messages =
	{ 4, DSC$K_DTYPE_L, DSC$K_CLASS_S, (char *)&one };
...
/*    TREE$SET_STACK_SIZE(&one); */
  ...
  if (strlen(y)) {
    ...
    ResetErrors();
    if ((TDI$EXECUTE(&y_dsc, &sig) & 1) && (TDI$DATA(sig.dsc$a_pointer, &y_xd) & 1)
	&& (TDI$F_FLOAT(&y_xd, &y_xd) & 1)) {
	...
	if (strlen(x)) {
	  ...
	  status = (TDI$COMPILE(&x_dsc, &sig) & 1) && (TDI$DATA(&sig, &x_xd) & 1)
	      && (TDI$F_FLOAT(&x_xd, &x_xd) & 1);
	} else
	  status = (TDI$DIM_OF(&sig, &x_xd) & 1) && (TDI$DATA(&x_xd, &x_xd) & 1)
	      && (TDI$F_FLOAT(&x_xd, &x_xd) & 1);
	...
	if (status) {
	  ...
	  count =
	      (x_a->dsc$b_class == DSC$K_CLASS_A) ? min(x_a->dsc$l_arsize / sizeof(float),
							count) : 1;
	  if (count >= 1) {
...
Boolean EvaluateText(String text, String error_prefix, String * text_ret, String * error)
{
  ...
  if (strlen(text)) {
	...
	(TDI$DATA(&string_xd, &string_xd) & 1) && (TDI$ADJUSTL(&string_xd, &string_d) & 1)) {
