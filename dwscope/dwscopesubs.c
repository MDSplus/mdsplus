...
static String GetPlotResource(XrmDatabase db, String prefix, int row, int column, String plot_resource, String default_answer);
...
static int FlipBitsIfNecessary(int in)
{
  ...
  else
  {
    int out = 0;
    int i;
    for (i=0;i<32;i++) out |= ((in & (1 << i))!=0) << (31-i);
    return out;
  }
}
...
Boolean   ConvertSelectionToWave(Widget w, Atom type, unsigned int length, CutHeader *header, WaveInfo *wave)
{
  ...
  if (type == XA_DWSCOPE_PANEL)
  {
	...
	(length == (sizeof(CutHeader) + header->database_length + header->shot_length +
		    ...
		    header->xmin_length + header->xmax_length + header->ymin_length + header->ymax_length +
		    header->title_length + header->event_length + header->print_title_length +
		    header->pad_label_length)))
    {
    ...
    switch ((*ctx)++)
    {
     ...
     case 21:
      if (wave->_global.global_defaults != (strlen(wave->y) ? 0 : -1))
      {
        ...
        sprintf(answer, "%s.%s: %d\n", prefix, "global_defaults", FlipBitsIfNecessary(wave->_global.global_defaults));
      }
      break;
     default:
      answer = 0;
				           ...
				           unsigned int *length, int *format)
{
  ...
  if (target == XA_DWSCOPE_PANEL)
  {
    ...
    *type = target;
    *format = 8;
    *length = sizeof(CutHeader) + strlen(wave->database) + strlen(wave->shot) + strlen(wave->default_node) + strlen(wave->x) +
	      strlen(wave->y) + strlen(wave->xmin) + strlen(wave->xmax) + strlen(wave->ymin) + strlen(wave->ymax) +
	      strlen(wave->title) + strlen(wave->event) + strlen(wave->print_title) + strlen(wave->pad_label);
...
static void LoadGlobalDefaults(Widget w, WaveInfo *info)
{
  ...
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_default_text"), info->_global.global.default_node,1);
  ...
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_update_limits"), (int) info->_global.global.update,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_event_text"), info->_global.global.event,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_print_text"), info->_global.global.print_title,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_padlabel_expression"), info->_global.global.pad_label,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_x_grid_labels"), info->_global.global.x_grid_labels,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_y_grid_labels"), info->_global.global.y_grid_labels,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_show_mode"), info->_global.global.show_mode,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_step_plot"), info->_global.global.step_plot,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_x_grid_lines"), info->_global.global.x_grid_lines,1);
  XmToggleButtonGadgetSetState(XtNameToWidget(w, "def_y_grid_lines"), info->_global.global.y_grid_lines,1);
}
...
void      GetWaveFromDb(XrmDatabase db, String prefix, int row, int col, WaveInfo *info)
{
  ...
  info->_global.global_defaults = FlipBitsIfNecessary(
         atoi(GetPlotResource(db, prefix, row, col, "global_defaults", strlen(info->y) ? "0" : "-1")));
}
...
static void GetGlobalDefaults(Widget w, WaveInfo *info)
{
  ...
  info->_global.global.default_node = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_default_text"));
  ...
  info->_global.global.update = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_update_limits"));
  info->_global.global.event = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_event_text"));
  info->_global.global.print_title = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_print_text"));
  info->_global.global.pad_label = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_padlabel_expression"));
  info->_global.global.x_grid_labels = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_x_grid_labels"));
  info->_global.global.y_grid_labels = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_y_grid_labels"));
  ...
  info->_global.global.x_grid_lines = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_x_grid_lines"));
  info->_global.global.y_grid_lines = XmToggleButtonGadgetGetState(XtNameToWidget(w, "def_y_grid_lines"));
}
...
static String FixupBARS(String in)
{
  ...
  int lfs=0;
  String out;
  for (i=0;in[i] && in[i+1] && in[i+2];i++) if (in[i] == '|' && in[i+1] == '|' && in[i+2] == '|') {lfs++;i += 2;}
  out = XtMalloc(strlen(in)-lfs*2+1);
  for (i=0,j=0;in[i];i++)
  {
...
void PopupComplaint(Widget parent,String string)
{
  ...
  Widget w;
  int i;
  for (new_parent = parent; new_parent && !XtIsWidget(new_parent); new_parent = XtParent(new_parent));
  if (new_parent)
  {
    ...
    for (i = 0; i < 3; i++) XmStringFree((XmString)args[i].value);
