    ...
    )
{
  ...
  if (is_cdescr(source) || is_ddescr(source)) {
    ...
    k = p ? strlen(p) : 0;
		 ...
		 * Prepare dsc_ret ...
		 *------------------------------------------------------*/
  if (is_ddescr(dsc_ret)) {	/* if dsc_ret is a dynamic descriptor ... */
    if (!k) {
    ...
    )
{
  ...
  return (sts);
	 ...
	 * str_prefix:
	 ****************************************************************/
int str_prefix(struct descriptor *dsc_ret	/* <w> Destination string       */
    ...
    )
{
  ...
  for (i = 0; i < argc; i++) {
    if (is_cdescr(argList[i]) || is_ddescr(argList[i])) {
      ...
      p = dsc->dscA_pointer;
      kk = dsc->dscW_length;
      for (k = 0; k < kk && p[k]; k++) ;	/* null may occur before "kk" chars     */
    } else {
      p = argList[i];
      k = p ? strlen(p) : 0;
    }
  ...
  for (i = 0; i < argc && ichar < nchar; i++, ichar += k) {
    if (is_cdescr(argList[i]) || is_ddescr(argList[i])) {
      ...
      p = dsc->dscA_pointer;
      kk = dsc->dscW_length;
      for (k = 0; k < kk && p[k]; k++) ;	/* actual strlen may be < "kk" chars    */
    } else {
      p = argList[i];
      k = p ? strlen(p) : 0;
    }
