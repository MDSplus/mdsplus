...
int old_MDSEventAst(char *eventnam_in, void (*astadr) (void *, int, char *), void *astprm,
		    int *eventid)
{
  ...
  if (use_local) {
    ...
    *eventid = (int)event;
    ...
    for (num = 0; (event->pipe == INVALID_HANDLE_VALUE) && (status == ERROR_PIPE_BUSY); num++) {
      ...
      event->pipe =
	  CreateNamedPipe(pipename, PIPE_ACCESS_INBOUND,
			  PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT, 1, 512, 512, 1000,
			  0);
      if (event->pipe == INVALID_HANDLE_VALUE) {
	status = GetLastError();
      }
...
*/
	...
	if (((msginfo.msg_rtime >
	      0) ? (curr_time - msginfo.msg_rtime) : (curr_time - msginfo.msg_ctime)) >
	    DEAD_MSG_TIME) {
	  removeDeadMsg(key);
    ...
    if ((status =
	 msgsnd(msgid, &message, sizeof(message) - sizeof(message.mtype), /*IPC_NOWAIT */ 0)) == -1)
      perror("msgsend");
  }
  return status;
#endif
...
STATIC_ROUTINE void removeDeadMsg(int key)
{
  ...
  for (i = 0; i < MAX_ACTIVE_EVENTS; i++) {
	...
	for (curr = prev = shared_name[shared_info[i].nameid].first_id; curr != i && curr != -1;
	     curr = shared_info[curr].next_id) ;
	if (curr > -1 && prev > -1)
...
/* local stuff */
  ...
  if (!shared_info)
    return 0;			/*must follow MdsEventAst_ */
  name_in_use = 0;
  LockMdsShrMutex(&privateMutex, &privateMutex_initialized);
  for (i = 0; i < MAX_ACTIVE_EVENTS; i++)
    if ((i != local_eventid) && private_info[i].active
	&& !strcmp(evinfo->name, private_info[i].name))
      name_in_use = 1;
  if (!name_in_use) {
    getLock();
...
/* Local stuff */
    ...
    for (name_idx = 0; name_idx < MAX_EVENTNAMES && (shared_name[name_idx].refcount == 0
						     || strcmp(shared_name[name_idx].name, evname));
	 name_idx++) ;
