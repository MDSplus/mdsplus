...
/* Trigger */
   _source = int(if_error(data(DevNodeRef(_nid, _N_TRIG_SOURCE)),(DevLogErr(_nid, "Missing trigger source"); abort();)),7BU);
   _level = real(if_error(data(DevNodeRef(_nid, _N_TRIG_LEVEL)),(DevLogErr(_nid, "Missing trigger level"); abort();)),53BU);
