public fun Dt200Readchannel(in _board, in _channel, in _start, in _end, in _inc, optional  _coeffs)
{
  ...
  if (Dt200IsAcq216(_board)) {
    _dummy = Dt200WriteRsh(_board, "echo  "//_start//" >/dev/dtacq/sample_read_start");
    _lun = libMitDevicesIO->FOPEN(ref(_devname), "r");
    _buf = zero((_end - _start + 1)/_inc, 0w);
    _count = libMitDevicesIO->FREAD(ref(_buf), val(2), val(size(_buf)), val(_lun));
    libMitDevicesIO->FCLOSE(val(_lun));
  } else {
    _buf = zero((_end - _start + 1)/_inc, 0w);
