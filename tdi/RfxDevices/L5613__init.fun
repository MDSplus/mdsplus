public fun L5613__init(as_is _nid, optional _method)
{
	...
	DevNodeCvt(_nid, _N_CHAN_A_RANGE, [100.,50.,20.,10.,5.,2.,1.,0.5,0.2,0.],
		[0,1,2,4,5,6,8,9,10,12], _range_a=-1);
	if(_range_a == -1)
	{
	...
	_input_a_path = getnci(DevNodeRef(_nid, _N_CHAN_A_INPUT), 'fullpath');
	_output_a = compile(_input_a_path // ' * ' //_range_a_path // ' / `_constant ');
	DevNodeCvt(_nid, _N_CHAN_B_RANGE, [100.,50.,20.,10.,5.,2.,1.,0.5,0.2,0.],
		[0,1,2,4,5,6,8,9,10,12], _range_b=-1);
	if(_range_b == -1)
	{
	...
	_input_b_path = getnci(DevNodeRef(_nid, _N_CHAN_B_INPUT), 'fullpath');
	_output_b = compile(_input_b_path // ' * ' //_range_b_path // ' / `_constant ');
