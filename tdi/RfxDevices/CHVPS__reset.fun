public fun CHVPS__reset(as_is _nid, optional _method)
{
...
/*  CAMAC control definition	*/
	...
	_chan_bit    = zero(3, 0UW);

	_target = (_crate_num << 8) | _K_GROUP_A;   
...
/* Read channels defined into group A */
	...
	_f = _K_READ;
	_size = 3;
 	_status = DevCamChk(_name, CamQrepw( _name, _a, _f,  _size, _chan_bit, 16));
	if (~_status & 1)
	{
		DevLogErr(_nid, "CAMAC command esecution error"); 
		abort();
...
/* Delete channels defined into group A */
	_ok = 1;
	
	_target -= _K_GROUP_A;

    for (_i = 0;  _i < 40 && _ok; _i++)
	{
		if(_chan_bit[_i / 16] & 1 << mod(_i,16)) 
		{
