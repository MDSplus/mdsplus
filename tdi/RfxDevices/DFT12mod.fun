public fun DFT12mod(in _sig0, in _sig1, in _sig2, in _sig3, in _sig4, in _sig5, in _sig6, in _sig7, in _sig8, in _sig9, in _sig10, in _sig11, in _n)
{
...
/*  _phi = [];
  for( _i = 0; _i < _nSamples; _i++)
     _phi = [ _phi,  phi0 + _i * _deltaPhi];
*/	   
  ...
  _im = _sig0             * sin( -1 * _n * _phi[0]) +
   ...
   _scale = 1.0/ _nSamples;

   _mod = _scale * sqrt ( _re * _re + _im * _im);
   
   _modSig = make_signal(_mod, *, dim_of(_sig0));
