public fun RfxRotPertConfig(in _system, in _type, in _idx)
{
    ...
    if ( _type == "TITLE" )
	{
		...
		if( _modeName ==  "ROT.PERTURBATION" || _modeName ==  "MODE CONTROL + ROT.PERT. (FEEDBACK)" || _modeName ==  "VS+ROT.PERT.(FEEDBACK)" || _modeName == "VS+ROT.PERT.(FEEDFORW)" || _modeName ==  "Closer VS + Rot.Pert." || _modeName == "MODE CONTROL + ROT.PERT. (FEEDFORW)")
		{
			switch( _idx )
			{
				case (1)
