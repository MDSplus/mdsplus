...
/* Initialize Library if the first time */
 ...
 /* Create _rec_times global variable if the first time */ 
    if_error(_rec_times, public _DIO2_rec_times = 0;));
...
/* Open device */
	...
	_event_count = byte(0);
	_status = DIO2->DIO2_ER_GetEventCount(val(handle), ref(_event_count));
	if(_status != 0)
	{
		if(_nid != 0)
	...
	for(_i = 0; _i < _event_count _i++)
	{
		_curr_event = byte(0);
		_curr_time = long(0);
		DIO2->DIO2_ER_GetEvent(val(handle), ref(_curr_event), ref(_curr_time));
