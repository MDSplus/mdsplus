public fun TR10__init(as_is _nid, optional _method)
{
	...
	if(_remote != 0)
	{
		...
		execute(_cmd);
	    _status = MdsValue('TR10HWInit(0, $, $, $, $, $)', _board_id, _clk_div, _pts, _ext_trig, _trig_edge);
		MdsDisconnect();
		if(_status == 0)
		{
			DevLogErr(_nid, "Error Initializing TR 10 device: seet CPCI console for details");
			abort();
		}
	}
	else
