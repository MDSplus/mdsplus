public fun LASER_ND__add(in _path, out _nidout)
{
    ...
    DevAddNode(_path // ':PORT',		'TEXT', *, *, _nid);
    ...
    DevAddNode(_path//':TOTAL', 'SIGNAL', *, '/compress_on_put/nomodel_write', _nid);

    DevAddAction(_path//':INIT_ACTION',  'INIT',  'INIT', 50,'THOMSON_SERVER', getnci(_path, 'fullpath'), _nid);
    DevAddAction(_path//':STORE_ACTION', 'STORE', 'RESET', 70,'THOMSON_SERVER', getnci(_path, 'fullpath'), _nid);
    DevAddEnd();
}
