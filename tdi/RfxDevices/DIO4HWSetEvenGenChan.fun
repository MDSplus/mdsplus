Dublic fun DIO4HWSetEventGenChan(in _nid, in _board_id, in _chan, in _events, in _termination, in _rising)
{
	...
	write(*, 'DIO4HWSetEventGenChan', _board_id, _events);
...
/* Open device */
	...
	for(_i = 0; _i < 2; _i++)
	{
		...
		_status = DIO4->DIO4_IO_SetIOConnectionInput(val(_handle), val(byte(2 * _chan + _i + 1)),
...
write(*, 'FOUND FREE ENCODER CHANNEL AT POSITION '//_ev);
			    ...
			    _status = DIO4->DIO4_EC_SetEventEncoder(val(_handle), val(byte(_ev)), val(byte(_events[_i])),
				val(byte(_edge)), val(byte(_DIO4_EC_SOURCE_IO)), val(byte(2*_chan+_i+1))); 
			}
		    }
		}
		if(_status != 0)
