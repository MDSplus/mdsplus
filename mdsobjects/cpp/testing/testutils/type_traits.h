...
#define _MDS_TESTING_TYPE_TRAIT_IMPL_PARTIAL1(param, trait, spec, result)      \
  template <param> struct trait<spec> { typedef result type; };
...
#define _MDS_TESTING_TYPE_TRAIT_IMPL_SP1(trait, spec, result)                  \
  template <> struct trait<spec> { typedef result type; };                     \
  template <> struct trait<spec##Array> { typedef std::vector<result> type; }; \
  /**/

template <typename T> struct mds2cpp_typemap { typedef T type; };
