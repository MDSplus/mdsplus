...
_usage_table={'ANY':0,'NONE':1,'STRUCTURE':1,'ACTION':2,'DEVICE':3,'DISPATCH':4,'NUMERIC':5,'SIGNAL':6,
             'TASK':7,'TEXT':8,'WINDOW':9,'AXIS':10,'SUBTREE':11,'COMPOUND_DATA':12}
...
class Tree(object):
    ...
    def getDbi(self,itemname):
        itemlist={'NAME':(1,str,12),'SHOTID':(2,int),'MODIFIED':(3,bool),
                  ...
                  'MAX_OPEN':(7,int),'DEFAULT':(8,str,256),'OPEN_READONLY':(9,bool),
 ...
 #           if len(subtrees) > 0:
 #               included=subtrees.nid_number.compress(subtrees.include_in_pulse==1)
 #               included=included.tolist()
 #               included=_N.array(included)
 #               inc_len=len(included)
 #               included=included.ctypes.data
 #           else:
            included = 0
            inc_len = 0
    ...
    def getDefault(self):
        ...
        try:
            ...
            if status & 1:
                ans = TreeNode(int(nid.value),self)
            else:
                raise _statToEx(status)
        finally:
            Tree.unlock()
    ...
    def getNodeWildIter(self, name, *usage):
        ...
        try:
            while _TreeShr._TreeFindNodeWild(self.ctx,
                ...
                yield TreeNode(int(nid.value),self)
        except GeneratorExit:
            pass
    ...
    def setTimeContext(begin,end,delta):
        ...
        status = _TreeShr.TreeSetTimeContext(_C.pointer(descriptor(begin)),
    ...
    def setVersionDate(date):
        ...
        @rtype: None
        """
        status = _TreeShr.TreeSetViewDate(_C.c_int64(_mdsshr.DateToQuad(_ver.tobytes(date)).data()))
        if not status & 1:
    ...
    def cleanDatafile(self):
        ...
        try:
             status = _TreeShr._TreeCleanDatafile(self.ctx,
                                                  _C.c_char_p(_vers.tobytes(self.tree)),
                                                  _C.c_int32(int(self.shot)))
             if not status & 1:
                 raise _statToEx(status)
        finally:
...
class TreeNode(object):
    ...
    def addTag(self,tag):
        ...
        Tree.lock()
        try:
            status =_TreeShr._TreeAddTag(self.tree.ctx,self.nid,_C.c_char_p(str.encode(tag)))
            if not status & 1:
                raise _statToEx(status)
        finally:
            Tree.unlock()
    ...
    def updateSegment(self,start,end,dim,idx):
        ...
        try:
            status = _TreeShr._TreeUpdateSegment(self.tree.ctx,
                                                 _C.c_int32(int(self.nid)),
                                                 _C.pointer(descriptor(start)),
                                                 _C.pointer(descriptor(end)),
                                                 _C.pointer(descriptor(dimension)),
                                                 _C.int32(idx))
            if not status & 1:
                raise _statToEx(status)
        finally:
